...
class jdbookconsole_ctl_admin_goods_cat extends desktop_controller{
    /*
        %1 - id
        %2 - title
        $s - string
    ...
    */
    ...
    function index(){
        ...
        if (!$objCat->checkTreeSize('jdbook'))
        {
            //超过100条
			$tree = $objCat->get_cat_list(null,null,'jdbook');
            $this->pagedata['hidensub'] = false;
			if($_GET['jd_cat_id']){
				$editShowCatId = $_GET['jd_cat_id'];
				$editShowCatPath = $objCat->getList('cat_path',array('jd_cat_id'=>$editShowCatId));
				$editShowCatPath = explode(',',$editShowCatPath[0]['cat_path']);
				$editChildId = '';
				$editShowCatPath = array_reverse($editShowCatPath); 
				$childcat_info =  array();
				foreach($editShowCatPath as $k=>$v){
					$objCat->catMapTree = array();
					if(!$v){
						unset($editShowCatPath[$k]);
					}else{
						if(!$editChildId){
							$childcat_info = $this->getChildrenCats($v);
						}else{
							$cat_info = $this->getChildrenCats($v);
							foreach($cat_info as $key=>$val){
								if($val['jd_cat_id']==$editChildId){
									$cat_info[$key]['children'] = $childcat_info;
								}
							}
							$childcat_info = $cat_info;
						}
				...
				foreach($tree as $key=>$val){
					if($val['jd_cat_id']==$childcat_info[0]['pid']){
						$tree[$key]['children'] = $childcat_info;
					}
				}
        ...
        if($tree){
            foreach($tree as $k=>$v){
                $tree[$k]['link'] = array('jd_cat_id'=>array(
                    'v'=>$v['jd_cat_id'],
	...
	function getChildrenCats($catId){
		$objCat = app::get('jdsale')->model('goods_cat');
		$tree = $objCat->get_cat_list(false,$catId,'jdbook');
        if($tree){
        	foreach($tree as $k=>$v){
        		$tree[$k]['link'] = array('jd_cat_id'=>array(
        				'v'=>$v['jd_cat_id'],
    ...
    function _info($id=0,$type='add'){
        $objCat = app::get('jdsale')->model('goods_cat');
        $catList =$objCat->get_cat_list(null,null,'jdbook');
        ...
        $oGtype = app::get('jdsale')->model('goods_type');
     ...
     function save(){
		 $objCat = app::get('jdsale')->model('goods_cat');
		 $this->begin('index.php?app=jdsale&ctl=admin_goods_cat&act=index');
    ...
    function toRemove($nCatId){
      //  $this->begin('index.php?app=b2c&ctl=admin_goods_cat&act=index');
		...
		$nextCat = $objCat->getList('cat_id',array('parent_id'=>$cat_sdf['parent_id']));
		
		if(count($nextCat)>1){
			foreach($nextCat as $k=>$v){
				if($v['cat_id']!==$nCatId){
					$jumpCatId = $v['cat_id'];
					break;
				}
			}
			$this->begin('index.php?app=b2c&ctl=admin_goods_cat&act=index&cat_id='.$jumpCatId);
		}else{
			$this->begin('index.php?app=b2c&ctl=admin_goods_cat&act=index&cat_id='.$cat_sdf['parent_id']);
		}
    ...
    function edit($nCatId){
		$o = app::get('jdsale')->model('goods_cat');
		$catDate = $o->getRow('cat_id,disabled',array('jd_cat_id'=>$nCatId));
		$this->pagedata['data'] = $catDate;
        $this->display('admin/goods/category/info.html');
    }
    ...
    function update(){
        ...
        foreach( $_POST['p_order'] as $k => $v ){
            $o->update(array('p_order'=>($v===''?null:$v)),array('jd_cat_id'=>$k));
        }
        $o->cat2json();
    ...
    function getChildNode(){
        ...
        $objCat = app::get('jdsale')->model('goods_cat');
        $tree = $objCat->get_cat_list(false,$_POST['catId'],'jdbook');
        
        /** modified by PanQiaolin **/
        if($tree){
        	foreach($tree as $k=>$v){
