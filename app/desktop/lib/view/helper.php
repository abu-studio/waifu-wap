...
class desktop_view_helper{
	...
	function block_permissionread($params, $content, &$tpl){
        //没有权限则增加属性readonly='true',以使不能编辑-@zhyu.与displayed区别是表单提交的时候可以$_POST获取值
        if($params['perm_id'] && !$tpl->has_permission($params['perm_id'])){
            if($params['noshow']){return null;}
            $content = preg_replace('/readonly \s*=?\s*((["\']?)[\w\s\r\n-]*\2)?/i', '', $content);
            $content = preg_replace('/(<input|<select|<textarea|<button)/i', '$1 readonly ="true" onfocus="this.blur()"', $content);
            return $content;
        }
        return $content;
    }
