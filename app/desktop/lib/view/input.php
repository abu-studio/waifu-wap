...
class desktop_view_input{
    ...
    function input_object($params){
		...
		if('jdsale_mdl_goods_cat'==get_class($o)){
			$key = $params['key']?$params['key']:'jd_cat_id';
            $cat=kernel::single('desktop_user')->get_user_cat();
            if($cat!==false && !empty($cat['topCat'])){
                if(isset($tmp_filter['cat_id'])){
                    if(!is_array($filter['cat_id'])){
                   ...
                   if(empty($tmp_filter['cat_id'])){
                       $tmp_filter['jd_cat_id']=$cat['topCat'];
                   }
                }else{
                    $tmp_filter['jd_cat_id']=$cat['topCat'];
                }
        ...
        if(isset($params['multiple']) && $params['multiple']){
            if(isset($params['items']) && count($params['items'])){
                ...
                if(!empty($params['items'])){
                    $item_price = 0;
                    if(!empty($params['items'])){
                        foreach($params['items'] as $card_key=>$card_value){
                            $item_price += $card_value['price'];
                        }
                    }
                }
                $params['item_price'] = $item_price ? $item_price : 0;
				$params['itemnums']=count($params['items']);
            }
