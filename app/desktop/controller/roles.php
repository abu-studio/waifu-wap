...
class desktop_ctl_roles extends desktop_controller{
    ...
    function procHTML($tree){
	  ...
	  function maintenance(){
		if(!$_POST){
			$this->pagedata['maintenance']['is_open'] = $this->app->getConf('site.maintenance');
			$this->page('maintenance.html');
		}else{
            $this->begin('index.php?app=desktop&ctl=roles&act=maintenance');
            
            if ($_POST['maintenance'] == 'true')
                $this->app->setConf('site.maintenance', 1);
            else
                $this->app->setConf('site.maintenance', 0);
            
            $this->end(true, app::get('desktop')->_("设置成功！"));
		}
    }
}
