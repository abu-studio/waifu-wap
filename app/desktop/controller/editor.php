...
EOF;
    ...
    function object_rows(){
        if($_POST['data']){
			...
			//如果是京东分类，将取值cat_id 改取jd_cat_id
			if(get_class($obj) == 'jdsale_mdl_goods_cat'){
				$schema['idColumn'] = 'jd_cat_id';
			}
            ...
            //新增模块
            if($_POST['cardcoupons'] == "true"){
                $this->pagedata['_input']['cardcoupons'] = true;
                if($_POST['change_way'] != "num"){
                    $item_price = 0;
                    if(!empty($this->pagedata['_input']['items'])){
                        foreach($this->pagedata['_input']['items'] as $card_key=>$card_value){
                            $item_price += $card_value['price'];
                        }
                    }
                    $this->pagedata['_input']['change_way'] = $_POST['change_way'];
                    $this->pagedata['_input']['item_price'] = $item_price;
                }else{
                    $this->pagedata['_input']['change_way'] = 'num';
                }
                $this->pagedata['_input']['itemnums'] = count($this->pagedata['_input']['items']);
                $this->display('finder/input-cardrow.html');
            }else{
