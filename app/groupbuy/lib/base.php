...
class groupbuy_base
{
    ...
    public function getInfo($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
         $sql = "select gd.name as g_name,gd.image_default_id as g_image,c.cat_name,a.start_time,a.end_time,g.id,g.gid,g.cat_id,g.aid,g.last_price,g.nums,g.remainnums,g.personlimit,gd.price as g_price,g.image_codeid,gd.mktprice as g_mktprice from sdb_groupbuy_groupapply as g".
        ...
        if($filter['scontent']){
           $sql=$sql.' and gd.name LIKE "%'.$filter['scontent'].'%"';
        }
        ...
        if($rs && !empty($rs)){
            foreach($rs as $k=>$v){
                ...
                $result[$k]['g_mktprice']=$v['g_mktprice'];
                ...
                $result[$k]['image_codeid']=$cimg[0]['image_id'];
