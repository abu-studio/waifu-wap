...
class business_finder_storemanger{
    ...
    public function column_control($row){
        ...
        $arr_link = array(
            ...
            'detail_service'=>array(
                    'href'=>'javascript:void(0);',
					'submit'=>'index.php?'.utils::http_build_query($arr).'&finderview=detail_service&id='.$row['store_id'].'&_finder[finder_id]='.$_GET['_finder']['finder_id'],
    ...
    function detail_accreditation($store_id){
        $render = $this->app->render();
        $arr_info = app::get('business') -> model('storemanger') -> dump($store_id, 'company_idname,company_no,company_codename,company_taxno,image,image_id,image_cid,image_codeid,image_taxid', 'default');
        $arr_info['image'] =    base_storager::image_path($arr_info['image']);
        $arr_info['image_id'] =    base_storager::image_path($arr_info['image_id']);
        $arr_info['image_cid'] =    base_storager::image_path($arr_info['image_cid']);
        $arr_info['image_codeid'] =    base_storager::image_path($arr_info['image_codeid']);
        $arr_info['image_taxid'] =    base_storager::image_path($arr_info['image_taxid']);
        $render->pagedata['shopinfo']=$arr_info;
        return $render->fetch('admin/store/detail/accreditation.html');
    }
