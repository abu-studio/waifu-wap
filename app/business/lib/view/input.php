  ...
  function input_vimage($params){
    ...
    function input_vobject($params){
        ...
        if(isset($params['multiple']) && $params['multiple']){
            ...
            $url = $path.'?app=desktop&act=alertpages&goto='.urlencode('index.php?app=desktop&ctl=editor&act=finder_common&sign=site&app_id='.$app_id.'&'.$object.$getdata).'&sign=site';
    ...
    {   
		...
		$arr_number=app::get('business')->model('customer_service')->getList('number,type',array('store_id'=>$params['store'],'is_defult'=>'1'));
		$number=$arr_number[0];
...
EOD;
    ...
    public function header_shop_set_extends(&$arr_shop){
		...
		$shop['url']['fav_url'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_member','act'=>'ajax_fav'));//团购加载至此处时会没有b2c_lib_site_view helper.php 中function_header方法内的$shop['url']
