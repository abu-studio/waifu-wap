      ...
      del:function(obj){
        if(obj.get('ident') && $('typeDetailView')){
          ...
          if(confirm(LANG_goodseditor['comfirmDelPic'])){
              ...
              obj = $(obj);
              var pic_box=obj.getParent('.gpic-box'),picNext=pic_box.getNext('.gpic-box'),pic_first=pic_box.getParent('.all-pics').getFirst('.gpic-box');
              try{
              if(obj.get('ident')){
              ...
              if(picNext){
				  picNext.getElement('.gpic').onclick();
			  }else{
				  pic_first.getElement('.gpic').onclick();
			  }
          }
      },
...
var goodsEditFrame = (function(op){
  ...
  var subGoodsForm = function (event,sign){
      var _catObjList = document.getElementsByName('goods[category][cat_id]');
      if(typeof(_catObjList[0])!='undefined' && _catObjList[0].value < 1){
          alert('请先选择分类');
          return false;
      }
