...
class business_ctl_site_storemsg extends business_ctl_site_member
{
	...
	public function store_msg($type='',$nPage=1)
	{
        ...
        $oMsg = app::get('b2c')->model('member_comments');
        //全部
        if(!$type){ 

            $row = $oMsg->getList('*',array('to_id' => $this->app->member_id,'has_sent' => 'true','for_comment_id' => 'all','inbox' => 'true')); //echo "<pre>";print_r($row);exit;
            $aData['data'] = $row;
            #print_r($row);
     ...
     function del_in_box_msg(){
        ...
        if(!empty($_POST['delete']))
        {
            // $objMsg = kernel::single('b2c_message_msg');
             $objMsg = app::get('b2c')->model('member_comments');
