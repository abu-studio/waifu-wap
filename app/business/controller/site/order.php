...
class business_ctl_site_order extends business_ctl_site_member{
     ...
     * @params string order id
     */
    public function gochange($order_id){
		$obj_delivery = app::get('b2c')->model('delivery');
		$obj_orders = app::get('b2c')->model('orders');

		$order_info = $obj_orders->dump($order_id);
		$deliveries = $obj_delivery->getList('*',array('order_id'=>$order_id));

		$this->pagedata['order'] = $order_info;

		$areas = explode(':',$this->pagedata['order']['consignee']['area']);
		$this->pagedata['order']['consignee']['area_show'] = $areas[1];
		$this->pagedata['data']['deliveries'] = $deliveries;
		$this->output();
    }
     ...
     * 修改订单运单号
     */
    public function change_logi_no(){
		$post = $_POST;
		$obj_delivery = app::get('b2c')->model('delivery');
		$objorder_log = $this->app->model('order_log');

		$op_id = $this->member['member_id'];
		$op_name = $this->member['uname'];

		$old_delivery = $obj_delivery->dump($post['delivery_id']);

		$this->begin($this->gen_url(array('app' =>'business','ctl'=>'site_member','act' =>'orderdetail','arg0'=>$post['order_id'])));
		if($obj_delivery->update(array('logi_no'=>$post['logi_no']),array('delivery_id'=>$post['delivery_id'],'order_id'=>$post['order_id']))){
			$log_text = '物流单号【'.$old_delivery['logi_no'].'】修改为【'.$post['logi_no'].'】';
			$log_addon = serialize(array('logi_no'=>$post['logi_no']));
			$sdf_order_log = array(
				'rel_id' => $post['order_id'],
				'op_id' => $op_id,
				'op_name' => $op_name,
				'alttime' => time(),
				'bill_type' => 'order',
	...
	public function to_export_dly(){
        $this->page('site/order/export_dly.html',true,'business');
	}

    /**
     * 导出批量发货模板
     */
    public function export_dlybatch(){
        $charset = kernel::single('base_charset');
        //商品列表
		$cols = "order_id,'' as logi_no,ship_area,ship_addr,'' as da_id,'' as corp_id,ship_name,ship_mobile,'' as good_name,memo";
		$type_sql="pay_status='1' and ship_status='0' and status='active'";
		$arrayorser = $this->get_orders_new($cols,$type_sql);
        

		$content = array();
		foreach($arrayorser as $k=>$v){
		...
		$title = '"订单号(order_id)","运单号(logi_no)","收货地区(ship_area)","收货地址(ship_addr)","发货地址ID(da_id)","物流公司ID(corp_id)","订购人姓名","手机号码","订购商品名","订单留言"';
        ...
        header("Content-Disposition: attachment; filename=dlybatch_".date('YmdHis').".csv");
	...
	public function export_ads(){
		...
		foreach($addresses as $k=>$v){
			...
			$v = array(
				'name'=>$v['region'].$v['address'].','.$v['uname'].','.$v['phone'].','.$v['mobile'],
				'corp_id'=>$v['da_id'],
			);
			$content[] = '"'.implode('","',$v).'"';
		}

		$title = '"发货地址(name)","发货地址ID(da_id)"';
	...
	public function export_dlytype(){
        $obj_corp = app::get('business')->model('dlycorp');
        $sto= kernel::single("business_memberstore",$this->app->member_id);        
        $filter["store_id"] = $sto->storeinfo['store_id'];
        $corp_id = $obj_corp->getList('corp_id',$filter);
        ...
        $dlycorp = $this->app->model('dlycorp');
        $corplist = $dlycorp->getList('*',$c_filter);
		...
		foreach($corplist as $k=>$v){
			...
			$content[] = '"'.implode('","',array($v['name'],$v['corp_id'])).'"';
		}

		$title = '"物流公司名称(name)","物流公司ID(corp_id)"';
        ...
        header('Expires:0');
        header('Pragma:public');
        header("Content-Disposition: attachment; filename=dlycorp.csv");
		//echo mb_convert_encoding($out, 'GBK', 'UTF-8');exit;
        echo $charset->utf2local($out,'zh');exit;
	}

	function get_orders($cols,$type_sql){
        ...
        $str_sql = "SELECT ".$cols." FROM ".$sdb."b2c_orders WHERE store_id=".$store_id;
        $str_sql .=" AND ".$type_sql;
        $str_sql .=" AND pay_status != '0' ";
		return $order->db->select($str_sql);
	}

    function get_orders_new($cols,$type_sql){
        $order = app::get('business')->model('orders');
        $sdb = kernel::database()->prefix;
        $sto= kernel::single("business_memberstore",$this->app_b2c->member_id);
        $sto->process($this->app_b2c->member_id);
        ...
        $data = $order->db->select($str_sql);
        $order_item = app::get("b2c")->model("order_items");
        $arr = array();
        header('Content-Type:text/html; charset=utf-8');
        foreach($data as $k_1=>$v_1){
                //重新订单zxc
                // $str_sql_order = "SELECT distinct order_id FROM sdb_b2c_order_items where order_id=".$v_1['order_id'];
                $str_sql_order = "select distinct order_id from sdb_b2c_order_items as oi INNER JOIN sdb_b2c_goods as bg on oi.goods_id=bg.goods_id where order_id=".$v_1['order_id'];
                $data_order_id = $order_item->db->select($str_sql_order);
                // $data_order_id = $order_item->getlist("*",array("order_id" => $v_1['order_id']));
                foreach($data_order_id as $k2=>$val_2){
                    
                    if($v_1['order_id']==$val_2['order_id']){
                        $arr[]=$v_1;
                    }
        ...
        if(!empty($data)){
            ...
            foreach($data as $k=>$v){
                ...
                $order_item_data = $order_item->getlist("bn,name,nums",array("order_id" => $v['order_id']));
                if(!empty($order_item_data)){
                    $tmp = "";
                    foreach($order_item_data as $key=>$value){
                        //如果还有双引号，先将双引号转义，避免两边加了双引号后转义错误 zxc 
                        $value['name'] =str_replace("\"","\"\"",$value['name']);

                        $tmp .= "货号:".$value['bn']."".$value['name']." * ".$value['nums']."\n";
                    }
                }
                $data[$k]['good_name'] = $tmp;
    ...
    function import_dlybatch(){
		$this->begin($_SERVER['HTTP_REFERER']);

		$dlybatch = $this->read_csv($_FILES['dlybatch_file'],$msg);
		if(!$dlybatch){
			$this->end(false,$msg);
		}
		//数据校验
		$order_id_arr=array();
		foreach($dlybatch as $key=>$row){
            $row['order_id'] = trim($row['order_id']);
			if(! $row['order_id']){
				$this->end(false,'第'.$key.'条数据订单号为空！');
			}

            $row['logi_no'] = trim($row['logi_no']);
			if(! $row['logi_no']){
				$this->end(false,'第'.$key.'条数据运单号为空！');
			}

            $row['da_id'] = trim($row['da_id']);
			if(! $row['da_id']){
				$this->end(false,'第'.$key.'条数据发货地址为空！');
			}

            $row['corp_id'] = trim($row['corp_id']);
			if(! $row['corp_id']){
				$this->end(false,'第'.$key.'条数据物流公司为空！');
			}
			$order_id_arr[]=$row['order_id'];
			$da_id_arr[]=$row['da_id'];
			$corp_id_arr[]=$row['corp_id'];
		}
		if($order_id_arr){
			$cols = "order_id";
			$type_sql="pay_status='1' and ship_status='0' and status='active'";
			$old = $this->get_orders($cols,$type_sql);
			$all_order = array();
			foreach($old as $k=>$v){
			...
			if(is_array($repeat_arr) && !empty($repeat_arr)){
				...
				$this->end(false,'存在重复订单号：'.implode(',',$repeat_arr));
			}
			//订单号存在性校验
			$not_exists = array();
			...
			if(count($not_exists)>=1){
				$this->end(false,'订单号不存在：'.implode(',',$not_exists));
			}
			//发货地址存在性校验
			$not_exists = array();
			$obj_ads = app::get('business')->model('dlyaddress');
			...
			foreach(array_unique($da_id_arr) as $key=>$val){
				if(!in_array($val,$all_da_id)){
					$not_exists[] = $val;
				}
			}
			if(count($not_exists)>=1){
				$this->end(false,'发货地址ID不存在：'.implode(',',$not_exists));
			}
			//物流公司存在性校验
			$not_exists = array();
			$obj_corp = app::get('business')->model('dlycorp');
			$corp_id = $obj_corp->getList('corp_id',array('store_id'=>$sto->storeinfo['store_id']));
			foreach($corp_id as $k=>$v){
				$all_corp_id[] = $v['corp_id'];
			}
			foreach(array_unique($corp_id_arr) as $key=>$val){
				if(!in_array($val,$all_corp_id)){
					$not_exists[] = $val;
				}
			}
			if(count($not_exists)>=1){
				$this->end(false,'物流公司ID不存在：'.implode(',',$not_exists));
			}
			foreach($dlybatch as $key=>$row){
				$obj_order = &$this->app->model('orders');
				$rp = app::get('aftersales')->model('return_product');
				$return_id = $rp->getRow('*',array('order_id'=>$row['order_id'],'refund_type'=>'1'));
				if($return_id){
					$obj_return_policy = kernel::single('aftersales_data_return_policy');
				...
				$objOrder = &$this->app->model('orders');
				$subsdf = array('order_objects'=>array('*',array('order_items'=>array('*',array(':products'=>'*')))));
				$aORet = $objOrder->dump($row['order_id'],'*',$subsdf);
				$sdf = array (
					...
					'is_protect' => 'false',
					'cost_freight' => '',
					'ship_name' => $aORet['consignee']['name'],
					'ship_tel' => $aORet['consignee']['telephone'],
					'ship_mobile' => $aORet['consignee']['mobile'],
					'ship_zip' => $aORet['consignee']['zip'],
					'ship_area' => $aORet['consignee']['area'],
					'ship_addr' => $row['ship_addr'],
					'memo' => '',
					'deladdress' => $row['da_id'],
					'logi_id' => $row['corp_id'],
					'logi_no' => $row['logi_no'],
				);
				$send=array();
				foreach($aORet['order_objects'] as $ovalue){
					foreach($ovalue['order_items'] as $oikey=>$oivalue){
						$send[$oikey] =$oivalue['quantity']-$oivalue['sendnum'];
					}
				}
				$sdf['send'] = $send;
				$arrMember = $this->get_current_member();
				...
				{            
					...
					$obj_apiv->rpc_caller_request($req_arr, 'orderupdatecrm');
					$obj_corp = app::get('b2c')->model('dlycorp');
					$code = $obj_corp->dump(array('corp_id'=>$sdf['corp_id']));
					if($code['corp_code'] == 'BAM'){
						$data['confirm_time'] = time()+(app::get('b2c')->getConf('member.to_finish_BAM'))*86400;
					}else{
						$data['confirm_time'] = time()+(app::get('b2c')->getConf('member.to_finish'))*86400;
			...
			$this->end(true, app::get('b2c')->_('批量发货成功！'));
		}else{
			$this->end(false, app::get('b2c')->_('导入数据为空！'));
		}
