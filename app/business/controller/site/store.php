    ...
    */
    ...
    function addcoupon(){
     ...
     function _prepareData($aData) {
        ...
        // 启用状态
        $aResult['rule']['status'] = empty($aData['coupon']['cpns_status'])?'false' : 'true'; // 和优惠券的状态一致
        $aResult['rule']['rule_type'] = 'C';            // 规则类型


        $aResult['rule']['name'] = app::get('b2c')->_("优惠券规则").'-'.$aData['coupon']['cpns_name']; // 名称
        if( !$aResult['rule']['name'] ) $this->end( false,'优惠券规则名称不能为空！' );
    ...
    function _checkData($aData) {
        ...
        if(empty($aData['coupon']['cpns_id'])) {
            if($oCoupon->checkPrefix($this->cup[$aData['coupon']['cpns_type']].$aData['coupon']['cpns_prefix'])){
                $this->end(false, app::get('b2c')->_('优惠券号码已经存在'));
            }
        }
    }
     ...
     function del_storecoupon() {
        ...
        {
            echo json_encode(array('status' => 'false', 'message' => '<span class="font-red">&nbsp;' . app :: get('b2c') -> _('优惠券删除失败。') . '</span>'));

        }
        else
        {
            echo json_encode(array('status' => 'success', 'message' => '<span class="font-green">&nbsp;' . app :: get('b2c') -> _('优惠券删除成功。') . '</span>')); 
   ...
   public function  edit_storecoupon(){
    ...
    function earnestdeposit(){
        ...
        foreach($aOld as $val){
            ...
            if(($val['app_id']!='offline')&&($val['app_id']!='sfscpay'))$aData[] = $val;
        }
