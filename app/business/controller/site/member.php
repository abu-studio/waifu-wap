...
class business_ctl_site_member extends b2c_ctl_site_member
{
    ...
    function _editor($cat_id,$type_id){
        ...
        $this->pagedata['brandList'] = $this->app->model('brand')->getList('*',array('brand_id'=>$aBBrand),0,-1,array("convert(brand_name using gbk)"));
    ...
    public function goods_add()
    {
		...
		//发票选择框
		$dbschema_goods = $oGoods->get_schema();
		$ticket_type = $dbschema_goods['columns']['ticket_type']['type'];
		$this->pagedata['ticket_type'] = $ticket_type;		
...
//        $this->pagedata['spec_default_pic'] = $this->app_b2c->getConf('spec.default.pic');
    ...
    function goods_info($filter, $switch, $orderBy = 0, $page = 1){
        ...
        $return = array();
        $orderBy = $orderBy?$orderBy:'uptime desc,buy_count desc';
        ...
        $aGoods = $objGoods->getList('is_tui,is_new,goods_id,goods_kind,name,bn,store,price,buy_count,uptime,thumbnail_pic,image_default_id,store_id', $filter, $pageLimit*($page-1), $pageLimit, $orderBy);
        ...
        if(!empty($goods_ids)){
            ...
            foreach((array)$aGoods as $key => $value){
				...
				if($value['goods_kind'] == 'jdgoods' || $value['goods_kind'] == 'jdbook'){
					$aGoods[$key]['image_default_id'] = jdsale_goods_import::$image_url_n3.$value['image_default_id'];
					$aGoods[$key]['goods_app'] = 'jdsale';
				}
            }
        }
        $return['goods'] = $aGoods;
		...
		//判断是否为京东商品店铺
		if(app::get('site')->getConf('jdsale.shopId') == $this->store_id){
			$catList =$objBGoods->get_jdcat_list($this->region_id);
			$return['jdstore'] = true;
		}else{
    ...
    function toAdd(){
        define('FILE_STORAGER','ftpsystem');//定义FTP上传
        set_time_limit(0);
        $oStoregrade = $this->app_current->model('storegrade');
        $aGrade = $oStoregrade->getList('goods_num', array('grade_id'=>$this->grade_id));
		...
		if (!isset($_POST['gdlytype']) || empty($_POST['gdlytype'])){
            $this->splash('failed',$url , app::get('b2c')->_('运费模板必填'),'','',true);
        }
        ...
        if(isset($_POST['specall']) && !empty($_POST['specall'])){
            if(is_array($_POST['goods']['product'])){
				if(!empty($_POST['goods']['product'])){
					...
					$this->splash('failed',$url , app::get('b2c')->_('未生成相应货品'),'','',true);
				}
            }else{
                $this->splash('failed',$url , app::get('b2c')->_('未选定全部规格'),'','',true);
            }
		...
		//增加卖家新增，编辑商品若是不平台不允许上架的商品则status为false；检测编辑的商品名称，价格，库存，详情，是否包邮是否已经修改，若修改则下架商品
		$this->check_goods_status($goods);
		//end
...
//                }
	...
	function check_goods_status(&$goods){
		if($goods['goods_id']){
			$goods_data=kernel::single('b2c_mdl_goods')->dump(array('goods_id'=>$goods['goods_id']),'*','default');
			//先验证名称，是否包邮，详情是否被修改
			if($goods_data['name']!=$goods['name']||$goods_data['freight_bear']!=$goods['freight_bear']||$goods_data['description']!=$goods['description']){
				$goods['status']='false';
			}else if(count($goods['product'])!=count($goods_data['product'])){	//判断商品的货品数量是否一致，如果不一致说明卖家删除或者新增了货品
				$goods['status']='false';
			}else{
				foreach($goods['product'] as $goodproduct_key=>$goodproduct_value){
					//判断是否是新增的货品，标识是新增的货品key是以new_开头，非新增的是以product_id为key
					if(is_numeric($goodproduct_key)){
						if($goods_data['product'][$goodproduct_value['product_id']]['price']['cost']['price']!=$goodproduct_value['price']['cost']['price']){
							$goods['goods_application']='0';
							$goods['status']='false';break;
						}
						if(floor($goods_data['product'][$goodproduct_value['product_id']]['store'])!=floor($goodproduct_value['store'])||$goods_data['product'][$goodproduct_value['product_id']]['price']['mktprice']['price']!=$goodproduct_value['price']['mktprice']['price']){
							$goods['status']='false';break;
						}
					}else{
						$goods['goods_application']='0';
    ...
    function toAddCorp(){
        ...
        $objCorp = $this->app_current->model('dlycorp');
        $objCorp->delete(array('store_id' => $this->store_id));
    ...
    public function seller_order($type='',$order_id='',$goods_name='',$goods_bn='',$time='',$pay_status='',$nPage=1)
    {
        ...
        foreach($aData['data'] as $k=>$v) {
            ...
            //获取运单号
            $deliveries = $obj_delivery->getList('*',array('order_id'=>$v['order_id']));
            $logi_nos = array();
            foreach($deliveries as $logi_k=>$logi_v){
                $logi_nos[] = $logi_v['logi_no'];
            }
            $aData['data'][$k]['logi_nos'] = implode(',<br/>',$logi_nos);
        }
    ...
    */
    private function get_search_order_ids($type='',$time=''){
		...
		if($type == 'nopayed'){
			...
			$type_sql=" pay_status='1' and ship_status='0'  and status='active' ";
		}else if($type == 'finish'){
    ...
    function index() {
     ...
     //秒杀活动
        ...
        //删除的评价
        $filter =array('item_type'=>'member_comments','app_key'=>'b2c');
        ...
        foreach($recycle as $key=>$value){
			if($value['item_sdf']['time']>=$time){
				$i++;
				if($value['item_sdf']['goods_point']['goods_point']<4){
					$a++;
				}
			}
			if($value['item_sdf']['time']>=strtotime($firstday)&&$value['item_sdf']['time']<=strtotime($lastday)){
				$j++;
				if($value['item_sdf']['goods_point']['goods_point']<4){
					$b++;
				}
        ...
        if( $countorder>0){
            //退款率
            $this->pagedata['refunds_lv1']=(round(strval($this->pagedata['refunds_count1'])/ $countorder,4)) * 100;

            //纠纷退款率
            $this->pagedata['refunds_ilv1']=(round(strval($this->pagedata['refunds_icount1'])/strval($this->pagedata['refunds_count1']),4)) * 100;
        } else {
        ...
        if($xcountorder>0){
            //退款率
            $this->pagedata['refunds_lv2']=(round(strval($this->pagedata['refunds_count2'])/$xcountorder,4)) * 100;
             //纠纷退款率
             $this->pagedata['refunds_ilv2']=(round(strval($this->pagedata['refunds_icount2'])/strval($this->pagedata['refunds_count2']),4)) * 100;
        } else {
    ...
    protected function output($app_id = 'business') {
     ...
     private function get_cpmenu(){
		...
		//投诉管理统计数量
		$complain=app::get('complain');
		$mdl_complain=$complain->model('complain');
		$filter=array();
		$filter['store_id']=$this->store_id;
		$filter['status|in'] =array('intervene','success');
		$no_main = $mdl_complain->count($filter);
        if($arr_bases){
            foreach($arr_bases as &$v){
                foreach($v['items'] as &$v1){
                    if($v1['link']=='store_msg'){
					 ...
					 if($v1['link']=='main'){
                        $v1['label'] = app::get('b2c')->_('投诉管理').'('.$no_main.')';
                    }
                }
            }
        }
    ...
    public function js_function_do_agree(){
        //进入页面是需要调用订单操作脚本
        kernel::single('b2c_orderautojob')->do_agree(array(array('return_id'=>$_POST['return_id'],'order_id'=>$_POST['order_id'])));
        
    }

    public function js_function_do_agrees(){
        //进入页面是需要调用订单操作脚本
        kernel::single('b2c_orderautojob')->do_agrees(array(array('return_id'=>$_POST['return_id'],'order_id'=>$_POST['order_id'])));
       
    }

    public function js_function_do_refund_pass(){
        //进入页面是需要调用订单操作脚本
        kernel::single('b2c_orderautojob')->do_refund_pass(array(array('return_id'=>$_POST['return_id'],'order_id'=>$_POST['order_id'])));
     
    }

    public function js_function_do_refund_agrees(){
        //进入页面是需要调用订单操作脚本
        kernel::single('b2c_orderautojob')->do_refund_agrees(array(array('return_id'=>$_POST['return_id'],'order_id'=>$_POST['order_id'])));
    ...
    */
    ...
    {
        $this->path[] = array('title' => app::get('b2c')->_('会员中心'), 'link' => $this->
                gen_url(array(
                'app' => 'b2c',
                'ctl' => 'site_member',
                'act' => 'index',
                'full' => 1)));
        $this->path[] = array('title' => app::get('b2c')->_('结算报表'), 'link' => '#');
        $GLOBALS['runtime']['path'] = $this->path;

        $sto= kernel::single("business_memberstore",$this->app_b2c->member_id);
        $sto->process($this->app_b2c->member_id);
        $store_info = $sto->storeinfo;

        $this->pagedata['store_type'] = $store_info['store_type'];
        $pageLimit = 10;

        $store_id = $this->store_id;
        $obj = app::get('business')->model('settlement');
        $data = $obj->getList('*', array('store_id' => $store_id, 'disabled' => 'false'),
            ($page - 1) * $pageLimit, $pageLimit);

        $total = ceil($data['size'] / $pageLimit);
        $this->pagination($page, $total, 'settlement_list', $arr_args, $app_id = 'business');
        $this->pagedata['data'] = $data;
        $this->pagedata['count'] = count($data);
        $this->pagedata['_PAGE_'] = 'settlement.html';
        $this->output();
    }

    //结算单搜索页面
    public function settlement_item($bill_id = "", $page = 1)
    {
        $this->path[] = array('title' => app::get('business')->_('卖家中心'), 'link' => $this->
                gen_url(array(
                'app' => 'business',
                'ctl' => 'site_member',
                'act' => 'index',
                'full' => 1)));
        $this->path[] = array('title' => app::get('b2c')->_('结算单明细'), 'link' => '#');
        $GLOBALS['runtime']['path'] = $this->path;
        $obj = app::get('business')->model('settlement_item');
        ...
        if (empty($bill_id))
        {
            $this->splash('failed', $this->gen_url(array(
                'app' => 'business',
                'ctl' => 'site_member',
                'act' => 'settlement_list',
                'full' => 1)), app::get('business')->_('结算单号不存在'));
        }
        //此处需要判断是否为本店铺的结算单
        $settlement_info = $obj_settlement->dump(array('settlement_no'=>$bill_id),'*');

        if($this->store_id != $settlement_info['store_id']){
            $this->splash('failed', $this->gen_url(array(
                'app' => 'business',
                'ctl' => 'site_member',
                'act' => 'settlement_list',
                'full' => 1)), app::get('business')->_('此结算报表不属于您的店铺！'));
        }

        //$temp = $obj->getList('*', array('settlement_no' => $bill_id), ($page - 1) * $pageLimit,$pageLimit);
        $temp = $obj->getList('*', array('settlement_no' => $bill_id,'item_type'=>'order'));
        $account_info = $obj->getList('sum(account) as account_all', array('settlement_no' => $bill_id,'item_type'=>'order'));
        $pre_refund = $obj->getList('*', array('settlement_no' => $bill_id,'item_type'=>'refund'));
        $size = $obj->count(array('settlement_no' => $bill_id));

        //获取负结算单
        $pre_bill = explode(',',$settlement_info['pre_settlement']);
        $pre_settlement_info = $obj_settlement->getList('*',array('settlement_no|in'=>$pre_bill));

        $arr_args = array($bill_id);
        $this->pagedata['count'] = $size;
        $this->pagedata['bill_id'] = $bill_id;
        //$total = ceil($size / $pageLimit);
        //$this->pagination($page, $total, 'settlement_item', $arr_args, $app_id ='business');
        $this->pagedata['data'] = $temp;
        $this->pagedata['settlement_info'] = $settlement_info;
        $this->pagedata['pre_settlement_info'] = $pre_settlement_info;
        $this->pagedata['pre_refund'] = $pre_refund;
        $this->pagedata['account_info'] = $account_info;
        $this->pagedata['_PAGE_'] = 'sbill.html';
        $this->output();
    }
    ...
    public function toprint($orderid)
    {
        ...
        {
            $this->begin('index.php?app=business&ctl=seller_order&act=index');
        ...
        $dbTmpl = app::get('b2c')->model('member_systmpl');
        foreach ($aInput as $orderid)
        {
            ...
            $aData['shopaddress'] = app::get('b2c')->getConf('store.address');
            $aData['shoptelphone'] =  app::get('b2c')->getConf('store.telephone');
            $aData['shopzip'] =  app::get('b2c')->getConf('store.zip_code');
			//shop信息修改为店铺的信息
			$shop_data=kernel::single('business_mdl_storemanger')->getList('store_id,store_name,zip,tel,area,addr',array('store_id'=>$aData['store_id']));
			if($shop_data[0]['store_id']==$aData['store_id']){
				$aData['shopname'] 		=$shop_data[0]['store_name'];
				$area=explode('/',$shop_data[0]['area']);
				$shop_p=explode(':',$area[0]);
				$province=$shop_p[1];
				
				$city=$area[1];
				$shop_d=explode(':',$area[2]);
				$district=$shop_d[0];
				
				$address=$shop_data[0]['addr'];
				$addrinfo=array();
				/*if(!stripos($province,$address)){
					$addrinfo[]=$province;
				}
				if(!stripos($city,$address)){
					$addrinfo[]=$city;
				}
				if(!stripos($district,$address)){
					$addrinfo[]=$address;
				}else{
					$addrinfo[]=$district;
					$addrinfo[]=$address;	
				}*/
				$aData['shopaddress'] 	=$shop_data[0]['addr'];
				$aData['shoptelphone'] 	=$shop_data[0]['tel'];
				$aData['shopzip'] 		=$shop_data[0]['zip'];
			}
        ...
        $this->display('site/order/print_order.html','business',false);
    }
    ...
    public function printing($type,$order_id)
    {
        ...
        $orderInfo['self'] = kernel::single('ectools_math')->number_minus(array(0, $orderInfo['discount'], $orderInfo['pmt_goods'], $orderInfo['pmt_order']));
		...
		//echo "<pre>";print_r($this->pagedata['res_url']);exit;
        $this->pagedata['memberPoint'] = $memberInfo[0]['point'] ? $memberInfo[0]['point'] : 0;
        $this->pagedata['storeplace_display_switch'] = app::get('b2c')->getConf('storeplace.display.switch');
        $this->pagedata['defaultImage'] = $imageDefault['S']['default_image'];
        ...
        $this->pagedata['shop'] = array(
            ...
            'email'=>app::get('b2c')->getConf('store.email'),
            'tel'=>app::get('b2c')->getConf('store.telephone'),
            'logo'=>app::get('b2c')->getConf('site.logo')
        );
        $this->_systmpl = app::get('b2c')->model('member_systmpl');
        switch($type)
        {
            case $order->arr_print_type['ORDER_PRINT_CART']:  /*购物清单*/
                ...
                $this->display('site/order/print.html','business',false);
                break;
            ...
            case $order->arr_print_type['ORDER_PRINT_DLY']:    /*快递单打印*/
				...
				$dly_addr = app::get('business')->model('dlyaddress');
				$dly_centers = $dly_addr->getList('da_id,address',array('disable'=>'false','store_id'=>$orderInfo['store_id']));
				foreach($dly_centers as $key=>$val){
					$dly_centers[$key]['dly_center_id'] = $val['da_id'];
					$dly_centers[$key]['name'] = $val['address'];
				}
                $this->pagedata['dly_centers'] = $dly_centers;

                $this->pagedata['default_dc'] = app::get('b2c')->getConf('system.default_dc');
                $this->pagedata['the_dly_center'] = $dly_addr->dump($this->pagedata['dly_centers'][0]['dly_center_id']);
                $this->pagedata['printContent']['express'] = true;
                $printer = &app::get('express')->model('print_tmpl');
                $this->pagedata['printers'] = $printer->getList('prt_tmpl_id,prt_tmpl_title',array('shortcut'=>'true','store_id'=>$this->store_id));
                $this->pagedata['type'] = 'ORDER_PRINT_DLY';
                $this->pagedata['order_status'] = $orderInfo['status']; 

                $this->singlepage('site/order/printer.html','business',false);
                break;
            default:
	...
	public function delivery_printer(){
		$printer_tpl = app::get('express')->model('print_tmpl');
		$this->pagedata['printers'] = $printer_tpl->getList('*',array('shortcut'=>'true','store_id'=>$this->store_id));
		$this->pagedata['_PAGE_'] = 'printer.html';
		$this->output('business');
	}
   ...
   public function gofinish()
    {
        ...
        {
			//卖家确认收货成功，更新收货状态为卖家确认收货
			$objOrders = app::get('b2c')->model('orders');
			$arr_data['receiving_state'] = '3';
