...
//      if($issue_type == 0 || $issue_type == 2){
          //判断品牌名称存在
		  if($_POST['brand_name']==''){
			  $this->splash('failed',$url,app::get('business')->_('品牌名称不能为空！'));
		  }
...
//        }
     ...
     function namecheck(){
        ...
        if($this->name_exsit($brand_name))
        {
		  ...
		  if($this->name_auditing($brand_name))
        {
            echo '<span class="font-red">&nbsp;'.app::get('business')->_('该品牌名称正在审核！').'</span>';
            exit;
        }
	...
	function name_auditing($brand_name){
       
       $obrand = app::get('b2c')->model('brand');
	    $brand_use = app::get('business')->model('brand');
       
        $b2c_brand = $obrand->getList('brand_name');
        $business_brand = $brand_use->getList('brand_name',array('status'=>0));
