...
class business_ctl_site_cart extends b2c_ctl_site_cart{
    ...
    public function create()
    {
        ...
        if($obj_filter->check_input($_POST['fastbuy'])){
            ...
            if(!empty($_SESSION['sfsc']['vcat'])){
                $vcat_id = $_SESSION['sfsc']['vcat']['virtual_cat_id'];
            }else{
                $vcat_id = -1;
            }
            $allCart['object']['goods'][0]['vcat_id'] = $vcat_id;
        }else{
        ...
        $defaule_area_id = explode(':',$obj_filter->check_input($_POST['delivery']['ship_area']));
        ...
        if($this->point_usage==2&& $this->get_policy_method!=1){
            $this->dis_point=$obj_filter->check_input($_POST['payment']['dis_point'])?$obj_filter->check_input($_POST['payment']['dis_point']):0;
        ...
        foreach($temp_split_order as $store_id=>$sgoods){
            foreach($sgoods['slips'] as $order_sp=>$sorder){
                ...
                //对于每个单子都要开启事务，否则去结算时出现拆单的情况，只有第一个单才开启事务
                $db = kernel::database();
                ...
                $postData=$obj_filter->check_input($_POST);
                //为分单信息重新赋值，避免字符串被过滤-hy
                $postData['split_order']=$_POST['split_order'];
                $postData=$this->get_post_cart($aCart,$sorder,$postData,$obj_filter->check_input($_POST['shipping'][$store_id][$order_sp]));
    ...
    */          $obj_order_create = kernel::single("b2c_order_create");
                ...
                if($_POST['payment']['QS_payment_id'] != ""){
                    $order_data['java_payment_company'] = $_POST['payment']['QS_payment_id'];
                }
                $result = $obj_order_create->save($order_data, $msg);
                //进入京东下单流程
                if($result){
                    //$order_data['order_id'] 本地订单
                    //$order_data['store_id'] 订单地址
                    //获取京东自营店铺信息
                    ...
                    if($jdshopId || $jdbookshopId){
                        if($jdshopId == $order_data['store_id']){
                            if(!$this->get_jdorders($order_data,$msg,$arrMember)){
                                $result = false;
                            }
                        }elseif ($jdbookshopId == $order_data['store_id']) {
                            if(!$this->get_jdorders($order_data,$msg,$arrMember,"book")){
                                $result = false;
                            }
                        }
                    }
                ...
                if ($result)
                {
                    ...
                    if($_POST['fastbuy']){
                        ...
                        foreach($sorder['object']['goods']['obj_ident'] as $obj_ident_key=>$obj_ident_val){
                            $this->cart_model->remove_object('goods',$obj_ident_val);
                        }
                    }
                    ...
                    // 生成cookie有效性的验证信息
                    if(IS_DOMAIN){
                        setcookie('S[ST_ShopEx-Anonymity-Buy]',"false", time() - 3600, kernel::base_url() . '/',COOKIE_DOMAIN,1);
                        setcookie('purchase[addr][usable]',"", time() - 3600, kernel::base_url() . '/',COOKIE_DOMAIN,1);
                        setcookie('purchase[shipping]',"", time() - 3600, kernel::base_url() . '/',COOKIE_DOMAIN,1);
                        setcookie('purchase[payment]',"", time() - 3600, kernel::base_url() . '/',COOKIE_DOMAIN,1);
                        setcookie('checkout_b2c_goods_buy_info',"", time() - 3600, kernel::base_url() . '/',COOKIE_DOMAIN,1);
                    }else{
                ...
                if ($result)
                {
                    ...
                    //如果是京东的订单失败后放在rollback之后执行
                    if(($jdshopId || $jdbookshopId) && ($msg == '京东商品价格有变化，请稍后下单')){
        ...
        if($flag){
            ...
            if(count($aOrders)>1){
                if($arrMember['seller'] == 'qiye'){
                    $this->end(true, $this->app->_("订单生成成功！"), $this->gen_url(array('app'=>'qiyecenter','ctl'=>'site_member','act'=>'all_orderPayments','args'=>array(false,'',$orderStr))),'',true);
                }else{
                ...
                if($arrMember['seller'] == 'qiye'){
                    $this->end(true, $this->app->_("订单生成成功！"), $this->gen_url(array('app'=>'qiyecenter','ctl'=>'site_member','act'=>'orderPayments','args'=>array($aOrders[0],false,true))),'',true);
                }else{
    ...
    function get_order_data($aCart,$postData,$store_id,&$msg,$arrMember){
        ...
        $split_order=$postData['split_order'];
        $postData = $obj_filter->check_input($postData);
        //为分单信息重新赋值，避免字符串被过滤-hy
        $postData['split_order']=$split_order;
        ...
        if($jdshopId){
            if($jdshopId == $order_data['store_id']){
                $order_data['order_kind'] = "jdorder";
            }
        }

        //lpc 修改orders表中的order_kind类型
        $bookshopId = app::get('site')->getConf('jdbook.shopId') ? app::get('site')->getConf('jdbook.shopId'): '';
        if($bookshopId){
            if($bookshopId == $order_data['store_id']){
                $order_data['order_kind'] = "jdbook";
            }
        }
    ...
    //京东预占库存订单
    private function get_jdorders($order_data,&$msg,$arrMember,$jdgoodsKind='normal'){
        $area_id = explode(':',$order_data['consignee']['area']);
        ...
        $jd_sku =array();
        foreach($order_data['order_objects'] as $k=>$v){
            $jd_sku[] = array(
                'skuId'=>intval($v['bn']),
                'num'=>intval($v['quantity']),
                'bNeedAnnex' => true,
                'bNeedGift' => true
            );

            //客户端订单价格快照 用来验证本地价格是否与京东实时价格是否一致 by zyp@2019-01-10
            $agreed_price = app::get("b2c")->model("goods")->getRow('goods_id,bn,agreed_price',array('bn|tequal'=>$v['bn']));
            $orderPriceSnap[]=array(
                'price' => floatval($agreed_price['agreed_price']),
                'skuId' => intval($v['bn'])
            );
        }
        ...
        $param = array(
            ...
            'address'=>$order_data['consignee']['addr'],
            'zip'=>$order_data['consignee']['zip'],
            ...
            ///客户端订单价格快照 用来验证本地价格是否与京东实时价格是否一致 by zyp@2019-01-10
            'doOrderPriceMode'=>'1',
            'orderPriceSnap'=>json_encode($orderPriceSnap),
        );
        ...
        if($jd_data['success'] == false){
            ...
            foreach ($jd_sku as $jk => $jv) {
                $goodsData = $oGoods->getRow('name',array('bn|tequal'=>$jv['skuId']));
            ...
            //3019代表快照价格与京东那边不等
            if ($jd_data['resultCode'] == '3019'){
                //将协议价不等的sku存入kv
                $this->storeJdSkuKv($orderPriceSnap,$jdgoodsKind);
                error_log("=-=-=-=-=-=-=-=-".PHP_EOL.var_export(array('skus'=>$orderPriceSnap,'msg'=> $msg,'order_objects'=>$order_data['order_objects']),1).PHP_EOL,3,DATA_DIR."/jd_price_issue.log");
                $msg = '京东商品价格有变化，请稍后下单';
            }
...
// 将需要更新协议价的sku存入kv
    function storeJdSkuKv($skus,$jdgoodsKind='normal'){
        if (empty($skus)){
            return;
        }
        ...
        $toStore = array();
        //首先获取一下kv中还保存着的数据
        base_kvstore::instance('jdsaleNeedUpdataAgreedPrice')->fetch($kvName, $oriSku);
        if (!empty($oriSku)){
            // $toStore = json_decode($oriSku,true);
            $toStore = $oriSku;
        }
        
        //kv中的和需要新增的数据合并
        foreach ($skus as $sk => $sv) {
            $toStore[$sv['skuId']] = $sv['price'];
        }

        // $skusMap = json_encode($toStore);
        base_kvstore::instance('jdsaleNeedUpdataAgreedPrice')->store($kvName, $toStore);

        unset($skusMap, $toStore, $oriSku, $kvName);
    }
