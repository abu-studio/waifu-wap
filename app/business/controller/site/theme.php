...
class business_ctl_site_theme extends business_ctl_site_member{
    ...
    public function previewtheme($theme,$file)
    {
        ...
        if ($_SESSION['WIDGET_TMP_DATA'][$theme.'/block/top.html']) $_SESSION['WIDGET_TMP_DATA'][$theme.'/block/top.html'] = array();
        if ($_SESSION['WIDGET_TMP_DATA'][$theme.'/block/shop_nav.html']) $_SESSION['WIDGET_TMP_DATA'][$theme.'/block/shop_nav.html'] = array();
        ...
        if ($this->pagedata['widgetsLib']['usual']){
            foreach((array)$this->pagedata['widgetsLib']['usual'] as $key=>$widgets){
                if ($widgets['theme']){
                    ...
                    $imgFliePath = $theme_url.'widgets/'.$widgets['name'].'/images/icon.jpg';
                    if(file_exists(realpath(ROOT_DIR.'/../').$imgFliePath)){
                        $this->pagedata['widgetsLib']['usual'][$key]['img'] = $imgFliePath;
                        $this->pagedata['widgetsLib']['usual'][$key]['bimg'] = $imgFliePath;
                    }else{
                        $this->pagedata['widgetsLib']['usual'][$key]['img'] = app::get('site')->res_url.'/images/widgets/icon.jpg';
                        $this->pagedata['widgetsLib']['usual'][$key]['bimg'] = app::get('site')->res_url.'/images/widgets/widget.jpg';
                    }
                }else{//获取系统级挂件信息
                    $this->pagedata['widgetsLib']['usual'][$key]['img'] = app::get('site')->res_url.'/images/widgets/icon.jpg';
                    $this->pagedata['widgetsLib']['usual'][$key]['bimg'] = app::get('site')->res_url.'/images/widgets/widget.jpg';
        ...
        if ($this->pagedata['widgetsLib']['list'])
            foreach((array)$this->pagedata['widgetsLib']['list'] as $key=>$widgets){
                ...
                if ($widgets['theme']){
                    ...
                    $imgFliePath = $theme_url.'widgets/'.$widgets['name'].'/images/icon.jpg';
                    if(file_exists(realpath(ROOT_DIR.'/../').$imgFliePath)){
                        $this->pagedata['widgetsLib']['list'][$key]['img'] = $imgFliePath;
                        $this->pagedata['widgetsLib']['list'][$key]['bimg'] = $imgFliePath;
                    }else{
                        $this->pagedata['widgetsLib']['list'][$key]['img'] = app::get('site')->res_url.'/images/widgets/icon.jpg';
                        $this->pagedata['widgetsLib']['list'][$key]['bimg'] = app::get('site')->res_url.'/images/widgets/widget.jpg';
                    }
                }else{//获取系统级挂件信息
                    $this->pagedata['widgetsLib']['list'][$key]['img'] = app::get('site')->res_url.'/images/widgets/icon.jpg';
                    $this->pagedata['widgetsLib']['list'][$key]['bimg'] = app::get('site')->res_url.'/images/widgets/widget.jpg';
    ...
    public function dopreview()
    {
        ...
        if(false !== ($map = kernel::single('site_theme_widget')->save_preview_all($widgetsSet,$files))){
			if(IS_DOMAIN){
				setcookie('site[preview]', 'true', 0, kernel::base_url() . '/',COOKIE_DOMAIN,1);
			}else{
