...
class business_ctl_site_tools extends site_controller{
    ...
    function image_upload(){
	   ...
	   $msg = '图片上传失败';  
       $image_id  = $mdl_img->store($_FILES['upload_item']['tmp_name'],null,null,$image_name,false,$store_id,1,$msg);
       if(!$image_id) {
            header('Content-Type:text/html; charset=utf-8');
            echo "{error:'".app::get('image')->_($msg)."',splash:'true'}";
            exit;
       }
       //非商品图片不生成小中大图。
...
//			
			if(window.curEditor.inc.doc.getSelection){
				window.curRange = window.curEditor.getRange();
				window.bookmark = window.curEditor.inc.doc.getSelection().focusOffset;
			}else if(window.curRange.getBookmark){
