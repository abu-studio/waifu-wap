...
function check_acountid($account_id, &$message) {
   ...
   function save(&$data,$mustUpdate = null){
	...
	//获取可以结算的店铺
    public function store_getList(){
        $sql = "select s.store_id from sdb_business_storemanger s WHERE (s.last_settlement_time+(s.settlement_cycle*86400)) < ".time();
        $stores = $this->db->select($sql);
