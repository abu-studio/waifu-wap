...
class ectools_regions_operation
{
    ...
    public function __construct($app)
    {
        ...
        $this->model = app::get('jdsale')->model("regions");
    }
    ...
    {
        if ($prId)
            $sql="select area_id as region_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1)." where p_region_id=r.region_id) as child_count from ".$this->model->table_name(1)." as r where r.p_region_id=".intval($prId)." order by ordernum asc,region_id";
        else
            $sql="select area_id as region_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1)." where p_region_id=r.region_id) as child_count from ".$this->model->table_name(1)." as r where r.p_region_id is null order by ordernum asc,region_id";
