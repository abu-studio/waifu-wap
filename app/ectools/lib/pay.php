...
{    
    ...
    public function generate(&$sdf, &$controller=null, &$msg='')
    {
        ...
        if ($sdf['pay_type']=='online')
        {
			...
			if ($pay_app_id != 'deposit' && $pay_app_id != 'sfscpay')
			{
     ...
     * @return 支付单
     */
    public function get_payment_by_order($order_id,$pay_app_id,$status)
    {
		$sql = "SELECT p.* FROM sdb_ectools_payments p LEFT JOIN sdb_ectools_order_bills ob ON p.payment_id=ob.bill_id WHERE ob.rel_id='".$order_id."' AND p.pay_app_id='".$pay_app_id."' AND p.status='".$status."'";
		$data = kernel::database()->select($sql);
        return $data[0];
    }
    
	public function get_combine_payment($order_id){
		$sql = "SELECT p.* FROM sdb_ectools_payments p LEFT JOIN sdb_ectools_order_bills ob ON p.payment_id=ob.bill_id WHERE ob.rel_id='".$order_id."' AND p.status='succ'";
		$data = kernel::database()->select($sql);
		$result = array();
		foreach($data as $k=>$v){
    ...
    public function gopay(&$sdf, &$msg='')
    {
        ...
        $obj_payment_create = kernel::single('ectools_payment_create');
        $is_save = $obj_payment_create->generate($sdf, $msg);
        $log_id = SFSC_HttpClient::log_insert($sdf);
        if (!$is_save)
        {
            return false;
        }
        ...
        SFSC_HttpClient::log_update($sdf,$log_id);
        if (!$is_save)
        {
            return false;
        }
