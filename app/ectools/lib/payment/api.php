...
class ectools_payment_api
{
	...
	public function parse($params='')
	{
        ...
        if($pay_data){
            ...
            $b2c_mdl_orders = app::get('b2c')->model('orders');
            $b2c_mdl_payments = app::get('ectools')->model('payments');
            $frst_order_id = current($sdf['orders']);
            $order_id = current($frst_order_id);
            // $paymentsData = $b2c_mdl_payments->get_payments_by_order_id($order_id);
            $paymentsData = $b2c_mdl_payments->db->select("SELECT payment.pay_app_id,payment.status,payment.payment_id FROM `sdb_ectools_payments` as payment LEFT JOIN sdb_ectools_order_bills as bill on payment.payment_id = bill.bill_id WHERE bill.rel_id='{$order_id}'");

            $ordersData = $b2c_mdl_orders->dump(array('order_id'=>$order_id),"pay_status,final_amount");

            if ($ordersData['pay_status'] == 3) {
                foreach ($paymentsData as $key => $value) {
                    if ($value['pay_app_id'] == "alipay" && $value['status'] == "succ") {
                        $arrOrder = array(
                            'pay_status' => '1',
                            'payed'      => $ordersData['cur_amount'],
                            'payment'    => "sfscpay,alipay",
                        );
                        $b2c_mdl_orders->update($arrOrder,array('order_id'=>$order_id));
                        $b2c_mdl_payments->update(array('status'=>"succ"),array('pay_app_id'=>"sfscpay",'status'=>'sfsc_freeze'));

                        file_put_contents(DATA_DIR . '/alipay.log', date('Y-m-d H:i:s',time()).":支付单数据：".json_encode($paymentsData)."\n\r订单数据".json_encode($ordersData)."---订单号：{$order_id}\n\r", FILE_APPEND);
                        file_put_contents(DATA_DIR . '/alipay.log', date('Y-m-d H:i:s',time()).":订单有异常，修复订单状态，支付单状态！\n\r\n\r\n\r\n\r", FILE_APPEND);
                    }
                }
            }
