...
class ectools_ctl_payment_cfgs extends desktop_controller{
    ...
    function combine_pay(){
		if(!$_POST){
			$this->pagedata['combine_pay']['is_open'] = $this->app->getConf('site.is_open_combine_pay');
			$this->page('payments/cfgs/combine_pay.html');
		}else{
            $this->begin('index.php?app=ectools&ctl=payment_cfgs&act=combine_pay');
            
            if ($_POST['combine_pay_open'] == 'true')
                $this->app->setConf('site.is_open_combine_pay', 1);
            else
                $this->app->setConf('site.is_open_combine_pay', 0);
