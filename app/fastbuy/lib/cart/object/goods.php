...
class fastbuy_cart_object_goods extends b2c_cart_object_goods
{
    ...
    public function add_object($aSave, &$msg = '', $append = true)
    {
        ...
        $aSave = array(
            ...
            'virtual_card' => $aData['virtual_card']
        );
    ...
    // 购物车里的所有商品项
    public function getAll($rich = false, $ebapp_flag = false)
    {
        if (true) {
        ...
        return $this->_get($aResult, $ebapp_flag);
    }
    ...
    private function _generateParams($aData)
    {
        ...
        //兑换卡券 2015/7/10
        if ($aData['virtual_card'] > 0 || $aData['goods_card'] > 0) {
            $return["card"]["entity"] = $aData['goods_card'] > 0 ? floatval($aData['goods_card']) : 0;
            $return["card"]["virtual"] = $aData['virtual_card'] > 0 ? floatval($aData['virtual_card']) : 0;
        }
        return $return;
    }
     ...
     * @return array
     */
    private function _get($aData, $ebapp_flag = false)
    {
    ...
    function _get_products($aProductId)
    {
        ...
        $sSql = "SELECT
                     ...
                     g.goods_type, g.nostore_sell, g.min_buy,g.type_id,g.cat_id,g.image_default_id,p.spec_info,p.spec_desc,p.price,p.weight,p.mktprice as p_mktprice,g.agreed_price,g.mktprice as g_mktprice,
        ...
        foreach ($aProduct as $row) {
            ...
            if ($goodsDateK['goods_kind'] == 'jdgoods' || $goodsDateK['goods_kind'] == 'jdbook') {
