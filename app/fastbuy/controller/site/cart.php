...
class fastbuy_ctl_site_cart extends b2c_ctl_site_cart
{
    ...
    public function addCheck($type = 'goods')
    {
        ...
        if (!$arr_objects[$type]) {
            ...
            echo json_encode(array('status' => 'faile', 'error' => $msg));
            exit;
        }

        if (method_exists($arr_objects[$type], 'get_data'))
            if (!$aData = $arr_objects[$type]->get_data($data, $msg)) {
                echo json_encode(array('status' => 'faile', 'error' => '参数错误，购买失败'));
                exit;
            }
        ...
        echo json_encode(array('status' => 'succ', 'error' => 'chenggong'));
        exit;
    ...
    public function checkout_result($isfastbuy = 0)
    {
        ...
        $areaCount = count(explode('/', $addr_default_addr['area']));
        if ($areaCount < 3) {
