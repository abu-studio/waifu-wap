    ...
    function viewStat($goods_id){
        ...
        if($pdtView>0){
            foreach($ObjGoods->getlist('view_count,view_w_count,view_m_count,count_stat,goods_id',array('goods_id'=>array_keys($pdtView))) as $row){
                ...
                $ObjGoods->db->exec("update sdb_b2c_goods set view_w_count=".intval($row['view_w_count']).",count_stat="
                        .$stat.",view_count=".intval($row['view_count']).",view_m_count=".intval($row['view_m_count'])." where goods_id=".intval($row['goods_id']),true);
