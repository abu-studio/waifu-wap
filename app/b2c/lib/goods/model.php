...
class b2c_goods_model{
	...
	function getstore($gid){
	    $obj = app::get('cardcoupons')->model('cards_pass');
	    $sql = "select card_id,type,count(card_pass_id) from sdb_cardcoupons_cards_pass where `status` = '0' and `ex_status` = 'true' and card_id =(select card_id from sdb_cardcoupons_cards where goods_id = '$gid') and disabled='false' GROUP BY card_id,type";   
	    $store = $obj->db->select($sql);
	    return $store;
