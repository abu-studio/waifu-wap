...
WHERE p.goods_id =".intval($aGoods['goods_id']));
	...
	function get_jdgoods($listgoods=null,$member_lv_id=-1)
    {

		$member_lv = app::get('b2c')->model("member_lv");
        $memLv = $member_lv->getList("member_lv_id,name,jd_discount");
        foreach ($memLv as $memLv_k => $memLv_v) {
            $memLvaData[$memLv_v['member_lv_id']] = $memLv_v;
        }
		$jd_discount = $memLvaData[$member_lv_id]['jd_discount']?$memLvaData[$member_lv_id]['jd_discount']:1;
        ...
        $cat['cat_id'] = $listgoods['jd_cat_id'];
        $aGoods['category'] = $cat;
        ...
        $aGoods['adjunct'] = $listgoods['adjunct'];//meta数据
        $aGoods['seo_info'] = $listgoods['seo_info'];
        $aGoods['price'] = $listgoods['price']*$jd_discount;
        $aGoods['cost'] = $listgoods['cost'];
		$aGoods['mktprice'] = $listgoods['mktprice']*$jd_discount;
        
        $aGoods['weight'] = $listgoods['weight'];
       
        $image = app::get("jdsale")->model("image");

        $image_data = $image->getList("goods_id,image_path",array("goods_id"=>intval($aGoods['goods_id'])),0,-1,"order_sort asc");
        foreach($image_data as $img_k=>$img_v)
        {
            $aGoods['images']['max'][] = jdsale_goods_import::$image_url_max.$img_v['image_path'];
			$aGoods['images']['mid'][] = jdsale_goods_import::$image_url_mid.$img_v['image_path'];
			$aGoods['images']['min'][] = jdsale_goods_import::$image_url_min.$img_v['image_path'];
        }
		$aGoods['param'] = $listgoods['param'];
        $aGoods['wareQD'] = $listgoods['wareQD'];
        $aGoods['description'] = $listgoods['intro'];
