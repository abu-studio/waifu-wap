...
/*                if(in_array('0', $filter['cat_id'])){
		...
		if($filter['jd_cat_id'] || $filter['jd_cat_id'] === 0){
            if(!is_array($filter['jd_cat_id'])){
                $filter['jd_cat_id']=array($filter['jd_cat_id']);
            }else{
            	foreach($filter['jd_cat_id'] as $vCat_id){
	                if($vCat_id !== '_ANY_' && $vCat_id !== ''){
	                    $aCat_id[] = intval($vCat_id);
	                }
                }
                $filter['jd_cat_id']=$aCat_id;
            }
            ...
            if($object->__show_goods){
                if(count($filter['jd_cat_id'])>0)
                    $where[] = 'jd_cat_id in ('.implode($filter['jd_cat_id'],' , ').')';
            }else{
                if($filter['jd_cat_id']){
                    $oCat = app::get('jdsale')->model('goods_cat');
                    $fcat_id = $filter['jd_cat_id'];
                    $aCat = $oCat->getList('cat_path,jd_cat_id',array('jd_cat_id'=>$fcat_id));
                }
                ...
                if(count($aCat)){
                    foreach($aCat as $v){
                        $pathplus.=' cat_path LIKE \''
                                .($v['cat_path']).$v['jd_cat_id'].',%\' OR';
                    }
                }
                if($aCat){
                    foreach($object->db->select('SELECT jd_cat_id FROM sdb_jdsale_goods_cat WHERE '.$pathplus.' jd_cat_id in ('.implode($filter['jd_cat_id'],' , ').')') as $rows){
                        $aCatid[] = $rows['jd_cat_id'];
                    }
                }else{
                    unset($aCatid);
                }
/*                if(in_array('0', $filter['cat_id'])){
                ...
                if(!is_null($aCatid)){
                    $where[] = 'jd_cat_id IN ('.implode(',', $aCatid).')';
                }else if($filter['jd_cat_id'] && $filter['jd_cat_id'][0]){                    
                    $where[] = 'jd_cat_id IN ('.implode(',', $filter['jd_cat_id']).')';
                }

            }
            $filter['jd_cat_id'] = null;
        } 
        ...
        if(isset($filter['jd_brand_id']) && $filter['jd_brand_id']){
            if(is_array($filter['jd_brand_id'])){
                foreach($filter['jd_brand_id'] as $brand_id){
                    if($brand_id!='_ANY_'){
                        $aBrand[] = intval($brand_id);
                    }
                }
                if(count($aBrand)>0){
                    $where[] = 'jd_brand_id IN('.implode(',', $aBrand).')';
                }
            }elseif($filter['jd_brand_id'] > 0){
                $where[] = 'jd_brand_id = '.$filter['jd_brand_id'];
            }
            unset($filter['jd_brand_id']);
        }
		...
		//增加销售税率搜索
		if(isset($filter['gross_profit'])){
			if($filter['_gross_profit_search'] == 'between'){
				$gpv = array($filter['gross_profit_from'],$filter['gross_profit_to']);
			}else{
			    $gpv = $filter['gross_profit']; 
			}
			$gptPre = 'ROUND((price-agreed_price)/price,3)*100';
			$gptype = $filter['_gross_profit_search'];
			$p_str = $this->_inner_getFilterType($gptype,$gpv,false);
			if(strpos($p_str,'{field}')!==false ){
                $where[] = str_replace('{field}',$gptPre,$p_str);
			}else{
				$where[] = $gptPre.$p_str;
			}
