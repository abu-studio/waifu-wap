...
class b2c_goods_goodsfilter extends dbeav_filter{
	...
	//京东商品搜索条件
	function goods_jdgoodsfilter(){
        $modTag = app::get('desktop')->model('tag');
        $brand = app::get('jdsale')->model('brand');
        $object = app::get('jdsale')->model('goods_cat');

        $return['cats'] = $object->getMapTree(0,'└','');
        $return['brands'] = $brand->getList('*',null,0,-1);
        $row = $object->db->selectrow('SELECT max(price) as max,min(price) as min FROM sdb_b2c_goods where goods_kind ="jdgoods"');
        $return['tags'] = $modTag->getList('*',array('tag_type'=>'goods'),0,-1);
        $return['prices'] = utils::steprange($row['min'],$row['max'],5);
        return $return;
    }
    ...
    //京东图书搜索条件
	function goods_jdbookfilter(){
        $modTag = app::get('desktop')->model('tag');
        $brand = app::get('jdsale')->model('brand');
        $object = app::get('jdsale')->model('goods_cat');

        $return['cats'] = $object->getMapTree(0,'└','','jdbook');
        $return['brands'] = $brand->getList('*',null,0,-1);
        $row = $object->db->selectrow('SELECT max(price) as max,min(price) as min FROM sdb_b2c_goods where goods_kind ="jdgoods"');
        $return['tags'] = $modTag->getList('*',array('tag_type'=>'goods'),0,-1);
        $return['prices'] = utils::steprange($row['min'],$row['max'],5);
