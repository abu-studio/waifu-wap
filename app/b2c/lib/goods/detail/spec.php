...
//            $this->pagedata['SelectSpecValue'] = array('value'=>implode("、",(array)$specValue),'selected'=>0);
        ...
        foreach($aGoods['product'] as $product_id=>$product){
			...
			//规格金额格式化-hy-2016年1月25日
			$product2spec[$product_id]['price'] = $mdl_currency->changer_odr($product2spec[$product_id]['price'], $_COOKIE["S"]["CUR"], true, false, $decimals,$carryset);
			$product2spec[$product_id]['mktprice'] = $mdl_currency->changer_odr($product2spec[$product_id]['mktprice'], $_COOKIE["S"]["CUR"], true, false, $decimals,$carryset);
