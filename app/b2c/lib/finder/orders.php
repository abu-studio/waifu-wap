...
class b2c_finder_orders{
    ...
    var $detail_msg = '顾客留言';
	var $detail_store = '店铺信息';
    var $column_editbutton = '操作';
    var $column_cardno = '卡号';
	var $column_company = '商社名称';
    var $column_company_no = '商社号';
    var $column_logi_no = '运单号';
    ...
    public function detail_items($order_id)
    {
		...
		//判断购买商品列表是否显示卡号信息
		if($order_items[0]['product']['card_pass']){
			$render->pagedata['is_card_pass']=true;
		}
                ...
                {                    
                    ...
                    {  
                        ...
                        if ($item['item_type'] != 'gift')
                        {
                            ...
                            {  
							...
							}else if($item['item_type'] == 'jdgoods' || $item['item_type'] =='jdbook'){//京东商品
								if ($arr_service_goods_type_obj['goods'])
                                {
                                    $str_service_goods_type_obj = $arr_service_goods_type_obj['goods'];
                                    $str_service_goods_type_obj->get_order_object(array('goods_id' => $item['goods_id'],'product_id'=>$item['products']['product_id']), $arrGoods, $tml);
                                ...
                                $order_items[$k]['product'] = $item;
                                $order_items[$k]['product']['small_pic'] = jdsale_goods_import::$image_url_n3.$arrGoods['image_default_id'];
                                $order_items[$k]['product']['is_type'] = $v['obj_type'];
                                $order_items[$k]['product']['item_type'] = $arrGoods['category']['cat_name'];
                                $order_items[$k]['product']['nums'] = $item['quantity'];
                                $order_items[$k]['product']['minfo'] = $gItems[$k]['minfo'];
                                $order_items[$k]['product']['total_amount'] = $objMath->number_multiple(array($item['price'], $item['quantity']));
                                $order_items[$k]['product']['link'] = kernel::single('b2c_frontpage')->gen_url(array('app'=>'jdsale','ctl'=>'site_product','act'=>'index','arg'=>$item['goods_id']));	
    ...
    public function detail_logs($order_id){
        ...
        if(!empty($aLog['data'])){
            foreach($aLog['data'] as $k=>$v){
                if($v['behavior'] == "发送短信" || $v['behavior'] == "cms" || $v['behavior'] == "delivery" || $v['behavior'] == "发货"){
                    $aLog['data'][$k]['log_text'] = " **** ";
                }
            }
        }
    ...
    {
		if($_GET['app']!='cardcoupons'){
			return '';
		}
		if($row['order_kind']=='card'){
			return '<a href="index.php?app=cardcoupons&ctl=admin_order&act=change&order_id='.$row['order_id'].'">'.app::get('cardcoupons')->_('卡券商品退换').'</a>';
		}
        $render = $this->app->render();
        $arr = array(
            'app'=>$_GET['app'],
	...
	public function column_cardno($row)
    {
        if ( $row['order_kind'] != 'virtual' && $row['order_kind'] != 'jdorder' && $row['order_kind'] != 'jdbook' ) {
            $card_pass = app::get('cardcoupons')->model('cards_pass');
            $info = $card_pass->getRow('card_no',array('exchange_order_id|has'=>$row['order_id']));
            return $info['card_no'];
        } else {
            return '';
        }
	...
	public function detail_store($order_id){
		$order_info = app::get('b2c')->model('orders')->getRow('store_id',array('order_id'=>$order_id));
		$store_id=$order_info['store_id'];
		$arr_info = app::get('business') -> model('storemanger') -> dump($store_id, '*', 'default');
        $render = $this->app->render();
        $area=explode(':', $arr_info['area']);
        $arr_info['area']=$area[1];
        $arr_info['last_time']=date('Y-m-d',$arr_info['last_time']);
        $area=explode(':', $arr_info['company_area']);
        $arr_info['company_area']=$area[1];
        $render->pagedata['url'] = app::get('site')->router()->gen_url(array('app'=>'business','ctl'=>'site_shop','full'=>1,'act'=>'view','arg'=>$store_id));
        $render->pagedata['shopinfo']=$arr_info;
        return $render->fetch('admin/order/detail/store.html');
	}
	public function column_company($row)
    {
        //订单列表上增加显示人事经理使用企业账户下单的商社名称，以java_payment_company来判定是使用哪个企业账户(或者是部门，群组)下单的，如java_payment_company为空则使用本身雇员积分，而非企业账户积分,走原本逻辑，但也存在企业人事经理登录下单前未选择默认支付账户啊，此时java_payment_company也为空，且走雇员逻辑也不会拿到商社信息
        $javaPayCompany = app::get('b2c')->model('orders')->getRow('java_payment_company',array('order_id'=>$row['order_id']));
        if (empty($javaPayCompany['java_payment_company'])){
            if ( $this->_cacheAccount[$row['member_id']] ) {
                $info = $this->_cacheAccount[$row['member_id']];
            } else {
                $pam = app::get('pam')->model('account');
                $info = $pam->getRow('company_name,company_no',array('account_id'=>$row['member_id']));
                $this->_cacheAccount[$row['member_id']] = $info;
            }
    		return $info['company_name'];
        }else{
            if ( $this->_cacheCompany[$javaPayCompany['java_payment_company']] ){
                $info = $this->_cacheCompany[$javaPayCompany['java_payment_company']];
            }else{
                //获取本地商社名称及商社号
                $pam = app::get('pam')->model('account');
                $info = $pam->getRow('company_no,company_name',array('company_no'=>$javaPayCompany['java_payment_company']));
                if (!empty($info['company_name'])){
                    $this->_cacheCompany[$javaPayCompany['java_payment_company']] = $info;
                    return $info['company_name'];
                }
                //如info为空，则需获取rdp那边对应的名称
                $serviceNo="SubAccountService";
                $_sjson = array(
                    'METHOD' => 'getCompanyDepGroupToPayByCompanyNo',
                    'RELATION_ID' => $javaPayCompany['java_payment_company'],
                );
                $sfscdata = SFSC_HttpClient::doLifCostMain($_sjson,$serviceNo);
                if('10001' == $sfscdata['RESULT_CODE'] && !empty($sfscdata['RESULT_DATA'][0])){
                    $info = array(
                        'company_name'=>$sfscdata['RESULT_DATA'][0]['CUSTOMER_NAME'],
                        'company_no' => $sfscdata['RESULT_DATA'][0]['COMPANY_NO']
                    );
                    $this->_cacheCompany[$javaPayCompany['java_payment_company']] = $info;
                }
            }
    ...
    public function column_logi_no($row)
    {
		$obj_delivery = app::get('b2c')->model('delivery');
		//获取运单号
		$deliveries = $obj_delivery->getList('*',array('order_id'=>$row['order_id']));
		$logi_nos = array();
		foreach($deliveries as $logi_k=>$logi_v){
			$logi_nos[] = $logi_v['logi_no'];
		}
		return implode(',',$logi_nos);
    }
    //后台订单列表同时显示商社号
    public function column_company_no($row)
    {
        $javaPayCompany = app::get('b2c')->model('orders')->getRow('java_payment_company',array('order_id'=>$row['order_id']));
        if (empty($javaPayCompany['java_payment_company'])){
            if ( $this->_cacheAccount[$row['member_id']] ) {
            ...
            return $info['company_no'];
        }else{
            //这里就不能直接返回java_payment_company，这个字段如果是公司账户的话没问题，但如果是部门账户或者群组账户就不对了
            //还是需要从rdp那边获取
            if ( $this->_cacheCompany[$javaPayCompany['java_payment_company']] ){
                $info = $this->_cacheCompany[$javaPayCompany['java_payment_company']];
            }else{
