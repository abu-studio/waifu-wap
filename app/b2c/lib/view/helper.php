...
class b2c_view_helper{
    ...
    function function_hfselector($params, &$smarty){
        $filter = $params['filter'];
        if(is_numeric($params['key'])){
            if($params['del']&&isset($filter['p_'.$params['key']])){
                unset($filter['p_'.$params['key']]);
        ...
        $args[4]=1;
        return app::get('site')->router()->gen_url(array('app'=>'b2c', 'ctl'=>'site_hfgallery','full'=>1,'args'=>$args));
    }

	//京东地址解析
	function function_jdselector($params, &$smarty){
        $filter = $params['filter'];
        if(is_numeric($params['key'])){           
            if($params['del']&&isset($filter['p_'.$params['key']])){
                 unset($filter['p_'.$params['key']]);
        ...
        $args[4]=1;
        return app::get('site')->router()->gen_url(array('app'=>'jdsale', 'ctl'=>'site_gallery','full'=>1,'args'=>$args));
    }
        ...
        if($args){
            if($args['filter']){
                ...
                if($filter['price']){
					...
					$url[]='scontent=p,'.$filter['price'][0].','.$filter['price'][1];
                }
                if($filter['cat_id'][0]){                    
                    $hidden[]='<input type="hidden" name="cat_id" value="'.$filter['cat_id'][0].'">';
                }
                if($filter['name'][0]){                    
                    $hidden[]='<input type="hidden" name="name" value="'.$filter['name'][0].'">';
                }
                if($filter['brand_id']){                    
            ...
            if($args['tab']){                
             ...
             if($args['tmpl']){                
                $hidden[]='<input type="hidden" name="tmpl" value="'.$args['tmpl'].'">';
            }
            if($args['cat_type']){                
...
function _pager_link($from,$to,$l,$p,$c=null){
	...
	function function_jdpagers($params, &$smarty){
        //print_r($params);
	    if(!$params['data']['current'])$params['data']['current'] = 1;
