...
class b2c_view_input{
	...
	//京东商品搜索
	function input_jdgoodsfilter($params){
        $render = new base_render(app::get('b2c'));
        $input_name = $params['name'];
        parse_str($params['value'],$value);
        $params =array(
                'gtype'=>'',
                'view' => 'admin/goods/jdfinder_filter.html',
                'params' => $params['params'],
                'json' => json_encode($data),
                'data' => $value,
                'from'=>$params['value'],
        ...
        $render->pagedata['params'] = $params;
        $goods_filter = kernel::single('b2c_goods_goodsfilter');
        $return = $goods_filter->goods_jdgoodsfilter();
        $render->pagedata['filter'] = $return;
        $render->pagedata['type_id'] = '';
        $render->pagedata['filter_items'] = array();
        foreach(kernel::servicelist('goods_filter_item') as $key=>$object){
        ...
        return $render->fetch('admin/goods/jdgoods_filter.html');
    }

    //京东图书搜索
	function input_jdbookfilter($params){
        $render = new base_render(app::get('b2c'));
        $input_name = $params['name'];
        parse_str($params['value'],$value);
        $params =array(
        ...
        $render->pagedata['params'] = $params;
        $goods_filter = kernel::single('b2c_goods_goodsfilter');
        $return = $goods_filter->goods_jdbookfilter();
        $render->pagedata['filter'] = $return;
        $render->pagedata['type_id'] = '';
        $render->pagedata['filter_items'] = array();
        foreach(kernel::servicelist('goods_filter_item') as $key=>$object){
