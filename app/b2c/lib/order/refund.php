...
class b2c_order_refund extends b2c_api_rpc_request
{
    ...
    public function order_refund_finish(&$sdf, $status='succ', $from='Back', &$msg='')
    {
        ...
        // 解除库存量的冻结.
        if ($arrFreez['unfreez'] && $sdf_order['order_kind'] !=='jdorder' && $sdf_order['order_kind'] !=='jdbook')
        {
            $is_unfreeze = true;                    
	...
	public function refund_finish_before($sdf,$refund_status,$Log){
		...
		$obj_members_point->change_point($sdf_order['member_id'],intval($sdf['return_score']), $msg, 'order_refund_use', 3,'', $sdf['order_id'],$sdf_order['member_id'], 'refund');

		if($rs){
			$is_refund_finished = false;                
			$is_refund_finished = $this->order_refund_finish($sdf, 'succ', 'Back',$msg);

			$order_info = $obj_order->getRow('pay_status',array('order_id'=>$sdf['order_id']));
