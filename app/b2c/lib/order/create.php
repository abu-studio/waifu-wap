...
class b2c_order_create extends b2c_api_rpc_request
{
    ...
    {	
        ...
        //卡券特殊判断方式 无需配送地址
        if(!empty($sdf['card_info'])){
            $shipping_name['dt_name'] = app::get('b2c')->_('电子券无需配送地址');
        }

        //ebapp 无需配送地址
        if(!empty($sdf['ebapp'])){
            $shipping_name['dt_name'] = app::get('b2c')->_('水电煤支付订单，无需配送地址');
        }
		
		//sand 无需配送地址
        if(!empty($sdf['sand'])){
            $shipping_name['dt_name'] = app::get('b2c')->_('杉德支付订单，无需配送地址');
        }

        //大客户订单 无需配送地址
        if(!empty($sdf['largeCustomer']) || !empty($sdf['cmpay'])){
            $shipping_name['dt_name'] = app::get('b2c')->_('后台自动生成订单，无需配送地址');
        }
        ...
        //水电煤订单增加属性
        if(!empty($sdf['ebapp'])){
            $order_data['order_type'] = 'ebapp';
        }
		
		
		//杉德订单增加属性
        if(!empty($sdf['sand'])){
            $order_data['order_type'] = 'sand';
        }
		
        //大客户订单增加属性
        if(!empty($sdf['largeCustomer'])){
            $order_data['order_type'] = 'largeCustomer';
        }
        //荷包平台订单增加属性
        if(!empty($sdf['cmpay'])){
            $order_data['order_type'] = 'cmpay';
        }
        ...
        if(isset($objCarts['object']['goods']['0']['obj_items']['products']['0']['goods_id'])){
            ...
            $info = $obj_goods->dump($objCarts['object']['goods']['0']['obj_items']['products']['0']['goods_id'],'goods_kind');
            if($info['goods_kind'] == 'entity'){
                $order_data['order_kind'] = 'entity';
            }
        }
		
		//卡券购买判断订单是否为卡券销售订单  2015/6/24
		if($objCarts['object']['goods']['0']['params']['card'] &&( $objCarts['object']['goods']['0']['params']['card']['entity'] >0 || $objCarts['object']['goods']['0']['params']['card']['virtual'] >0 )){
			$order_data['order_kind'] = 'b2c_card';
		}
                    ...
                    if ($obj_type == "goods")
                    {
                        ...
                        foreach ($arrObjInfo as $arrObjItems)
                        {
                            ...
                            if ($arrObjItems['obj_items']['products'][0]['package_use'] == '1')
                            {
								...
								//卡券  2015/6/19
								if( $arrObjItems['params']['card'] && ($arrObjItems['params']['card']['entity'] > 0 || $arrObjItems['params']['card']['virtual'] > 0) ){
									/*卡券开始*/
									$order_objects = array(
										'order_id' => $order_data['order_id'],
										'obj_type' => $obj_type,
									...
									$order_items = array(
										...
										'addon'=>$strAddon,
										'virtual_card'=>$arrObjItems['virtual_card'],
									);
									$quantity_1 = $arrObjItems['params']['card']['entity'];
									if($quantity_1 > 0){
										$order_items1 = $order_items;
										$order_items1['item_type'] = 'entity';
										$order_items1['quantity'] = $this->objMath->number_multiple(array($arrObjItems['obj_items']['products'][0]['quantity'], $quantity_1));
										$order_items1['amount'] = $this->objMath->number_multiple(array($arrObjItems['obj_items']['products'][0]['price']['buy_price'], $quantity_1));

										$order_objects['order_items'][] = $order_items1;
									}
									$quantity_2 = $arrObjItems['params']['card']['virtual'];
									if($quantity_2 > 0){
										$order_items2 = $order_items;
										$order_items2['item_type'] = 'virtual';
										$order_items2['quantity'] = $this->objMath->number_multiple(array($arrObjItems['obj_items']['products'][0]['quantity'], $quantity_2));
										$order_items2['amount'] = $this->objMath->number_multiple(array($arrObjItems['obj_items']['products'][0]['price']['buy_price'], $quantity_2));

										$order_objects['order_items'][] = $order_items2;
									}
									$order_data['order_objects'][$index] = $order_objects;
									/*卡券结束*/
								}else{
									/*原有逻辑*/
									$order_data['order_objects'][$index] = array(
										...
										'order_items' => array(
											array(
												...
												'goods_id'=>$arrObjItems['obj_items']['products'][0]['goods_id'],
												'order_id' => $order_data['order_id'],
												'item_type'=>$arrObjItems['obj_items']['products'][0]['goods_kind'] == 'jdgoods'?'jdgoods':($arrObjItems['obj_items']['products'][0]['goods_kind'] == 'jdbook'?'jdbook':'product'),
												'bn'=>$arrObjItems['obj_items']['products'][0]['bn'],
												'name'=>$arrObjItems['obj_items']['products'][0]['name'],
												'type_id'=>$arrObjItems['obj_items']['products'][0]['type_id'],
												'cost'=>$arrObjItems['obj_items']['products'][0]['price']['cost'],
                            ...
                            else
                            {
								...
								if( $arrObjItems['params']['card'] && ($arrObjItems['params']['card']['entity'] > 0 || $arrObjItems['params']['card']['virtual'] > 0) ){
                                    ...
                                    $goods_cost = $arrObjItems['obj_items']['products'][0]['price']['cost'];
                                    if($arrObjItems['obj_items']['products'][0]['goods_kind'] == 'jdgoods' || $arrObjItems['obj_items']['products'][0]['goods_kind'] == 'jdbook'){
                                        $goods_cost = $arrObjItems['obj_items']['products'][0]['price']['agreed_price'];
                                    }
                                ...
                                if (isset($arrObjItems[$str_service_name]) && $arrObjItems[$str_service_name])
                                {
                                    ...
                                    foreach ($arrObjItems['adjunct'] as $key=>$adjunctItems)
                                    {
                                        ...
                                        if ($arr_service_goods_type_obj['adjunct'])
                                        {
                                            ...
                                            if ($store_mark == '1' && (!$sdf['largeCustomer'] || !$sdf['cmpay']))
											{
