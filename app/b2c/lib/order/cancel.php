...
class b2c_order_cancel extends b2c_api_rpc_request
{
    ...
    public function generate($sdf, &$controller=null, &$msg='')
    {
        ...
        switch($sdf_order['order_type']){
            case 'group':
                ...
                if($apply['nums'] != ''){
                    $goodsnum = $orderItemObj->dump(array('goods_id'=>$apply['gid'],'order_id'=>$sdf['order_id']),'(nums-sendnum) as gnum');
		...
		//判断是否是京东订单。如果是京东订单，调接口更改京东那边订单状态  start
		$orderObj = app::get('b2c')->model('orders');
		$orderDate = $orderObj->dump(array('order_id'=>$sdf['order_id']),'order_kind');
		if($orderDate['order_kind'] == 'jdorder' || $orderDate['order_kind'] == 'jdbook'){
			$jdorderObj = app::get('jdsale')->model('jdorders');
			$jdorderDate =  $jdorderObj->dump(array('order_id'=>$sdf['order_id'],'jdstatus'=>'npaid'),'jdorders_id,order_kind');
			if(!empty($jdorderDate)){
				$params = array('jdOrderId'=>$jdorderDate['jdorders_id']);

                //lpc 获取类型（goods、book）
                $jdgoodsKind = "normal";
                if ($jdorderDate['order_kind'] == "jdbook")
                    $jdgoodsKind = "book";

				$result = kernel::single('jdsale_api_orders')->getCancelOrder($params,$jdgoodsKind);
				if(!$result['result']){
					return false;
				}
				$jdorderObj->update(array('jdstatus'=>'close','order_state'=>'8'),array('order_id'=>$sdf['order_id']));
			}
		}
		//end
    ...
    private function unfreezeGoods($order_id)
    {
        ...
        foreach($sdf_order['order_objects'] as $k => $v)
        {
            ...
            {
                if ($arrItem['item_type'] == 'product' || $arrItem['item_type'] == 'jdgoods' || $arrItem['item_type'] == 'jdbook')
                    $arrItem['item_type'] = 'goods';
				if($arrItem['item_type'] == 'entity' || $arrItem['item_type'] == 'virtual'){
                    $arrItem['item_type'] = 'cards';
                }else{
                    $arrItem['item_type'] = 'goods';
                }
                $arr_params = array(
