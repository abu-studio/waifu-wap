    ...
    public function minus_stock(&$arrData)
    {
        ...
        if ($storage_enable != 'true')
        {
            ...
            if ($arrData['item_type'] != 'gift')
            {
				...
				if ($arrData['item_type'] == 'virtual' || $arrData['item_type'] == 'entity')
                    $arrData['item_type'] = 'cards';
				...
				//修改卡密状态为 1已发放
				if($arrData['item_type'] == 'cards'){
					$obj_cards_pass = kernel::single('cardcoupons_mdl_cards_pass');
					$obj_cards = kernel::single('cardcoupons_mdl_cards');
					$cards_info=$obj_cards->getRow('card_id,goods_id,source',array('goods_id'=>$arrData['goods_id']));
					$where=array(
						'order_id'=>$arrData['order_id'],
						'card_id'=>$cards_info['card_id'],
					);
					if( !$obj_cards_pass->update(array('status'=>'1'),$where) ){
						return false;
					}
				}
            }
