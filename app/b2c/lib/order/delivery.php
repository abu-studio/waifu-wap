    ...
    public function generate($sdf, &$controller=null, &$msg='')
    {
        ...
        foreach($sdf_order['order_objects'] as $k=>$v)
        {
            ...
            foreach ($v['order_items'] as &$__arr_order_items)
            {
				...
				}elseif($__arr_order_items['item_type'] == 'virtual' || $__arr_order_items['item_type'] == 'entity'){
					$goods_item_type = 'cards';
				}else{
        ...
        */
        ...
        {//弹窗确认
            ...
            foreach ($order_items as $key=>$dinfo)
            {
                if ($dinfo['item_type'] == 'product' || $dinfo['item_type'] == 'adjunct' || $dinfo['item_type'] == 'virtual'|| $dinfo['item_type'] == 'entity')
                {
