...
class b2c_cart_postfilter_promotion implements b2c_interface_cart_postfilter {
    ...
    private function _init_rules($cart_objects=null){
        if (!$this->_rules){
    ...
    private function _filter_sql($aFilter) {
		...
		if(isset($aFilter['store_id'])){
			$aWhere[] =sprintf('sdb_b2c_sales_rule_order.store_id in(%s)',$aFilter['store_id']);
			unset($aFilter['store_id']);
		}
	...
	*/
    ...
    public function get_unuse_rule($cart_objects){
        ...
        foreach($this->_rules as $_k => $rule){
            ...
            $validated = $oCond->validate($aCart,$rule['conditions']);
            if(!$validated){
                $s = $oCond->get_unuse_rule( $aCart,$rule );
                if( !$s ) continue;
    ...
    private function _apply_to_item(&$object, &$cart_object){
        ...
        foreach($this->_rules as &$rule) {
				if($object['store_id']!=$rule['store_id']) continue; //将不是当前购物车商品所属店铺的优惠规则去掉验证
                if($this->stop_rules_processing[$rule['rule_type']][$rule['store_id']] && $this->stop_rules_processing[$rule['rule_type']][$rule['store_id']]!==$rule['rule_id'] ) continue;//增加排它性验证
                ...
                if (!$oCond->validate($object, $rule['action_conditions'])) continue;
                    ...
                    if( $rule['stop_rules_processing'] ) {//此处执行结果是如果是店铺促销规格（store_id有值），则不去计算排它？？？
                        $this->stop_rules_processing[$rule['rule_type']][$rule['store_id']] = $rule['rule_id'];
                    }
    }
        ...
        if( isset($arr_use_rule) && is_array($arr_use_rule) ) {
            ...
            if($temp_solution_name) {
                ...
                if(isset($cart_object['promotion'][$tmp_promotion_name][$tmp_use_rule['rule_id']])) {
					...
					$cart_object['promotion']['store_ids'][]= $tmp_use_rule['store_id'];//给优惠规则增加店铺集合，为在页面显示时只有当有当前店铺规则时才显示“享受的优惠”
                                if($oDefault->score_add) continue;
