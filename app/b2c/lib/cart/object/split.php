<?php

 
class b2c_cart_object_split
{
    private $arr_parea=array();
    private $arr_area=array();
    public function split_order(&$controller,$area_id=null,&$sdf_cart){
        if(!isset($area_id)){
            //return;
        }
        $dt_id=array();
        //取得所有购物车类型对应的配送方式。
        foreach( kernel::servicelist('b2c_cart_object_split') as $object ) {
            if( !is_object($object) ) continue;
            if( method_exists($object,'get_dlytype') && is_callable(array($object,'get_dlytype')) ) {
               $temp_dt_id=$object->get_dlytype($sdf_cart);
               if(is_array($temp_dt_id)){
                    $dt_id=array_merge($dt_id,$temp_dt_id);
               }
            }
        }
        $dt_id=array_unique($dt_id);
        //取得结算单中所有的配送方式（不分店铺）。
        $all_dly_types=$this->get_dlytpye($controller,$dt_id,$area_id); 

        $all_dly_types=utils::array_change_key($all_dly_types,'dt_id');
        $valite_dt_id=array_keys($all_dly_types);
        
        //分单数据保存。
        $split_dly=array();
        $index=800;
        //根据配送方式对现有购物车类型分单。
        
        foreach( kernel::servicelist('b2c_cart_object_split') as $object ) {
            if( !is_object($object) ) continue;
            if( method_exists($object,'split_cart_object') && is_callable(array($object,'split_cart_object')) ) {
                $object->split_cart_object($sdf_cart,$split_dly,$valite_dt_id,$index);
            }
        }
        $mdl_business_store = app::get('business')->model('storemanger');
        $store_ids=array_keys($split_dly);
        $storeinfo = $mdl_business_store->getList('store_id,store_name,shop_name',array('store_id'=>$store_ids));
        $storeinfo=utils::array_change_key($storeinfo,'store_id');
        //每一张分单中的商品重量。
        foreach($split_dly as $store_id=>$sgoods){
            $split_dly[$store_id]['info']=$storeinfo[$store_id];
            foreach($sgoods['slips'] as $order_sp=>$order){
                if(!empty($order['dly_type'])){
                    foreach($order['dly_type'] as $dkey=>$type_id){
                       $all_dly_types[$type_id]['subtotal_weight']=$order['subtotal_weight'];
                       $all_dly_types[$type_id]['subtotal_prefilter_after']=$order['subtotal_prefilter_after'];
                       $all_dly_types[$type_id]['store_free_shipping']=$order['store_free_shipping'];
                    }
                }
            }
        }
        
//        $this->select_delivery_method($controller,$area_id,$sdf_cart,'',$all_dly_types);
        foreach($split_dly as $store_id=>$sgoods){
            foreach($sgoods['slips'] as $order_sp=>$order){
                $temp_dly=array();
                if(!empty($order['dly_type'])){
                    foreach($order['dly_type'] as $dkey=>$type_id){

                       //分单后计算运费
                       $this->select_delivery_method($controller, $area_id, $order, '', $all_dly_types);
                        //获取京东店铺运费 start
                        $jdfreight = false;
                        $jdshopId = app::get('site')->getConf('jdsale.shopId') ? app::get('site')->getConf('jdsale.shopId'): '';
                        $jdbookshopId = app::get('site')->getConf('jdbook.shopId') ? app::get('site')->getConf('jdbook.shopId'): '';
                        if($all_dly_types[$type_id]['store_id'] == $jdshopId){
                            $jdprice = $this->get_jddly_types($area_id,$order['object']['goods']['obj_ident'],$order['object']['goods']['quantity']);
                            if(isset($jdprice) && $jdprice !== false){
                                $all_dly_types[$type_id]['dt_name'] = "京东配送";
                                $all_dly_types[$type_id]['money'] = $jdprice;
                            }else{
                                $jdfreight = true;
                            }
                        }
                        elseif ($all_dly_types[$type_id]['store_id'] == $jdbookshopId) {//lpc 增加京东图书运费
                            $jdprice = $this->get_jddly_types($area_id,$order['object']['goods']['obj_ident'],$order['object']['goods']['quantity'],'book');
                            if(isset($jdprice) && $jdprice !== false){
                                $all_dly_types[$type_id]['dt_name'] = "京东配送";
                                $all_dly_types[$type_id]['money'] = $jdprice;
                            }else{
                                $jdfreight = true;
                            }
                        }
                        //获取京东店铺运费 end
                        if(!$jdfreight){
                            $temp_dly[$type_id]=array(
                                'dt_id'=>$all_dly_types[$type_id]['dt_id'],
                                'dt_name'=>$all_dly_types[$type_id]['dt_name'],
                                'store_id'=>$all_dly_types[$type_id]['store_id'],
                                'protect'=>$all_dly_types[$type_id]['protect'],
                                'protect_money'=>$all_dly_types[$type_id]['protect_money'],
                                'protect_text'=>$all_dly_types[$type_id]['protect_text'],
                                'money'=>$all_dly_types[$type_id]['money'],
                                'default_type'=>'false'
                            );
                        }

                       //$temp_dly[$type_id]['dt_id']=$all_dly_types[$type_id];dt_name
                       //$temp_dly[$type_id]['default_type']='false';
                    }
                }
                if($temp_dly){
                    reset($temp_dly);
                    $temp_dly[key($temp_dly)]['default_type']='true';
                    
                }
                $split_dly[$store_id]['slips'][$order_sp]['shipping']=$temp_dly;
            }
        }
        //echo '<pre>';   print_r($sdf_cart['object']['goods']);echo '</pre>';
        //echo '<pre>';   print_r($split_dly);echo '</pre>';
        $sdf_cart['order_split']=$split_dly;
        $sdf_cart['json_order_split']=serialize($split_dly);
        $sdf_cart['all_dly_types']=$all_dly_types;
        return $split_dly;
    }
    
    public function select_delivery_method(&$controller, $area_id='', &$sdf_cart, $shipping_method='',&$all_dly_types)
    {   
        $pay_app_id = $sdf_cart['pay_app_id'] ? $sdf_cart['pay_app_id'] : '';
        $controller->pagedata['shipping_method'] = json_decode($shipping_method, true);
        $shipping_id = $controller->pagedata['shipping_method']['shipping_id'];
        $controller->pagedata['is_shipping_match'] = 0;
		/** 阶梯费用只能根据优惠后的金额来处理 - 除去商品优惠和订单优惠后的最终价格 **/
		$objMath = kernel::single('ectools_math');
        $cost_item = $objMath->number_minus(array($sdf_cart['subtotal'], $sdf_cart['discount_amount_prefilter'], $sdf_cart['discount_amount_order']));

        foreach ($all_dly_types as $rows)
        {
            if ($rows['protect'] == 'true')
            {  
                /** 保价费界定为商品的最原始价格 **/
                $protect = $objMath->number_multiple(array($rows['subtotal_prefilter_after'], $rows['protect_rate']));
                $rows['protect_money'] = $protect>$rows['minprice']?$protect:$rows['minprice'];//保价费
                $rows['protect_text']=app::get('b2c')->_('商品价格的').($rows['protect_rate']*100).'%，'. app::get('b2c')->_('不足').$rows['minprice'].app::get('b2c')->_('元按').$rows['minprice'].app::get('b2c')->_('元计算');
            }
            if ($rows['is_threshold'])
            {
                if ($rows['threshold'])
                {
                    $rows['threshold'] = unserialize(stripslashes($rows['threshold']));
                    if (isset($rows['threshold']) && $rows['threshold'])
                    {
                        foreach ($rows['threshold'] as $res)
                        {
                            if ($res['area'][1] > 0)
                            {
                                if ($cost_item >= $res['area'][0] && $cost_item < $res['area'][1])
                                {
                                    $rows['firstprice'] = $res['first_price'];
                                    $rows['continueprice'] = $res['continue_price'];
                                }
                            }
                            else
                            {
                                if ($cost_item >= $res['area'][0])
                                {
                                    $rows['firstprice'] = $res['first_price'];
                                    $rows['continueprice'] = $res['continue_price'];
                                }
                            }
                        }
                    }
                }
            }
			if($rows['store_free_shipping'] === 0){
				$rows['money'] = 0;
			}else{
				$rows['money'] = @utils::cal_fee($rows['dt_expressions'], $rows['subtotal_weight'], $sdf_cart['subtotal'], $rows['firstprice'], $rows['continueprice'], $rows['firstprice']);
			}
            $shipping[$rows['dt_id']] = $rows;
            
        }
		foreach ((array)$obj_dlytype_detail_extends = kernel::servicelist('b2c.dlytype.detail.extends') as $obj)
		{
			if (method_exists($obj, 'extends_shipping_detail'))
			{
				$obj->extends_shipping_detail($shipping);
			}
		}
        $all_dly_types=$shipping;
    }
    private function get_dlytpye(&$controller,$dt_id=array(),$area_id=null){
        if(empty($dt_id)){
            return array();
        }
        $objRegions = app::get('jdsale')->model('regions');
        $objdlytype = $controller->app->model('dlytype');
        $filter = array('dt_status'=>'1');
        $filter['dt_id'] = $dt_id;
        $dlytype = $objdlytype->getList('*',$filter,0,-1,'ordernum ASC');

        if ($dlytype && is_array($dlytype)) 
        {
            $setting_0 = $setting_1 = array();
            $sdf_area_id = $area_id;

            foreach ($dlytype as $key => $value) 
            {
                if ($value['setting'] == 1) 
                {
                    $setting_1[$value['ordernum'].'.'.$value['dt_id']] = $value;
                }
                else
                {
                    if ($value['def_area_fee'] == 'true') 
                    {
                        $setting_0[$value['ordernum'].'.'.$value['dt_id']] = $value;
                    }

                    $area_fee_conf = unserialize($value['area_fee_conf']);
                    if ($area_fee_conf && is_array($area_fee_conf)) 
                    {
                        foreach ($area_fee_conf as $conf_key => $conf_value) 
                        {
                            $conf_areas = explode(',', $conf_value['areaGroupId']);

                            foreach ($conf_areas as $area_key => &$area_value) 
                            {
                                if (strpos($area_value, '|') !== false) 
                                {
                                    $area_value = substr($area_value, 0, strpos($area_value, '|'));
                                    
                                    if (!empty($sdf_area_id)) 
                                    {
                                        if(!in_array($sdf_area_id, $this->arr_area)){
                                            $this->arr_area[$sdf_area_id]= $objRegions->dump(array('area_id'=>$sdf_area_id));
                                        }
                                        $arrRegion =$this->arr_area[$sdf_area_id];
                                        while ($row = $objRegions->dump(array('area_id'=>$arrRegion['p_region_id'])))
                                        {
                                            $arrRegion = $row;
                                            $tmp_area_id = $row['area_id'];


                                            if ($tmp_area_id == $area_value)
                                            {
                                                $sdf_area_id = $tmp_area_id;
                                                break;
                                            }
                                        }
                                    }
                                }
                            }

                            if (!empty($sdf_area_id)) 
                            {
                                if(in_array($sdf_area_id, $conf_areas))
                                {   //如果地区在其中，优先使用地区设置的配送费用，及公式
                                    $value['firstprice'] = $conf_value['firstprice'];
                                    $value['continueprice'] = $conf_value['continueprice'];
                                    $value['dt_expressions'] = $conf_value['dt_expressions'];
                                    $setting_0[$value['ordernum'].'.'.$value['dt_id']] = $value;
                                    break;
                                }
                            }
                            else
                            {
                                $value['firstprice'] = $conf_value['firstprice'];
                                $value['continueprice'] = $conf_value['continueprice'];
                                $value['dt_expressions'] = $conf_value['dt_expressions'];
                                $setting_0[$value['ordernum'].'.'.$value['dt_id']] = $value;
                            }
                        }
                    }
                }
            }
            $return = array_merge($setting_1, $setting_0);
            ksort($return);
            return $return;
        }
        return array();
    }

    function get_jddly_types($area_id,$goods_array,$quantity,$jdgoodsKind='normal'){
        if($area_id == "")  return false;
        if(empty($goods_array)) return false;
        $goods_id = array();
        foreach($goods_array as $k=>$v){
            if($v){
                $goods_tmp = explode('_',$v);
                $goods_id[$k] = $goods_tmp[1];
            }
        }

        if(empty($goods_id)){
           return false;
        }

		$jdskuidarray = array();
        $mdl_b2c_goods = app::get('b2c')->model('goods');
		foreach($goods_id as $k=>$val){
			$goodsData = $mdl_b2c_goods->getRow('bn',array('goods_id'=>$val));
			$jdskuidarray[] = array(
                'skuId'=>intval($goodsData['bn']),
                'num'=>$quantity[$k],
            );

		}

        $lib_jdsale = kernel::single('jdsale_api_goods');
        $mdl_jdsale_regions = app::get('jdsale')->model('regions');
        
        $jdsale_array = $mdl_jdsale_regions->dump(array('area_id'=>$area_id),'region_path');

        $jdsale_area_array = explode(",",$jdsale_array['region_path']);
        foreach( $jdsale_area_array as $k1=>$v1){
            if( !$v1 )
                unset( $jdsale_area_array[$k1] );
        }
        reset($jdsale_area_array);
        $jd_array = array(
            'sku'=>$jdskuidarray,
            'province'=>$jdsale_area_array[1],
            'city'=>$jdsale_area_array[2]?$jdsale_area_array[2]:0,
            'county'=>$jdsale_area_array[3]?$jdsale_area_array[3]:0,
            'Town'=>$jdsale_area_array[4]?$jdsale_area_array[4]:0,
            'paymentType'=>1
        );
        $jdresponse = $lib_jdsale->queryOrderFreight($jd_array,$jdgoodsKind);
        if(!empty($jdresponse['result'])){
            return $jdresponse['result']['freight'];
        }
        return false;

    }
    
}
