...
class b2c_cart_object_goods implements b2c_interface_cart_object
{
    ...
    public function __construct()
    {
        ...
        if (!empty($this->arr_member_info)) {
            ...
            $this->jd_discount = (empty($aMLV['jd_discount']) || $aMLV['jd_discount'] <= 0) ? 1 : $aMLV['jd_discount'];
        }
    ...
    public function check_store($arr_data, $arr_carts, &$msg = '', $_check_adjunct = true)
    {
        ...
        /**判断卡券数量 2015/7/14**/
        if ($arr_data['goods']['cards_pass_type']) {
            $obj_cards_pass = kernel::single('cardcoupons_mdl_cards_pass');
            $obj_cards = kernel::single('cardcoupons_mdl_cards');

            $cards_info = $obj_cards->getRow('card_id,source', array('goods_id' => $goods_id));
            if ($cards_info) {
                $source = $cards_info['source'];
                $type = $arr_data['goods']['cards_pass_type'];

                //内部卡——电子码，自动生成无需判断卡券数量
                //内部卡--电子码发货需校验库存
                if ($source != 'internal' || $type == 'virtual') {
                    $pass = $obj_cards_pass->getList('*', array('card_id' => $cards_info['card_id'], 'disabled' => 'false', 'status' => '0', 'ex_status' => 'true', 'type' => $type, 'source' => $source), 0, intval($aData['quantity']));
                    if (empty($pass) || count($pass) < $aData['quantity']) {
                        $msg = '卡券卡密数量不足';
                        return false;
                    }
                }
    ...
    public function add_object($aSave, &$msg = '', $append = true)
    {
        ...
        if ($append) {
            ...
            if ($aData = $this->oCartObjects->getList('*', $filter, 0, -1, -1)) {
                ...
                //卡密类型 2015/6/19
                if ($aSave["params"]['card']) {
                    $aSave["params"]["card"]["entity"] += $aData["params"]["card"]["entity"];
                    $aSave["params"]["card"]["virtual"] += $aData["params"]["card"]["virtual"];
                }
            }
        }
    ...
    public function getAll($rich = false, $aData = array())
    {
        ...
        if ($_SESSION['cart_select_data']['cart_check'] == 1) {
            foreach ($aResult as $key => $value) {
                $obj_idents = explode('_', $value['obj_ident']);
                if ($obj_idents['0'] == 'goods') {
                    if ($_SESSION['cart_select_data']['select_products'] && is_array($_SESSION['cart_select_data']['select_products'])) {
                        if (!in_array($value['obj_ident'], $_SESSION['cart_select_data']['select_products'])) {
                            unset($aResult[$key]);
                        }
                    }
                }
    ...
    private function _generateParams($aData)
    {
        ...
        //卡密类型 2015/6/19
        if ($aData['cards_pass_type']) {
            $return["card"]["entity"] = $aData['cards_pass_type'] == "entity" ? floatval(($aData['num'] ? $aData['num'] : 1)) : 0;
            $return["card"]["virtual"] = $aData['cards_pass_type'] == "virtual" ? floatval(($aData['num'] ? $aData['num'] : 1)) : 0;
        }
        return $return;
    }
    ...
    private function _get($aData)
    {
        ...
        foreach ($aData as $key => &$row) {
            ...
            $store_id = $oGoods->dump(array('goods_id' => $goods_id), 'store,store_freeze,store_id,freight_bear');
            ...
            if (isset($tmp_products_store[$product_id])) {
                ...
                $tmp_store = array(
                    ...
                    'real_store' => $store_id['store'] - $store_id['store_freeze'],//商品真实库存
                );
    ...
    function _get_products($aProductId)
    {
        ...
        $sSql = "SELECT
                     ...
                     g.goods_type,g.agreed_price, g.nostore_sell, g.act_type,g.min_buy,g.type_id,g.cat_id,g.image_default_id,p.spec_info,p.spec_desc,p.price,p.weight,p.mktprice as p_mktprice,g.mktprice as g_mktprice,
        ...
        //////////// 整理数据 /////////////////////////////
        $jdProductKinds = array('jdgoods', 'jdbook');
        $aResult = array();

        foreach ($aProduct as $row) {

            $goodsDateK = app::get('b2c')->model('goods')->getRow('goods_kind', array('goods_id' => $row['goods_id']));
            if (in_array($goodsDateK['goods_kind'], $jdProductKinds)) {
                $discout = $this->jd_discount;
            }
            ...
            $aResult[$row['product_id']] = array(
                ...
                'goods_kind' => $goodsDateK['goods_kind'],
                'default_image' => array(
            ...
            if (in_array($goodsDateK['goods_kind'], $jdProductKinds)) {
                $aResult[$row['product_id']]['url'] = $router->gen_url(array('app' => 'jdsale', 'ctl' => 'site_product', 'full' => 1, 'act' => 'index', 'arg' => $aResult[$row['product_id']]['goods_id']));
                $aResult[$row['product_id']]['thumbnail'] = jdsale_goods_import::$image_url_n3 . $aResult[$row['product_id']]['default_image']['thumbnail'];
            } else {
