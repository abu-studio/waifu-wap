...
class b2c_cart_object_split_goods
{
    ...
    public function split_cart_object(&$sdf_cart,&$split_dly,$valite_dt_id=array(),&$index){
        ...
        foreach ((array)$sdf_cart['object']['goods'] as $key =>$value) {
			...
			$slip['object']['goods']['quantity'][$key]=$value['quantity'];
            $slip['dly_type']=$t_dly;
			...
			//卡券只选电子码时免运费。2015/6/17 
			$slip['store_free_shipping']+=( $value['params']['card'] && $value['params']['card']["entity"] == 0 && $value['params']['card']["virtual"] > 0 )?0:($value['freight_bear']=='business'?0:1); 
