...
class b2c_mdl_orders extends dbeav_model{
    ...
    public function gen_id()
    {
        ...
        do{
            ...
            $row_fyw = $this->db->selectrow('SELECT order_id from sdb_b2c_orders_fyw where order_id ='.$order_id);
        }while($row || $row_fyw);
        return $order_id;
    }
    ...
    public function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null)
    {
        ...
        foreach ( $arr_list as $key => $val ) {
            $this->_cacheOrder[$val['order_id']] = $val;
        }
        return $arr_list;
    }
    ...
    public function modifier_member_id($row)
    {
        if ($row === 0 || $row == '0'){
            ...
            if ( isset($this->_cacheAccount[$row]) ) {
                $sdf = $this->_cacheAccount[$row];
            } else {
                $obj_member = app::get('pam')->model('account');
                $sdf = $obj_member->dump($row);
                $this->_cacheAccount[$row] = $sdf;
            }
    ...
    public function modifier_final_amount($row)
    {
        ...
        if ( $this->_cacheOrder[$this->pkvalue] ) {
            $tmp[0] = $this->_cacheOrder[$this->pkvalue];
        } else {
    ...
    function _filter($filter,$tableAlias=null,$baseWhere=null){
		//商社名称模糊搜索
        if (isset($filter) && $filter && is_array($filter) && array_key_exists('company_name', $filter))
        {
            $obj_pam_account = app::get('pam')->model('account');
            $pam_filter = array(
                'company_name|has'=>$filter['company_name'],
            );
            $row_pam = $obj_pam_account->getList('*',$pam_filter);
            $arr_member_id = $arr_company_no = array();
            if ($row_pam)
            {
                foreach ($row_pam as $str_pam)
                {
                    $arr_member_id[] = $str_pam['account_id'];
                    $arr_company_no[] = $str_pam['company_no'];
                }
                // $filter['member_id|in'] = $arr_member_id;
                $arr_member_id_list = implode(",", $arr_member_id);
                $filter['filter_sql'] = ' `member_id` in ('.$arr_member_id_list.')';
                //按商社名称搜索时，一般雇员订单按照前面逻辑，但人事经理的订单，商社应显示为下单选择支付账户的商社，所以按商社名称搜索时，按照java_payment_company来搜索
                $arr_company_no = array_unique($arr_company_no);
                if(!empty($arr_company_no)){
                    $arr_company_no_list = implode('","', $arr_company_no);
                    $filter['filter_sql'] = '( `member_id` in ('.$arr_member_id_list.') or `java_payment_company` in ("'.$arr_company_no_list.'"))';
                }
            }
            unset($filter['company_name']);
        }
		//会员用户名模糊搜索
    ...
    public function searchOptions(){
		...
		/** 添加商社名称搜索 **/
        $columns['company_name'] = app::get('b2c')->_('商社名称');
        /** end **/
    ...
    */
    ...
    public function fgetlist_csv( &$data,$filter,$offset,$exportType =1 ){
        ...
        if(!$data['title']){
            $this->title = array();
			array_push($this->title,'订单号(order)id)','运单号(logi_no)','下单时间(createtime)','更新时间(last_modified)','成本价(cost)','订单总额(final_amount)','支付状态(pay_status)','发货状态(ship_status)','支付方式(payment)','配送方式(shipping)','订单状态(status)','退款状态(refund_status)','订单类型(order_kind )','产品名称(name)','产品编号(bn)','会员名称(login_name)','商社号(company_no)','商社名称(company_name)','店主名称(shop_name)','店铺名称(store_name)','商品数量(nums)','收货地区(ship_area)','收货地址(ship_addr)','收货人手机(ship_mobile)','邮编(ship_zip)','留言(memo)','收货人(ship_name)','支付宝支付金额','福点支付金额');
		...
		$paydata = $obj_paymentsfg->getList('app_id,app_display_name');
		foreach($paydata as $val){
			$payArr[$val['app_id']] = $val['app_display_name'];
		}


        if($filter['isSelectedAll'] == '_ALL_' &&  empty($filter['order_id'])){
            $tmp = $this->getList('order_id',$filter,$offset*$limit,$limit);
            $tmp_order = array();
            if(!empty($tmp)){
		...
		$orderId_str = implode(',',$filter['order_id']);
		$sql="SELECT  
			o.order_id,
			it.cost,
			o.createtime,
			...
			LEFT JOIN sdb_b2c_goods gd on gd.goods_id = it.goods_id where o.order_id in (".$orderId_str.") limit ".($offset*$limit).",".$limit;
		if(!$item_list = kernel::database()->select($sql))return false;

        foreach ( $item_list as $key => $val ) {
            $_orderIda[] = $val['order_id'];
        }
		
		//获取各类支付金额
		$sqls = "SELECT p.pay_app_id,p.money,b.rel_id from  sdb_ectools_payments p LEFT JOIN sdb_ectools_order_bills b on p.payment_id = b.bill_id  where b.rel_id in (".implode(',', $_orderIda).")"; 
		$payTypedata = kernel::database()->select($sqls);
		foreach($payTypedata as $payval){
			$payType_data[$payval['rel_id']][$payval['pay_app_id']] = $payval['money'];
		}

		
		foreach($item_list as $key=>$item){
			$payname = '';
			$pay_name_money = array();
			$newpay_name_money = array();
            $paynameArr = array();
			if (intval($item['payment']) < 0)
				$payname = app::get('b2c')->_('货到付款');
			else
			{  
				$payment_arr = explode(',',$item['payment']);
				foreach($payment_arr as $payval){
					$paynameArr[] = $payArr[$payval];
				}
				$payname = implode('+',$paynameArr);
			} 

			$dataarr = array();
			$dataarr = array(
				'order_id'=>$item['order_id'],
				'logi_no'=>"\r".$item['logi_no'],
				...
				'alpay_money'=>$payType_data[$item['order_id']]['alipay'],
				'sfscpay_momey'=>$payType_data[$item['order_id']]['sfscpay']
			);
			$rowVal = array();
			foreach( $dataarr as $col => $val ){
				...
				if(is_array($cols[$col]['type'])){
					$val = $cols[$col]['type'][$val];
				}
