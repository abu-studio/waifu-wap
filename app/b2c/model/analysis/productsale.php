...
class b2c_mdl_analysis_productsale extends dbeav_model{
    ...
    public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
        $sql = 'SELECT I.goods_id as rownum,G.goods_kind,G.name as pname,G.bn as bn,sum(I.nums) as saleTimes,sum(I.amount) as salePrice,I.goods_id FROM '.
            kernel::database()->prefix.'b2c_orders as O LEFT JOIN '.
            kernel::database()->prefix.'b2c_order_items as I ON O.order_id=I.order_id LEFT JOIN '.
            kernel::database()->prefix.'b2c_goods as G ON G.goods_id=I.goods_id WHERE '.
        ...
        foreach($rows as $key=>$val){
			...
			//如果是京东商品，修改图片地址
			if($val['goods_kind'] == 'jdgoods' || $val['goods_kind'] == 'jdbook'){
				$rows[$key]['image_default_id'] = jdsale_goods_import::$image_url_n3.$image['image_default_id'];
				$rows[$key]['udfimg'] = jdsale_goods_import::$image_url_n3.$image['image_default_id'];
				$rows[$key]['thumbnail_pic'] = jdsale_goods_import::$image_url_n3.$image['image_default_id'];
			}else{
