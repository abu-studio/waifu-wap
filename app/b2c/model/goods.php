...
//            return $this->ioTitle['csv'][$filter['type_id']];
        ...
        switch ($ioType) {
            ...
            default:
                ...
                $this->oSchema['csv'][$filter['type_id']] = array(
                    ...
                    app::get('b2c')->_('col:店铺名称') => 'store_id',
                );
                ...
                $this->oSchema['csv'][$filter['type_id']] = array_merge(
                    ...
                    array(
                        ...
                        app::get('b2c')->_('tax_rate:税率') => 'tax_rate',
                        app::get('b2c')->_('ticket_type:票种') => 'ticket_type',
    ...
    function dump($filter, $field = '*', $subSdf = null)
    {
        ...
        if (empty($dumpData['images']) && $dumpData['image_default_id']) {
            $timeNow = time();
            $new_image_attach = array(
                'target_id' => $dumpData['goods_id'],
                'target_type' => 'goods',
                'image_id' => $dumpData['image_default_id'],
                'last_modified' => $timeNow,
            );
            $dumpData['images'] = array($new_image_attach);
        }
    ...
    function save(&$goods, $mustUpdate = null)
    {
        ...
        if ($goods['product']) {
            ...
            $goodsName = addslashes($goods['name']);
            $goodsId = intval($goods['goods_id']);

            $sql = "update `sdb_b2c_products` set `name` = '$goodsName' where `goods_id` = '$goodsId'" ;
            $db = kernel::database();
            $db->exec($sql);
        }
        unset($goods['spec']);
        ...
        if ($obj_operatorlogs = kernel::service('operatorlog')) {
            if (method_exists($obj_operatorlogs, 'inlogs')) {
                if (empty($goods['goods_id']) || $goods['goods_kind'] == 'jdgoods' || $goods['goods_kind'] == 'jdbook') {
    ...
    function jdorderBy($id = null)
    {
        $order = array(//更新单一维度排序时如果排序相同则按照评分排序。
            1 => array('label' => app::get('b2c')->_('默认')),
            2 => array('label' => app::get('b2c')->_('按销量 从高到低'), 'sql' => 'buy_m_count desc'),
            4 => array('label' => app::get('b2c')->_('按价格 从高到低'), 'sql' => 'diffprice desc'),
            5 => array('label' => app::get('b2c')->_('按价格 从低到高'), 'sql' => 'diffprice'),
            6 => array('label' => app::get('b2c')->_('按价格 从高到低'), 'sql' => 'price desc'),
            7 => array('label' => app::get('b2c')->_('按价格 从低到高'), 'sql' => 'price'),
        );
...
//                    }
    ...
    function getHfPath($gid, $method = null)
    {
        $gids['goods_id'] = $gid;
        $list_row = $this->getList("cat_id,name", array('goods_id' => $gid));
        $row = $list_row[0];
        $goods = &$this->app->Model('goods_cat');
        $ret = $goods->getHfPath($row['cat_id'], $method);
        $ret[] = array('type' => 'goods', 'title' => $row['name'], 'link' => '#');
        return $ret;
    }

    function jdgetPath($gid, $method = null)
    {
        $gids['goods_id'] = $gid;
        $list_row = $this->getList("jd_cat_id,name", array('goods_id' => $gid));
        $row = $list_row[0];
        $goods = app::get('jdsale')->Model('goods_cat');
        $ret = $goods->getPath($row['jd_cat_id'], $method);
        $ret[] = array('type' => 'goods', 'title' => $row['name'], 'link' => '#');
        return $ret;
    }

    function jdgetHfPath($gid, $method = null)
    {
        $gids['goods_id'] = $gid;
        $list_row = $this->getList("jd_cat_id,name", array('goods_id' => $gid));
        $row = $list_row[0];
        $goods = app::get('jdsale')->Model('goods_cat');
        $ret = $goods->getHfPath($row['jd_cat_id'], $method);
        $ret[] = array('type' => 'goods', 'title' => $row['name'], 'link' => '#');
        return $ret;
    }
    ...
    function fgetlist_csv(&$data, $filter, $offset)
    {
        ...
        //获取票种数组
        $schema_data = $this->get_schema();
        $ticketType_arr = $schema_data['columns']['ticket_type']['type'];

        //获取用户等级
        $mem_lv_object = app::get('b2c')->model('member_lv');
        $mem_data = $mem_lv_object->getList('name');
        foreach ($mem_data as $val) {
            $title[] = $val['name'];
        }

        //获取分类数据
        $goods_cat_object = app::get('b2c')->model('goods_cat');
        $cat_data = $goods_cat_object->getList('cat_id,cat_name');
        foreach ($cat_data as $key => $val) {
            $catdata[$val['cat_id']] = $val['cat_name'];
        }

        if ($filter['isSelectedAll'] == '_ALL_' && empty($filter['goods_id'])) {
            $tmp = $this->getList('goods_id', $filter);
            $tmp_order = array();
            if (!empty($tmp)) {
                foreach ($tmp as $tmp_k => $tmp_v) {
                    $tmp_order[] = $tmp_v['goods_id'];
                }
            }
            if (!empty($tmp_order)) {
                $filter['goods_id'] = $tmp_order;
            }
        }


        $goods_id = implode(',', $filter['goods_id']);
        $sql = 'SELECT 
				t.name as type_name,g.bn,p.bn as gbn,g.cat_id,c.cat_path,b.brand_name,group_concat(distinct(k.keyword)) as keyword,p.mktprice,p.cost,p.price,g.`name`,g.marketable,p.spec_desc,bs.store_name,g.brief,g.intro,p.weight,g.unit,g.tax_rate,g.ticket_type,p.store,p.product_id
				from  
				sdb_b2c_goods g 
				LEFT JOIN sdb_b2c_products p ON g.goods_id = p.goods_id
				LEFT JOIN sdb_b2c_goods_type t ON g.type_id =t.type_id
				LEFT JOIN sdb_b2c_goods_cat c ON g.cat_id = c.cat_id
				LEFT JOIN sdb_b2c_brand b ON b.brand_id = g.brand_id
				LEFT JOIN sdb_b2c_goods_keywords k ON k.goods_id = g.goods_id 
				LEFT JOIN sdb_business_storemanger  bs ON bs.store_id = g.store_id 
				where  g.goods_id in  (' . $goods_id . ')
				GROUP BY p.product_id';
        $goods_data = kernel::database()->select($sql);
        if (!$goods_data) return false;

        $new_data = array();
        foreach ($goods_data as $k => $v) {
            $new_data[$k][] = $v['type_name'];   //商品类型
            $new_data[$k][] = $v['bn'];                //商品编号
            $new_data[$k][] = $v['gbn'];            //规格货号
            $path_arr = array_filter(explode(',', $v['cat_path']));
            //分类
            $path_str = '';
            foreach ($path_arr as $val) {
                $path_str .= $catdata[$val] . ' -> ';
            }
            $path_str .= $catdata[$v['cat_id']];
            $new_data[$k][] = $path_str;
            $new_data[$k][] = $v['brand_name'];  //品牌
            $new_data[$k][] = $v['keyword'];    //商品关键字
            $new_data[$k][] = $v['mktprice'];   //市场价
            $new_data[$k][] = $v['cost'];  //成本价
            $new_data[$k][] = $v['price'];   //销售价
            $new_data[$k][] = $v['name'];   //商品名称
            $new_data[$k][] = $v['store_name'];  //店铺名称
            $new_data[$k][] = $v['marketable'] == 'true' ? 'Y' : 'N';   //上架
            //规格
            $spec = unserialize($v['spec_desc']);
            $new_data[$k][] = implode(',', $spec['spec_value']);
            $new_data[$k][] = $v['type_name'];   //商品简介
            $new_data[$k][] = str_replace("\n", " ", str_replace('"', '""', $v['intro']));;  //商品详情
            $new_data[$k][] = $v['weight'];   //重量
            $new_data[$k][] = $v['unit'];    //单位
            $new_data[$k][] = $v['store'];    //库存
            $new_data[$k][] = $v['tax_rate'];    //税率
            $new_data[$k][] = $ticketType_arr[$v['ticket_type']];    //票种
            $product_id[] = $v['product_id'];    //商品ID数组
        }

        //根据product获取对应货品对应的会员等级价格
        $sqls = "SELECT 
				p.product_id,l.name,price 
				from  sdb_b2c_goods_lv_price p
				LEFT JOIN sdb_b2c_member_lv l ON p.level_id = l.member_lv_id where p.product_id in (" . implode(',', $product_id) . ")";

        $lv_price_data = kernel::database()->select($sqls);
        foreach ($lv_price_data as $k => $val) {
            $lvPrice_data[$val['product_id']][$val['name']] = $val['price'];
        }

        //将对应商品等级价格加入数据中
        foreach ($goods_data as $key => $value) {
            foreach ($mem_data as $v) {
                $new_data[$key][] = $lvPrice_data[$value['product_id']][$v['name']] ? $lvPrice_data[$value['product_id']][$v['name']] : $value['price'];
            }
        }
        $data['title'] = $title;
...
//        $product = $sdfdata['product'];
        ...
        $dbschema_goods = app::get('b2c')->model('goods')->get_schema();
        foreach ($this->oSchema['csv'][$sdfdata['type']['type_id']] as $title => $sdfpath) {
            if (!is_array($sdfpath)) {
                $tSdfCol = utils::apath($sdfdata, explode('/', $sdfpath));
                //如果字段 type是数组
                if (is_array($dbschema_goods['columns'][$sdfpath]['type'])) {
                    $tSdfCol = $dbschema_goods['columns'][$sdfpath]['type'][$tSdfCol];
                }
        ...
        //给导出的字段增加店铺名称，由id更换为shop_name
        $shop = kernel::single('business_mdl_storemanger')->getList('store_name,store_id', array('store_id' => $gcontent[app::get('b2c')->_('col:店铺名称')]));
        if ($shop[0]['store_id'] == $gcontent[app::get('b2c')->_('col:店铺名称')]) {
            $gcontent[app::get('b2c')->_('col:店铺名称')] = $shop[0]['store_name'];
        }
        /*
        if( $sdfdata['images'] ){
            $oImage = &app::get('image')->model('image');
		...
		*/
        ...
        if (!$sdfdata['spec']) {
            ...
            foreach ($sdfdata['product'] as $row => $aSdfdata) {
                $content = $gcontent;
                foreach ($this->oSchema['csv'][$sdfdata['type']['type_id']] as $title => $sdfpath) {
                    $content[$title] = $content[$title] ? $content[$title] : utils::apath($aSdfdata, explode('/', (!is_array($sdfpath) ? $sdfpath : $sdfpath[0])));
                }
                $specValue = array();
...
//            break;
    ...
    private function _getSearchGoodsCount($filter = array(), $str_where = '')
    {
        ...
        $sql[] = " and `sdb_b2c_goods`.cat_id not in (select cat_id from `sdb_b2c_goods_cat` where hidden = 'true')";
        $sql[] = " and `sdb_b2c_goods`.goods_kind != 'jdgoods'";
        $sql[] = " and `sdb_b2c_goods`.goods_kind != 'jdbook'";
        $ssql = implode(' ', $sql);
        //print_r($ssql);
    ...
    function getSearchCards($cols = '*', $filter = array(), $offset = 0, $limit = -1, $orderType = null, $_cat = array(), &$count = 0)
    {
        $total = $this->_getSearchCardsCount($filter);
        if (empty($total)) {
            $count = 0;
            return array();
        }
    ...
    // 卡券列表页搜索专用
    private function _getSearchCardsCount($filter = array(), $str_where = '')
    {
        $sql = array();
        $sql[] = "SELECT";
        $sql[] = "COUNT(`sdb_b2c_goods`.goods_id) as gcount";
    ...
    /*用于手机端 预留 -xhk
       function getSearchGoods_cat($filter=array()){
        $sql=array();
        $sql[]="SELECT";
        $sql[]=" `sdb_b2c_goods`.cat_id,count(`sdb_b2c_goods`.cat_id) as count";
        $sql[]=" from sdb_b2c_goods";
        $sql[]=" left join sdb_business_storemanger as s on `sdb_b2c_goods`.store_id=s.store_id";
        $sql[]=" left join (";
        //店铺评分
        ...
        $sql[]=" and s.limit_storedown='0'";
        $sql[]=" and s.limit_store='0'";
        $sql[]=" group by `sdb_b2c_goods`.cat_id";
        $ssql=implode(' ',$sql);
        //echo $ssql;
        $rows=$this->db->select($ssql);
        return $rows;
    ...
    */
    ...
    //京东商品搜索
    function getSearchjdGoods($cols = '*', $filter = array(), $offset = 0, $limit = -1, $orderType = null, $_cat = array(), &$count = 0)
    {
        $total = $this->_getSearchJdgdoodsCount($filter);
        if (empty($total)) {
            $count = 0;
            return array();
        }
        ...
        //商品价格	价格越低，排名越前	2%
        if ($total['maxPrice'] != $total['minPrice'] && !empty($total['subPrice'])) {
            $sql[] = " + (100 - `sdb_b2c_goods`.price/" . floatval($total['subPrice']) . ")*0.02";
        }
        //获取京东商品屏蔽分类
        $jd_cat_id = $this->get_jd_cat_id();


        $sql[] = ")) as dorder,ABS(d.new_price - d.old_price) AS diffprice";
        $sql[] = " from `sdb_b2c_goods`";
        $sql[] = " left join sdb_business_storemanger as s on `sdb_b2c_goods`.store_id=s.store_id";
        $sql[] = " LEFT JOIN sdb_jdsale_diffprice AS d ON `sdb_b2c_goods`.goods_id = d.goods_id";
        $sql[] = " where " . $this->jdgood_filter($filter);
        if ($jd_cat_id) {
            //为避免出现12,,23,24的情况，这里先处理下
            $jd_cat_id_str = implode(',', $jd_cat_id);
            $jd_cat_id_str = preg_replace('/,,/', ',', $jd_cat_id_str);

            $sql[] = " and `sdb_b2c_goods`.jd_cat_id not in (" . $jd_cat_id_str . ")";
        }
        // $sql[]=" and goods_kind = 'jdgoods' and marketable = 'true'";
        // lpc 修改sql语句
        $sql[] = " and (`sdb_b2c_goods`.goods_kind = 'jdgoods' or `sdb_b2c_goods`.goods_kind = 'jdbook') and marketable = 'true'";
        if (!empty($orderType)) {
    ...
    // 京东商品
    function _getSearchJdgdoodsCount($filter = array())
    {
        $sql = array();
        //获取京东商品屏蔽分类
        $jd_cat_id = $this->get_jd_cat_id();
        $sql[] = "SELECT";
        $sql[] = " jd_brand_id,b.brand_name as brand_name,price,buy_m_count";
        $sql[] = " from sdb_b2c_goods";
        $sql[] = " LEFT JOIN sdb_jdsale_brand AS b ON `sdb_b2c_goods`.jd_brand_id = b.brand_id";
        $sql[] = " where " . $this->jdgood_filter($filter);
        if ($jd_cat_id) {
            //为避免出现12,,23,24的情况，这里先处理下
            $jd_cat_id_str = implode(',', $jd_cat_id);
        ...
        foreach ($rows as $val) {
            $goodsDate['brandDate'][$val['jd_brand_id']] = $val['brand_name'];
            $price[] = $val['price'];
            $buy_m_count[] = $val['buy_m_count'];
        }
        $goodsDate['gcount'] = count($rows);
        $goodsDate['maxPrice'] = max($price);
        $goodsDate['minPrice'] = min($price);
        $goodsDate['subPrice'] = max($price) / 100;
        $goodsDate['maxBuyMonthCount'] = max($buy_m_count);
        $goodsDate['minBuyMonthCount'] = min($buy_m_count);
        $goodsDate['subBuyMonthCount'] = max($buy_m_count) / 100;
        return $goodsDate;
    }

    //京东搜索条件
    function jdgood_filter($filter)
    {
        if (isset($filter['loc']) && !empty($filter['loc'])) {
            if (is_string($filter['loc'])) {
                $str_where[] = " ((s.area  LIKE '%" . implode("%') or (s.area LIKE '%", explode(',', $filter['loc'])) . "%'))";
    ...
    private function get_jd_cat_id()
    {
        $objCat = app::get('jdsale')->model('goods_cat');
        $jd_cat_id = array();
        $cat_id_data = $objCat->getList('jd_cat_id', array('disabled' => 'true'));
        if (empty($cat_id_data)) {
            return false;
        }
        foreach ($cat_id_data as $k => $v) {
            $sub_cat = $objCat->getTreeListCat($v['jd_cat_id']);
            array_push($jd_cat_id, $v['jd_cat_id']);
            if (!empty($sub_cat)) {
                $jd_cat_id = array_merge($jd_cat_id, $sub_cat);
            }
        }
     ...
     */
    function modifier_jd_cat_id($row)
    {
        $name = app::get('jdsale')->model('goods_cat')->getRow('cat_name', array('jd_cat_id' => $row));
        return $name['cat_name'];
    }

    function modifier_price($row)
    {
        return '￥' . sprintf("%.2f", $row);
    }

    function modifier_agreed_price($row)
    {
        return '￥' . sprintf("%.2f", $row);
    }
    /**
