...
class b2c_mdl_order_items extends dbeav_model{
    ...
    public function fgetlist_csv( &$data,$filter,$offset,$exportType =1 ){
        $limit = 100;
        if(!$data['title']){
            $data['title'] = '"订单ID","商品编号","商品名称","购买量","原始单价","实际单价","总金额","成本价","分类","抽成比率","积分"';
        }
		$where = $this->_filter($filter);
		$where = str_replace('`sdb_b2c_order_items`', 'oi', $where);
        $sql = "
select oi.order_id,g.bn,g.name,oi.nums,g.mktprice,oi.price,oi.amount,oi.cost,gc.cat_name,gc.profit_point,oi.score from sdb_b2c_order_items oi
