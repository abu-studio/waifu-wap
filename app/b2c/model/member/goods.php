...
####根据会员ID获得该会员收藏的商品
    ...
    function get_favorite($member_id,$member_lv_id,$page=1,$num=10,$type){
        ...
        if(!$type){
            $aGid = $this->select()->columns(array('goods_id','vcat_id'))
            ...
            if($type == 'pmt'){
                ...
                $pmt_goods = $Mgoods->getList('goods_id,vcat_id',array('type'=>'fav','member_id'=>$member_id));
        ...
        if(is_array($agid)&&$agid){
                ...
                foreach($aGid as $val2){
                    $params['data'][$val2['goods_id']]['vcat_id'] =$val2['vcat_id'];
                }
