...
class b2c_mdl_goods_hfvirtual_cat extends dbeav_model{
    ...
    function getCatParentById($id){
		...
		$default_view = $conf_default_view?$conf_default_view:'index';
            ...
            foreach((array)$result as $cat_key=>$cat_value){
			    ...
			    $result[$cat_key]['link']=$this->gen_url(array('app'=>'b2c','ctl'=>'site_hfgallery','act'=>$default_view,'args'=>array(null,null,'0','','',$cat_value['cat_id'])));
            }
            return $result;
    }
    ...
    function getTreeList($pid=0, $listMark='all'){
        ...
        if($listMark == 'all'){
            ...
            $aCat = $this->db->select('SELECT virtual_cat_name, virtual_cat_id, o.parent_id AS pid, o.p_order,o.filter, o.cat_path, t.name as type_name, o.goods_type FROM sdb_b2c_goods_hfvirtual_cat o
            ...
            foreach($aCat as $k => $row){
                ...
                $aCat[$k]['url'] = $this->gen_url(array('app'=>'b2c','ctl'=>'site_hfgallery','view'=>$default_view,'args'=>array(implode(",",$filters['cat_id']),$oSearch->encode($filters))));
    ...
    function getTree($count=true,$node=null,$disabled=true){
        ...
        $sql = 'SELECT vCat.virtual_cat_id,vCat.disabled,vCat.url,vCat.virtual_cat_name,vCat.filter,vCat.child_count,vCat.p_order,vCat.cat_path,vCat.goods_type,'
            .'vCat.parent_id,vCat.child_count,vType.name as type_name from sdb_b2c_goods_hfvirtual_cat as vCat LEFT JOIN sdb_b2c_goods_type as vType ON vType.type_id=vCat.type_id where '
            .implode($where,' and ').($disabled?' AND vCat.disabled ="false" ':'').' ORDER BY vCat.p_order,vCat.virtual_cat_id';

        return $this->db->select('SELECT vCat.virtual_cat_id,vCat.disabled,vCat.url,vCat.virtual_cat_name,vCat.filter,vCat.child_count,vCat.p_order,vCat.goods_type,'
            .'vCat.cat_path,vCat.parent_id,vCat.child_count,vType.name as type_name from sdb_b2c_goods_hfvirtual_cat as vCat LEFT JOIN sdb_b2c_goods_type as vType ON vType.type_id=vCat.type_id where '
            .implode($where,' and ').($disabled?' AND vCat.disabled ="false" ':'').' ORDER BY vCat.p_order,vCat.virtual_cat_id');
    ...
    function getVCatByVid($cat_id=0){
        return $this->getList('virtual_cat_id,filter,parent_id,virtual_cat_name,child_count,url',array('virtual_cat_id'=>intval($cat_id)),0,-1,'cat_path,p_order ASC');
    }
    ...
    function toRemove($catid,&$msg=''){
        ...
        $this->db->exec('DELETE FROM sdb_b2c_goods_hfvirtual_cat WHERE virtual_cat_id='.intval($catid));
        $this->db->exec('UPDATE sdb_b2c_goods_hfvirtual_cat SET child_count = child_count-1 WHERE virtual_cat_id ='.intval($parent_id));
...
//        $goods_type = $data['goods_type'];
        ...
        if($data['virtual_cat_id']){
            ...
            if(parent::save($data)){
                ...
                $this->db->exec('UPDATE sdb_b2c_goods_hfvirtual_cat SET url = \''.app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_hfgallery','args'=>array(null,null,0,'','',$virtual_cat_id) ) ).'\' WHERE virtual_cat_id = '.$virtual_cat_id );
   ...
   function updatePath($cat_id,$cat_path){
          $result = $this->db->select('SELECT virtual_cat_id,cat_path FROM sdb_b2c_goods_hfvirtual_cat WHERE cat_path like \''.$cat_id.',%\' or parent_id='.$cat_id);
    ...
    function get_virtualcat_list($show_stable=false , $disabled = true,$catid=null){
        $kvName = $disabled?'goods_hfvirtualcat.data':'goods_hfvirtualcat.all.data';
    ...
    function getPath($cat_id,$method=null){
        ...
        $ret = array(array('type'=>'goodsCat','title'=>$row['cat_name'],'link'=> app::get('site')->router()->gen_url(array('app'=>'b2c', 'ctl'=>'site_hfgallery','act'=>'index','args'=>array(null,null,0,'','',$row['virtual_cat_id'])))  ));
        ...
        if($row['cat_path'] != ',' && $row['cat_path']){
            ...
            foreach($arr_rows as $row){
                ...
                array_unshift($ret,array('type'=>'goodsCat','title'=>$row['cat_name'],'link'=>app::get('site')->router()->gen_url(array('app'=>'b2c', 'ctl'=>'site_hfgallery','act'=>'index','args'=>array(null,null,0,'','',$row['virtual_cat_id'])))  ));
            }
        }
        $first_link = app::get('site')->router()->gen_url(array('app'=>'b2c', 'ctl'=>'site_hfgallery','act'=>'index'));
        array_unshift($ret,array('type'=>'goodsCat','title'=>app::get('site')->_('首页'),'link'=>$first_link ));
        return $ret;
    }
    ...
    function map($data,$sID=0,$preStr='',&$cat,&$step){
        ...
        if($data){
            ...
            if($data[$i]['goods_type'] == 'jdbook'){
                $isNormal = 'jdbook';
            }else if($data[$i]['goods_type'] == 'jdtype'){
                $isNormal = 'jdtype';
            }else{
                $isNormal = 'true';
            }
    ...
    function get_level_1_list($type='all'){
        $filter = array('parent_id' =>'0','disabled'=>'false');
        if (($type!='all')){
            $filter['goods_type']=$type;
        }

        $list = parent::getList('virtual_cat_id,virtual_cat_name,url',$filter,0,-1,'p_order ASC');
        return $list;
    }
