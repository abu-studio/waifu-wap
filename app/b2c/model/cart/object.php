...
class b2c_mdl_cart_objects extends dbeav_model{
    ...
    public function _setCookie($count=0, $number=0, $totalPrice=0) {
        ...
        if($count!==$_COOKIE['S[CART_COUNT]']){
			if(IS_DOMAIN){
				setcookie('S[CART_COUNT]',$count, null, '/',COOKIE_DOMAIN,1);
			}else{
        ...
        if($number!==$_COOKIE['S[CART_NUMBER]']){
			if(IS_DOMAIN){
				setcookie('S[CART_NUMBER]',$number, null, '/',COOKIE_DOMAIN,1);
			}else{
				setcookie('S[CART_NUMBER]',$number, null, '/');
			}
        }
        if(IS_DOMAIN){
				setcookie('S[CART_TOTAL_PRICE]', kernel::single('ectools_mdl_currency')->changer_odr($totalPrice), null, '/',COOKIE_DOMAIN,1);
			}else{
    ...
    public function _del_cookie() {
        ob_start();
		if(IS_DOMAIN){
				setCookie('S[CART_NUMBER]', 0, time()-3600, '/',COOKIE_DOMAIN,1);
				setCookie('S[CART_COUNT]', 0, time()-3600, '/',COOKIE_DOMAIN,1);
				setCookie('S[CART_TOTAL_PRICE]', 0, time()-3600, '/',COOKIE_DOMAIN,1);
			}else{
