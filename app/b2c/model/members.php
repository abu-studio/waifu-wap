...
class b2c_mdl_members extends dbeav_model{
    ...
    public function get_member_info( $member_id ) {
        ...
        if( !empty($member_sdf) ) {
            ...
            //添加企业中心用户
            if($member_sdf['seller'] == 'qiye'){
                $this->member_info['seller'] ='qiye';
            }
...
//                    }
        ...
        if($data['reg_type'] == 'mobile'){
            if(trim($data['contact']['phone']['mobile']) == ''){
                ...
                if(!preg_match('/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}|14[0-9]{1}[0-9]{8}$/', trim($data['contact']['phone']['mobile']))){
...
//                    }
    ...
    function check_mobile($mobile, &$message, $member_id=null){
        ...
        if(empty($mobile)){
            ...
            return false;
        }elseif(!preg_match("/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}|14[0-9]{1}[0-9]{8}$/", $mobile)){
...
//        }
    ...
    */
    ...
    public function fgetlist_csv( &$data,$filter,$offset,$exportType =1 ){
        ...
        if($filter['isSelectedAll'] == '_ALL_' &&  empty($filter['member_id'])){
            $tmp = $this->getList('member_id',$filter,$offset*$limit,$limit);
            $tmp_order = array();
            if(!empty($tmp)){
                foreach($tmp as $tmp_k=>$tmp_v){
                    $tmp_order[] = $tmp_v['member_id'];
                }
            }
            if(!empty($tmp_order)){
                $filter['member_id'] = $tmp_order;
            }
            $acc = app::get('pam')->model('account');
            if(!$list = $acc->getList('*',array('account_id|in'=>$filter['member_id'])))return false;
        }else{
            $acc = app::get('pam')->model('account');
            if(!$list = $acc->getList('*',array('account_id|in'=>$filter['member_id']),$offset*$limit,$limit))return false;
        }
