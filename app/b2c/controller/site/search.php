...
class b2c_ctl_site_search extends b2c_frontpage{
    ...
    function result(){
        $_POST = utils::filter_input($_POST);
        $_GET = utils::filter_input($_GET);
		$this->set_no_store();
        ...
        $tmpl=$_POST['tmpl'];
        array_push($args,$tmpl);
        $url_link=implode('&',$url);		
        //过滤xss攻击 start
		//$obj_filter = kernel::single('b2c_site_filter');
        //$args_tmp = $obj_filter->check_input($args);  
        $url_link = utils::_filter_crlf($url_link);
	...
	//卡券搜索方法
	 function result_card(){
        ...
        //$args_tmp = $obj_filter->check_input($args);  
		//print_r($args);exit;
		$this->sredirect(array('app'=>'b2c', 'ctl'=>'site_gallery', 'act'=>'card', 'args'=>$args),false,$url_link);
	...
	//京东搜索方法
	 function result_jdgoods(){
		...
		$this->sredirect(array('app'=>'jdsale', 'ctl'=>'site_gallery', 'act'=>'index', 'args'=>$args),false,$url_link);
    }
    ...
    function result_hf(){
        ...
        //$obj_filter = kernel::single('b2c_site_filter');
        //$args_tmp = $obj_filter->check_input($args);
        $this->sredirect(array('app'=>'b2c', 'ctl'=>'site_hfgallery', 'act'=>'index', 'args'=>$args),false,$url_link);
