    ...
    function toReply($comment_id=null){
        ...
        if(!$objComment->toValidate($aComment['object_type'], $aComment['goods_id'], $member_data, $message)){
            ...
            if($objComment->send($aData,$aComment['object_type'])){
				...
				$aData['goods_id'] = $aComment['type_id'];
				$goodsname = app::get('b2c')->model('goods')->getList('name',array('goods_id'=>$aComment['type_id']));
				$aData['goods_name'] = $goodsname[0]['name'];
                if($aComment['object_type'] == 'discuss') {
                    $comments->fireEvent('discussreply',$aData,$aComment['author_id']);
                }elseif($aComment['object_type']=='ask'){
                    $comments->fireEvent('gaskreply',$aData,$aComment['author_id']);
                }
