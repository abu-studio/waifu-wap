...
class b2c_ctl_site_hfgallery extends b2c_frontpage{
    ...
    public function index($cat_id='',$urlFilter=null,$orderBy=0,$tab=null,$page=1,$cat_type=null,$view='grid',$st='g',$tmpl=null,$vt='n') {
        @ini_set('memory_limit','1024M');
        $virCatObj = &$this->app->model('goods_hfvirtual_cat');
        $productCat = &$this->app->model('goods_cat');
        $objGoods = $this->app->model('goods');
        $isSubCat =$cat_id?true:false;
        if ($cat_type){
            $vcatList = $virCatObj->getList('cat_id,cat_path,virtual_cat_id,filter,virtual_cat_name as cat_name,goods_type,url',
                array('virtual_cat_id'=>intval($cat_type)));
            $vcat = $vcatList[0];
            ...
            if (!empty($_SESSION['sfsc']['vcat'])){
                //指定京东商品类型的虚拟分类下的实际商品分类
                $vcat = $_SESSION['sfsc']['vcat'];
                $cat_type = $vcat['virtual_cat_id'];
            }
        }
        if(in_array($vcat['goods_type'] , array('jdtype','jdbook'))){
            $this->jd_index($cat_id,$urlFilter,$orderBy,$tab,$page,$cat_type,$view,$st,$tmpl,$vcat,$isSubCat);
            return ;
        }
        ...
        if ($vcat['filter']){
            parse_str($vcat['filter'],$goodsFilter);
        }
        ...
        if( $cat_type ){
            ...
            /** 颗粒缓存商品虚拟分类 **/
            if(!cachemgr::get('goods_hfvirtual_cat_'.intval($vcatid), $vcat)){
                cachemgr::co_start();
                $vcat = $virCatObj->getList('cat_id,cat_path,virtual_cat_id,filter,virtual_cat_name as cat_name,goods_type,url',array('virtual_cat_id'=>intval($vcatid)));
                cachemgr::set('goods_hfvirtual_cat_'.intval($vcatid), $vcat, cachemgr::co_end());
            }
            ...
            if ($vcatFilters['brand_id']){
                $vcat_brand = $vcatFilters['brand_id'];
                unset($vcatFilters['brand_id']);
            }

            $old_cat_id = $cat_id;
            $old_urlFilter = $urlFilter;
            $urlFilter = $urlFilter?$urlFilter.'_'.$oSearch->encode($vcatFilters):$oSearch->encode($vcatFilters);
...
//self::_print($propargs);
        ...
        if(empty($brand_id[0])==false && $brand_id[0]!=='_ANY_' ){
            /** 颗粒缓存商品品牌 **/
            if(!cachemgr::get('hfgallery_brand_'.md5(serialize($brand_id)), $brand_options)){
                cachemgr::co_start();
                ...
                cachemgr::set('hfgallery_brand_'.md5(serialize($brand_id)), $brand_options, $co_end);
            }
        ...
        if($propargs['cat_id']){
            if ($isSubCat){
                ...
                $propargs['cat_id'][0]=$old_cat_id;
            }
           $cat_id=implode(",",$propargs['cat_id']);
        }
        //echo '2<pre>';var_dump($propargs);
        //如果设置了分类。
        if($cat_id){
...
//$pageLimit=5;
        ...
        $global_runtime_path = "";
        if($isSubCat){
            if(!cachemgr::get('global_runtime_path_hfcat_id'.md5(serialize($cat_id[0])), $global_runtime_path)){
                cachemgr::co_start();
                $global_runtime_path = $productCat->getHfPath($cat_id[0],'',$vcat['url']);
                $co_end=cachemgr::co_end();
                $co_end['expires']=time()+300;
                cachemgr::set('global_runtime_path_hfcat_id'.md5(serialize($cat_id[0])), $global_runtime_path, $co_end);
            }
        }
        //echo '3<pre>';var_dump($global_runtime_path);die();
        ...
        if( count($global_runtime_path) < 2 ){
            $global_runtime_path = array(
                array('type'=>'goodsCat',
                      ...
                      'link'=>$this->gen_url( array('app'=>'b2c','ctl'=>'site_hfgallery')).'?scontent='.$search_key)
            );
        }
        ...
        //如果存在分类则取出分类信息。
        if ($cat_id[0]){
            if(!cachemgr::get('hf_goods_cat_'.$cat_id[0], $this->cat_result)){
                cachemgr::co_start();
                $this->cat_result = $productCat->getList('cat_name,gallery_setting,type_id',array('cat_id|in'=>$cat_id),0,1);
                cachemgr::set('hf_goods_cat_'.$cat_id[0], $this->cat_result, cachemgr::co_end());
            }
        ...
        //取出分类。
        $cache_key='hf_gallery_cat_'.md5(serialize(array($cat_id,$view,$type_filter['type_id'],$cat_type)));
        //echo $cache_key,'<br>';
        ...
        $cat['tpl']='/site/hfgallery/type/grid.html';//rewrite here；
       ...
       foreach($path as $p){
        ...
        if($res === false){
            ...
            if (empty($filter['cat_id'][0])){
                $filter['cat_id'] = $goodsFilter['cat_id'];
            }
            if (!empty($vcat_brand) && empty($filter['brand_id'])){
                $filter['brand_id'] = $vcat_brand;
            }
        ...
        if (is_array($cat['brand'])){
            ...
            if($goodsFilter['brand_id'][0]!=='_ANY_'){
                foreach($brand['options'] as $k=>$v){
                    if (!in_array($k,$goodsFilter['brand_id'])){
                        unset($brand['options'][$k]);
                    }
                }
            }
            $brand['value'] = null ;
        ...
        if(isset($cat_id[0]) && $cat_id[0] && count($cat_id)==1){
            if(!cachemgr::get('hfgallery_goods_cat_parent_id'.md5(serialize($cat_id)), $cat_list)){
                cachemgr::co_start();
                $cat_list=$productCat->getList('cat_name,cat_id,parent_id,cat_path,child_count',
                    array('parent_id|in'=>$cat_id,'hidden'=>'false'),0,-1,'p_order');
                $co_end=cachemgr::co_end();
                $co_end['expires']=time()+300;
                cachemgr::set('hfgallery_goods_cat_parent_id'.md5(serialize($cat_id)), $cat_list, $co_end);
            }

            if ($goodsFilter['cat_id'][0]!=='_ANY_' && !in_array($cat_id[0],$goodsFilter['cat_id'])){

                foreach($cat_list as $k=>$v){
                    if (!in_array($v['cat_id'],$goodsFilter['cat_id'])){
                        unset($cat_list[$k]) ;
                    }
                }
            }
            ...
            if(!cachemgr::get('hfgallery_goods_cat_id'.md5(serialize(array_keys($catList))), $cat_list)){
                cachemgr::co_start();
                $cat_list=$productCat->getList('cat_name,cat_id,parent_id,cat_path,child_count',
                    array('cat_id|in'=>array_keys($catList),'hidden'=>'false'),0,-1);
                $co_end=cachemgr::co_end();
                $co_end['expires']=time()+300;
                cachemgr::set('hfgallery_goods_cat_id'.md5(serialize(array_keys($catList))), $cat_list, $co_end);
            }
            $plist=utils::array_change_key($cat_list,'parent_id', 1);
        }
        ...
        if(empty($plist)==false){
            ...
            foreach($plist as $pid=>$cl){
                ...
                if($has_select_cat){
                    ...
                    if($pid==0){//顶级分类
                        ...
                        if(!in_array($pid,$this->arr_cat)){
                            if(!cachemgr::get('hfgallery_goods_cat_id'.intval($pid), $row)){
                                cachemgr::co_start();
                                ...
                                $co_end=cachemgr::co_end();
                                $co_end['expires']=time()+300;
                                cachemgr::set('hfgallery_goods_cat_id'.intval($pid), $row, $co_end);
                            }
                ...
                foreach($cl as $key=>$c){
                    if($catList[$c['cat_id']]){
                        $item['options'][$c['cat_id']]['name']=$c['cat_name'];
                        $item['options'][$c['cat_id']]['url']=app::get('site')->router()->gen_url(
                            array('app'=>'b2c', 'ctl'=>'site_hfgallery','act'=>'index','args'=>array($c['cat_id'])));
                    }
                }
                if($item['options']){
                    $catchild[$pid]=$item;
        ...
        if((!is_array($cat_id) && $cat_id) || $cat_id[0] || $cat_type){//为什么再选择一遍 huoxh
            ...
            $_pfilter['goods_kind'] = array('virtual','entity');
        ...
        $this->pagedata['pager'] = array(
            ...
            'formAction'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_search','full'=>1,'act'=>'result_hf')),
            'args'=>array( 'cat_id'=>($cat_type?$old_cat_id:implode(',',$cat_id) ),'filter'=>$filter,'orderby'=>$orderBy,
                           'tab'=>$tab,'cat_type'=>$cat_type,'view'=>$view,'st'=>$st,'tmpl'=>$tmpl,'store_id'=>$_GET['sid'],'loc'=>$_GET['loc']),
            'total'=>ceil($count/$pageLimit),
            'link'=>  $this->gen_url(array('app'=>'b2c', 'ctl'=>'site_hfgallery','full'=>1,'act'=>'index',
                                           'args'=>array( ($cat_type?$old_cat_id:implode(',',$cat_id) ),urlencode($p['str']),$orderBy,$tab,($tmp=time()),$cat_type,$view,$st,$tmpl))),//.'?scontent='.urlencode($p['str']),
            'token'=>$tmp);
        //echo '<pre>';var_dump( $this->pagedata['pager']);die();
        //如果当前页大于总页数。则报错。
        ...
        //如果选择了品牌则列表中不显示品牌
        if(empty($propargs['brand_id']) || empty($propargs['brand_id'][0]) ){
        ...
        if($tmpl){
            $this->set_tmpl_file('hfgallery-'.$tmpl.'.html');
            $this->pagedata['tmpl'] = true;
        }else{
            $this->set_tmpl('hfgallery');//设定模板类型为列表页。
        }
        ...
        if(!empty($seo_info['seo_info']['seo_title']) || !empty($seo_info['seo_info']['seo_keywords']) || !empty($seo_info['seo_info']['seo_description'])){
            ...
            $this->setSeo('site_hfgallery','index',$this->prepareSeoData($this->pagedata));
        }
		...
		//print_r($this->pagedata['pager']);die;
        $this->page('site/hfgallery/index.html');

    }


    private function jd_index($cat_id='',$urlFilter=null,$orderBy=0,$tab=null,$page=1,$cat_type=null,$view='grid',$st='g',$tmpl=null,$vcat=array(),$isSubCat=false){
        $virCatObj = &$this->app->model('goods_hfvirtual_cat');
        ...
        $path =array();
        $cat=array();
        $searchtools = app::get('jdsale')->model('search');
        $propargs = $searchtools->decode($urlFilter,$path,$cat);
        ...
        if($brand_id){
            $goodsFilter['jd_brand_id']=$brand_id;
        }

        //虚拟分类
        if ($cat_id){
            $catDte = $cat_object->getTreeListCat($cat_id);
            $selector_cat = $cat_object->getHfGalleryListCat($goodsFilter,$cat_id);
            $catDte[] = $cat_id;
            $filter['jd_cat_id|in'] = $catDte;
            if ($goodsFilter['jd_brand_id']) {
                $vcat_brand = $goodsFilter['jd_brand_id'];
            }
        }else{
            $vcatFilters = $virCatObj->_mkFilter($vcat['filter']);
            $vcatFilters = $virCatObj->getFilter($vcatFilters);
            ...
            $selector_cat = $cat_object->getHfGalleryListCat($goodsFilter);
        }

        if(!empty($search_key)){
            $GLOBALS['runtime']['search_key'] = $search_key;
            $filter['name'][0]=$search_key;
        }
        ...
        $global_runtime_path = "";
        if($cat_id){
            if(!cachemgr::get('global_runtime_path_hfjdcat_id'.md5(serialize($cat_id[0])), $global_runtime_path)){
                cachemgr::co_start();
                $global_runtime_path = $cat_object->getHfPath($cat_id[0],'',$vcat['url']);
                $co_end=cachemgr::co_end();
                $co_end['expires']=time()+300;
                cachemgr::set('global_runtime_path_hfjdcat_id'.md5(serialize($cat_id[0])), $global_runtime_path, $co_end);
            }
        }
        // ajx 以下是为了当无分类和搜索条件时 显示所有商品
        if( count($global_runtime_path) < 2 ){
        ...
        //var_dump($urlFilter);die();
        $args = array($cat_id[0],urlencode($urlFilter),$orderBy,$tab,$page,$cat_type,$default_view,$default_searchType,$tmpl);
        ...
        if (!empty($vcat_brand) && empty($filter['jd_brand_id'])){
            $filter['jd_brand_id'] = $vcat_brand;
        }
        ...
        //echo '<pre>';var_dump($selector_brand);die();
        //如果选择了品牌，怎前台不显示品牌
        if($brand_id){
            $selector_brand = '';

            $bfilter['brand_id']=$brand_id;
            $arr_brand=app::get('jdsale')->model('brand')->getList('brand_id,brand_name',$bfilter,0,-1);
            foreach($arr_brand as $bv){
                $brand_options[$bv['brand_id']]=$bv['brand_name'];
            }
            $GLOBALS['runtime']['brand']['name']='品牌';
        ...
        //var_dump($selector_brand);die();

        $count = $total['gcount'];
        $this-> pagedata['products'] = $aProduct;
        $this->pagedata['_PDT_LST_TPL'] = '/site/hfgallery/type/grid.html';

        $GLOBALS['runtime']['cat'] = $cat_id;
        $GLOBALS['runtime']['goods_count'] = $count;
        $GLOBALS['runtime']['st']=$st;
        $GLOBALS['runtime']['vt']='j';
        ...
        $this->pagedata['pager'] = array(
            ...
            'urlFilter'=>$urlFilter,
            'formAction'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_search','full'=>1,'act'=>'result_hf')),
            'args'=>array('cat_id'=>($isSubCat?$cat_id[0]:''),'filter'=>$filter,'orderby'=>$orderBy,'tab'=>$tab,
                          'cat_type'=>($isSubCat?'':$cat_type),'view'=>$view,'st'=>$st,'tmpl'=>$tmpl,'store_id'=>$_GET['sid'],'loc'=>$_GET['loc']),
            'total'=>ceil($count/$pageLimit),
            'link'=>  $this->gen_url(array('app'=>'b2c', 'ctl'=>'site_hfgallery','full'=>1,'act'=>'index',
                                           'args'=>array($isSubCat?$cat_id[0]:'',urlencode($path[0]['str']),$orderBy,$tab,($tmp=time()),$isSubCat?'':$cat_type,$view,$st,$tmpl))),
            'token'=>$tmp
        );
        //echo '<pre>';var_dump( $this->pagedata['pager']);die();
        //如果当前页大于总页数。则报错。
        ...
        $this->set_tmpl("hfgallery");
        $this->page('site/hfgallery/jd_index.html');
    }
    ...
    */
	...
	//卡券搜索
	public function card($cat_id='',$urlFilter=null,$orderBy=0,$tab=null,$page=1,$cat_type=null,$view='grid',$st='g') {
...
//echo '</pre>';
        ...
        if($res === false){
            ...
            if($view=='index'){
                ...
                //print_r($tmp_filter);
                $temp=0;                   
                $temp_product =$objGoods->getSearchCards('sdb_b2c_goods.goods_id,sdb_b2c_goods.store_id,sdb_b2c_goods.cat_id,sdb_b2c_goods.brand_id',$tmp_filter,0,-1,$torderby,$arr_cat_id,$temp);
                $all_goods_id=array_map('current',$temp_product);
                $temp_store_array=utils::array_change_key($temp_product,'store_id', 1);
                $no_indexCat=utils::array_change_key($temp_product,'cat_id', 1);
                $no_indexBrand=utils::array_change_key($temp_product,'brand_id', 1);
                ...
                if(empty($orderby)){   
                    $torderby=' dorder desc';
                    $temp=0;
                    $aProduct =$objGoods->getSearchCards($this->getGoodsCol(),$tmp_filter,0,-1,$torderby,$arr_cat_id,$temp);
                }else{
                    $aProduct=$objGoods->getList_1($this->getGoodsCol(),$tmp_filter,0,-1,$orderby);
                }*/
            }else{
                $ttemp_product=$objGoods->getSearchCards('sdb_b2c_goods.goods_id,sdb_b2c_goods.store_id,sdb_b2c_goods.cat_id,sdb_b2c_goods.brand_id',$tmp_filter,0,-1,$orderby,$arr_cat_id,$temp);//$objGoods->getList('store_id,cat_id',$tmp_filter,0,-1);
                $all_goods_id=array_map('current',$ttemp_product);
                $ttemp_store_array=utils::array_change_key($ttemp_product,'store_id', 1);
                $no_indexCat=utils::array_change_key($ttemp_product,'cat_id', 1);
                $no_indexBrand=utils::array_change_key($ttemp_product,'brand_id', 1);
                ...
                //}
                $temp=0;                    
                $aProduct =$objGoods->getSearchCards($this->getGoodsCol(),$tmp_filter,$pageLimit*($page-1),$pageLimit,$orderby,$arr_cat_id,$temp);
                $count=$temp;
            }  
        }
        ...
        $this->pagedata['pager'] = array(
			...
			'scontent'=>$p['str'],
            'urlFilter'=>$cat_type?$old_urlFilter:urlencode($urlFilter),
            'formAction'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_search','full'=>1,'act'=>'result_card')),
            ...
            'total'=>ceil($count/$pageLimit),
            'link'=>  $this->gen_url(array('app'=>'b2c', 'ctl'=>'site_gallery','full'=>1,'act'=>'card','args'=>array( ($cat_type?$old_cat_id:implode(',',$cat_id) ),urlencode($p['str']),$orderBy,$tab,($tmp=time()),$cat_type,$view,$st))),//.'?scontent='.urlencode($p['str']),
            'token'=>$tmp);
        ...
        if(is_array($aProduct) && count($aProduct) > 0){
            ...
            foreach ($aProduct as $key=>&$val) {
                ...
                */
                ...
                $aProduct[$key]['goods_url']=$promotion->gen_site_url($val['goods_id'],$val['promotion_id'],$val['act_type']);
				$aProduct[$key]['goods_url']=str_replace('product-','carddetail-',$aProduct[$key]['goods_url']);
                // add storeINfo and sepcl;
				$val['store_name']=$store_info[$val['store_id']]['store_name'];
				$this->specFomart(&$val);
       ...
       /* if($searchSelect){
        ...
        if(!empty($seo_info['seo_info']['seo_title']) || !empty($seo_info['seo_info']['seo_keywords']) || !empty($seo_info['seo_info']['seo_description'])){
            ...
            $this->setSeo('site_gallery','card',$this->prepareSeoData($this->pagedata));
        }
        ...
        $this->page('site/gallery/card.html');
