...
class b2c_ctl_site_cart extends b2c_frontpage
{
    ...
    public function add($type = 'goods')
    {
        ...
        //购物车中是否有卡券商品(实体券还是电子码)  2015/7/10
            ...
            if (isset($aData[1]) && $aData[1] == 'quick') {
                ...
                if ($aData[0] == 'coupon') {
                    if (!$data['response_type']) {
                        ...
                        echo json_encode($arr_json_data);
                        exit;
                    }
                } else {
    ...
    private function update_cart(&$msg = '', $type = 'all')
    {
        ...
        foreach ($aCart as $row) {
            if (isset($aParams['modify_quantity'][$row['obj_ident']])) {
                if (isset($aParams['modify_quantity'][$row['obj_ident']]['adjunct'])) {
                    $adjunctUpdate = true;
                    $key = key($aParams['modify_quantity'][$row['obj_ident']]['adjunct'][0]);
                    $allowednum = $row['params']['adjunct'][0]['adjunct'][$key];
                }
                $update_row = $row;
                break;
            }
                ...
                if (is_array($_flag) && isset($_flag['status']) && isset($_flag['msg'])) {
                    ...
                    if (!$_flag) {
                        $error_json = array(
                            ...
                            'allowednum' => $adjunctUpdate ? $allowednum : $update_row['quantity'],
                        );
                        $this->pagedata = $error_json;
                        $this->page($view);
                    } else {
    ...
    public function checkout($isfastbuy = 0)
    {
            ...
            if ($v['store_id'] > 0 && ($v['store_id'] == $jdshopId || $v['store_id'] == $jdbookshopId)) {
                foreach ($v['obj_items']['products'] as $productRow) {
                    ...
                    if ($agreedPrice > $salePrice) {
                        $this->splash('failed', $this->gen_url(array('app' => 'b2c', 'act' => 'index', 'ctl' => 'site_cart')), '"' . $productRow['name'] . '"<br />' . app::get('b2c')->_('该商品京东价格有调整,暂不能购买'));
                        return;
                    }
                }
            }
        ...
        if (count($_SESSION['cart_select_data']['select_products']) == 0) {
            //$this->splash('failed', $this->gen_url( array('app'=>'b2c','act'=>'index','ctl'=>'site_cart') ) , app::get('b2c')->_('请选择商品！'));
            $this->end(false, app::get('b2c')->_('请选择商品！'), $this->gen_url(array('app' => 'b2c', 'act' => 'index', 'ctl' => 'site_cart')));
        }
        // 购物车是否为空
    ...
    public function get_default_info()
    {
        if ($_POST['_type']) {
            ...
            switch ($_POST['_type']) {
                case 'get_addr':
                    if ($arr_addon) {
                        echo $arr_addon['def_addr']['addr_id'] ? $arr_addon['def_addr']['addr_id'] : '0';
                        exit;
                    } else {
                        echo '';
                        exit;
                ...
                case 'get_payment':
                    if ($arr_addon) {
                        echo $arr_addon['payment']['pay_app_id'] ? $arr_addon['payment']['pay_app_id'] : '0';
                        exit;
                    } else {
                        echo '0';
                        exit;
                ...
                case 'get_default_shipping':
                    if ($arr_addon) {
                        echo $arr_addon['delivery']['shipping_id'] ? $arr_addon['delivery']['shipping_id'] : '0';
                        exit;
                    } else {
                        echo '0';
                        exit;
        ...
        if ($arr_delivery) {
            ...
            if ($_POST['member_id']) {
                ...
                if ($arr_delivery) {
                    ...
                    if (IS_DOMAIN) {
                        setcookie('purchase[addr][usable]', $seKey, 0, kernel::base_url() . '/', COOKIE_DOMAIN, 1);
                    } else {
                ...
                if (IS_DOMAIN) {
                    setcookie('purchase[addon]', serialize($arr_addon), 0, kernel::base_url() . '/', COOKIE_DOMAIN, 1);
                } else {
            ...
            $this->pagedata['site_checkout_receivermore_open'] = $this->app->getConf('site.checkout.receivermore.open');
            echo $obj_addr->get_def_addr($this, $arr_delivery, $is_insert);
            exit;
        }
    }
    ...
    public function checkout_goodsbody()
    {
        ...
        //lpc 在这判断用户默认地址是否填写完整,长度为4即为完整 mainland:内蒙古/包头市/固阳县/城区:38288
        $areaData = explode('/', $_POST['area']);
        $_POST['area'] = $areaData[0];
        $addr_id = $areaData[1];
        $obj_member_addrs = $this->app->model('member_addrs');
        $addrData = $obj_member_addrs->getRow('area', array('addr_id' => $addr_id));
        $areaCount = count(explode('/', $addrData['area']));
        if ($areaCount < 3) {
            $this->pagedata['areaerro'] = 1;
        }
    ...
    public function purchase_shipping()
    {
        ...
        if (!isset($_POST['shipping_id']) || !$_POST['shipping_id']) {
            echo app::get('b2c')->_('配送方式的id不能为空！');
            exit;
        }
        ...
        if (IS_DOMAIN) {
            setcookie('purchase[shipping]', serialize($arr_shipping), 0, kernel::base_url() . '/', COOKIE_DOMAIN, 1);
        } else {
            setcookie('purchase[shipping]', serialize($arr_shipping), 0, kernel::base_url() . '/');
        }
        echo app::get('b2c')->_('配送方式保存成功！');
        exit;
    }
    ...
    public function purchase_payment()
    {
        ...
        if (!isset($_POST['pay_app_id']) || !$_POST['pay_app_id']) {
            echo app::get('b2c')->_('支付方式的id不能为空！');
            exit;
        }
        ...
        if (IS_DOMAIN) {
            setcookie('purchase[payment]', serialize($arr_payment), 0, kernel::base_url() . '/', COOKIE_DOMAIN, 1);
        } else {
            setcookie('purchase[payment]', serialize($arr_payment), 0, kernel::base_url() . '/');
        }
        echo app::get('b2c')->_('支付方式保存成功！');
        exit;
    }
    ...
    public function getAddr()
    {
        ...
        header("Expires: Mon, 26 Jul 1997 05:00:00 GMT");// 强制查询etag
        header('Progma: no-cache');
        echo $obj_addr->get_receive_addr($this, $addr_id, $arr_member['member_id']);
        exit;
    }
    ...
    private function _cart_main($msg = '', $json_type = 'all')
    {
        ...
        //增加店铺id（store_id）用于显示优惠规则时区分当前修改的商品属于哪个店铺
        if ($_POST['modify_quantity']) {
            $goods_id = explode('_', key($_POST['modify_quantity']));
            if ($goods_id[0] == 'goods') {
                $goods = kernel::single('b2c_mdl_goods')->dump($goods_id[1], 'store_id');
                $arr_json_data['store_id'] = $goods['store_id'];
            }
        }
    ...
    */
            ...
            if (isset($row['params']['adjunct']) && is_array($row['params']['adjunct'])) {
                foreach ($row['params']['adjunct'] as $adjunct) {
                    ...
                    foreach ($adjunct['adjunct'] as $p_id => $p_quantity) {
                        if (!isset($temp['adjunct'][$adjunct['group_id']][$p_id])) {
                            $flag = false;
                            continue;
                        }
    ...
    public function save_new_addr()
    {
        ...
        if (!$arrMember['member_id']) {
            ...
            if ($nums >= 6) {
                echo json_encode(array('status' => 'many', 'msg' => '您最多可添加6个收货地址'));
                exit;
            }
            $member_id = $arrMember['member_id'];
            ...
            if ($check[0]) {
                echo json_encode(array('status' => 'like', 'msg' => '相同的收货地址'));
                exit;
            }
