...
class b2c_ctl_site_passport extends b2c_frontpage{
    ...
    function gen_login_form($company){
    ...
    function postSfscLogin($url=null){
        $url = base64_decode($url);
        $mini = $_GET['mini'];
        $member_id = $_SESSION['account'][pam_account::get_account_type($this->app->app_id)];
        unset($_SESSION['next_page']);
    ...
    function namecheck(){
        ...
        if(strlen($name) > 0){
            if(!preg_match('/^[a-zA-Z0-9_]*$/u', $name)){	//后台取消纯数字不可进行会员添加
	 ...
	 $login_name = trim($_POST['login_name']);
        if($login_name != ''){
            ...
            if(null != $member_info){
                ...
                //登陆时验证 外服用户名
                $sfsc_data = SFSC_HttpClient::doCheckUname($login_name);
                if($sfsc_data['RESULT_CODE'] == "10001"){
                    echo '<span class="font-red">'.app::get('b2c')->_('用户名不存在').'</span>';
                    exit;
                }
			}
