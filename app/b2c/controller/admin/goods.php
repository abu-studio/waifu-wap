...
class b2c_ctl_admin_goods extends desktop_controller{
    ...
    function index(){
		...
		if($this->has_permission('editmktprice') &&$_GET['view']==5){
			$group[] = array('label'=>app::get('b2c')->_('审核成本价'),'icon'=>'download.gif','submit'=>'index.php?app=b2c&ctl=admin_goods&act=prices_allow_all&type=cost','target'=>'dialog');
		}
        ...
        */
        ...
        $actions_base['use_view_tab'] = true;
		 $actions_base['base_filter'] = array('goods_kind'=>array('virtual','entity'));
   ...
   function _views(){
        ...
        if($_GET['view'] || !$_GET['filter']){
            //已下架商品
            $filter = array('marketable'=>'false','goods_type'=>'normal','goods_kind'=>array('virtual','entity'));
            ...
            $goods_id_arr = $mdl_products->db->select("select DISTINCT goods_id from sdb_b2c_products where goods_type='normal' and store='0' and goods_kind in ('virtual','entity')");
            ...
            //同缺货商品的情况 
            $goods_id_arr = $mdl_products->db->select("select DISTINCT goods_id from sdb_b2c_products where goods_type='normal' and store <='".$alert_num."' and goods_kind in ('virtual','entity')");
            if(is_array($goods_id_arr)){
                foreach($goods_id_arr as $gk=>$gv){
                    $fgoods['goods_id'][] = $gv['goods_id'];
                }
            ...
            unset($fgoods);
            $goods_id_arr = $mdl_products->db->select("select DISTINCT a.goods_id from sdb_b2c_goods_marketable_application as a join sdb_b2c_goods as g on a.goods_id=g.goods_id where a.status='0' where  g.goods_kind in ('virtual','entity')");
            if(is_array($goods_id_arr)){
                foreach($goods_id_arr as $gk=>$gv){
                    $fgoods['goods_id'][] = $gv['goods_id'];
                }
            ...
            if(count($fgoods['goods_id']) >0){
                $sub_menu[4] = array('label'=>app::get('b2c')->_('违规商品'),'optional'=>true,'filter'=>$fgoods,'addon'=>count($fgoods['goods_id']),'href'=>'index.php?app=b2c&ctl=admin_goods&act=index&view=4&view_from=dashboard');
            }
			if($this->has_permission('editmktprice')){
				$costprice_filter=array('goods_application'=>array('0','1'),'goods_kind'=>array('virtual','entity'));
				$costprice_count=$mdl_goods->count($costprice_filter);
				$sub_menu[5] = array('label'=>app::get('b2c')->_('审核成本价'),'optional'=>true,'filter'=>$costprice_filter,'addon'=>$costprice_count,'href'=>'index.php?app=b2c&ctl=admin_goods&act=index&view=5&view_from=dashboard');
			}
			if($this->has_permission('editmarketable')||$this->has_permission('batcheditmarketable')){
				$marketable_filter=array('goods_application'=>'4','marketable'=>'false','goods_kind'=>array('virtual','entity'));
				$marketable_count=$mdl_goods->count($marketable_filter);
				$sub_menu[6] = array('label'=>app::get('b2c')->_('待上架'),'optional'=>true,'filter'=>$marketable_filter,'addon'=>$marketable_count,'href'=>'index.php?app=b2c&ctl=admin_goods&act=index&view=6&view_from=dashboard');
			}
			//上架商品
			$markettrue_filter=array('marketable'=>'true','goods_type'=>'normal','goods_kind'=>array('virtual','entity'));
			$markettrue_count = $mdl_goods->count($markettrue_filter);
			if($markettrue_count>0){
				$sub_menu[7] = array('label'=>app::get('b2c')->_('已上架'),'optional'=>true,'filter'=>$markettrue_filter,'addon'=>$markettrue_count,'href'=>'index.php?app=b2c&ctl=admin_goods&act=index&view=7&view_from=dashboard');
			}
			
           if($sub_menu){
                $sub_menu[0] = array('label'=>app::get('b2c')->_('全部'),'optional'=>true,'filter'=>'','addon'=>$mdl_goods->count(array('goods_kind'=>array('virtual','entity'))));
            }
...
//                    }
	...
	//价格审核
	function prices_allow(){
		if($_POST['prices_allow']=='prices_allow'){
			$this->begin();
			$status=array('1'=>'审核不通过','4'=>'审核通过');
			if(in_array($_POST['status'],array('1','4'))){
				$data=array('goods_application'=>$_POST['status']);
				$objGoods = $this->app->model('goods');
				$objGoods->update($data,array('goods_id'=>$_POST['goods_id']));
				if($obj_operatorlogs = kernel::service('operatorlog')){
					if(method_exists($obj_operatorlogs,'inlogs')) {
						$memo = '审核成本价:审核状态('.$status[$_POST['status']].'),商品ID('.$_POST['goods_id'].'),商品名称('.$_POST['goods_name'].')商品编号('.$_POST['goods_bn'].')操作员备注('.$_POST['memo'].')';
						$obj_operatorlogs->inlogs($memo, '审核成本价', 'goods');
					}
				}
			}
			$this->end(true,app::get('b2c')->_('审核成功'));	
			...
			if($_GET['type']=='cost'){
				$sql="SELECT g.name,g.bn as goods_bn, p.cost ,p.bn ,p.spec_info from ".DB_PREFIX."b2c_products as p LEFT JOIN ".DB_PREFIX."b2c_goods as g on p.goods_id=g.goods_id where g.goods_application in ('0','1') and  p.goods_id='".$_GET['goods_id']."'";
			}
			$products=kernel::database()->select($sql);
			
			$this->pagedata['products']=$products;
			$this->pagedata['goods_name']=$products[0]['name'];
			$this->pagedata['goods_bn']=$products[0]['goods_bn'];
			$this->pagedata['goods_id']=$_GET['goods_id'];
			$this->singlepage('admin/goods/detail/allow.html');
		}
	}
	//批量审核成本价
	function prices_allow_all(){
		$objGoods = $this->app->model('goods');
		if($_POST['prices_allow_all']=='prices_allow_all'){
			$this->begin();
			$status=array('1'=>'审核不通过','4'=>'审核通过');
			...
			if(!$filter['goods_id']){
				...
				foreach($goods_id as $key=>$value){
					$tmp_id['goods_id'][]=$value['goods_id'];
				}
				$goods_id=array();
				$goods_id=$tmp_id;
			}else{
			...
			$goods_id_str=implode(',',$goods_id['goods_id']);
			if(in_array($_POST['status'],array('1','4'))){
				$data=array('goods_application'=>$_POST['status']);
				
				$objGoods->update($data,$filter);
				if($obj_operatorlogs = kernel::service('operatorlog')){
					if(method_exists($obj_operatorlogs,'inlogs')) {
						$memo = '批量审核成本价:审核状态('.$status[$_POST['status']].'),商品ID('.$goods_id_str.')操作员备注('.$_POST['memo'].')';
						$obj_operatorlogs->inlogs($memo, '审核成本价', 'goods');
					}
				}
			}
			...
			if($_POST['goods_id']){
				...
				$count=$objGoods->count(array('goods_application'=>array('0','1'),'goods_kind'=>array('virtual','entity')));
				$_POST=array('goods_application'=>array('0','1'),'goods_kind'=>array('virtual','entity'));
			}
			$this->pagedata['count']=$count;
			$this->pagedata['filter']=json_encode($_POST);
			$this->display('admin/goods/detail/allow_all.html');
		}
	}
