...
class b2c_ctl_admin_order extends desktop_controller{
    ...
    public function __construct($app)
    {
        ...
        //lpc 加半年全部订单
        $ym = date('Y-m' , time());
        $date = strtotime('-3 month' , strtotime($ym));
        ...
        $this->mfilter = array(
            ...
            '_DTIME_'=>
                array(
                    'H'=>array('createtime_from'=>'00','createtime_to'=>date('H',time())),
                    'M'=>array('createtime_from'=>'00','createtime_to'=>date('i',time()))
                )
        );
    ...
    public function index(){
		...
		//获取京东自营店铺
		$shopId = app::get('site')->getConf('jdsale.shopId') ? app::get('site')->getConf('jdsale.shopId'): '';
		array_push($store_id_arr,$shopId);
        ...
        $filter = array(
            'order_refer'=>'local',
            'disabled'=>'false',
            'store_id|notin'=>$store_id_arr,
            'order_type|notin'=>array('largeCustomer')
        );
        if (empty($_GET['view'])) 
            $filter = array_merge($filter,$this->mfilter);

        $this->index_filter = array(
            'order_refer'=>'local',
            'store_id|notin'=>$store_id_arr,
        );
        ...
        $this->finder('b2c_mdl_orders',array(
            ...
            'base_filter'=>$filter,//array('order_refer'=>'local','disabled'=>'false','store_id|notin'=>$store_id_arr,'order_type|notin'=>array('largeCustomer')),
    ...
    public function _views(){
        ...
        $filter = array(
            'disabled'=>'false',
            'order_type|notin'=>array('largeCustomer'),
            'order_kind|notin'=>array('b2c_card')
        );
        $filter = array_merge($filter,$this->mfilter);

        $mdl_order = $this->app->model('orders');
        $sub_menu = array(
            0=>array('label'=>app::get('b2c')->_('近3月全部订单'),'optional'=>false,'filter'=>array_merge($this->index_filter,$filter)),
            1=>array('label'=>app::get('b2c')->_('全部'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            2=>array('label'=>app::get('b2c')->_('未处理'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('pay_status'=>array('0'),'ship_status'=>array('0'),'status'=>'active','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            3=>array('label'=>app::get('b2c')->_('已付款待发货'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('pay_status'=>array('1','2','3'),'ship_status'=>array('0','2'),'status'=>'active','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            4=>array('label'=>app::get('b2c')->_('已发货'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('ship_status'=>array('1'),'status'=>'active','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            5=>array('label'=>app::get('b2c')->_('已完成'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('status'=>'finish','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            6=>array('label'=>app::get('b2c')->_('已退款'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('pay_status'=>array('4','5'),'status'=>'active','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            7=>array('label'=>app::get('b2c')->_('已退货'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('ship_status'=>array('3','4'),'status'=>'active','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
            8=>array('label'=>app::get('b2c')->_('已作废'),'optional'=>false,'filter'=>array_merge($this->index_filter,array('status'=>'dead','disabled'=>'false','order_type|notin'=>array('largeCustomer'),'order_kind|notin'=>array('b2c_card')))),
        );
        //新留言订单
        ...
        $sub_menu[9] = array('label'=>app::get('b2c')->_('新留言订单'),'optional'=>true,'filter'=>$forders,'addon'=>count($forders['order_id']),'href'=>'index.php?app=b2c&ctl=admin_order&act=index&view=8&view_from=dashboard');
        ...
        $sub_menu[10] = array('label'=>app::get('b2c')->_('今日订单'),'optional'=>true,'filter'=>$today_filter,'addon'=>$today_order,'href'=>'index.php?app=b2c&ctl=admin_order&act=index&view=9&view_from=dashboard');
        ...
        $sub_menu[11] = array('label'=>app::get('b2c')->_('昨日订单'),'optional'=>true,'filter'=>$yesterday_filter,'addon'=>$yesterday_order,'href'=>'index.php?app=b2c&ctl=admin_order&act=index&view=10&view_from=dashboard');

        //今日已付款订单
        $today_filter = array_merge($today_filter,array('pay_status'=>'1'));
        $today_payed = $mdl_orders->count($today_filter);
        $sub_menu[12] = array('label'=>app::get('b2c')->_('今日已付款'),'optional'=>true,'filter'=>$today_filter,'addon'=>$today_payed,'href'=>'index.php?app=b2c&ctl=admin_order&act=index&view=11&view_from=dashboard');

        //昨日已付款订单
        $yesterday_filter = array_merge($yesterday_filter,array('pay_status'=>'1'));
        $yesterday_payed = $mdl_orders->count($yesterday_filter);
        $sub_menu[13] = array('label'=>app::get('b2c')->_('昨日已付款'),'optional'=>true,'filter'=>$yesterday_filter,'addon'=>$yesterday_payed,'href'=>'index.php?app=b2c&ctl=admin_order&act=index&view=11&view_from=dashboard');
    ...
    public function printing($type,$order_id)
    {
        ...
        foreach ($orderInfo['order_objects'] as $k=>$v)
        {
            ...
            if ($v['obj_type'] == 'goods')
            {
                foreach ($v['order_items'] as $key => $item)
                {
                    ...
                    {
                        if ($item['item_type'] == 'product' ||$item['item_type']=='virtual'||$item['item_type']=='entity')
        ...
        switch($type)
        {
            ...
            case $order->arr_print_type['ORDER_PRINT_DLY']:    /*快递单打印*/
                ...
                $this->pagedata['printers'] = $printer->getList('prt_tmpl_id,prt_tmpl_title',array('shortcut'=>'true','store_id'=>$orderInfo['store_id']));
    ...
    public function godelivery($order_id)
    {
        ...
        $this->pagedata['shippings'] = $shippings->getList('*',array("store_id"=>$aORet['store_id']));
		
		$dlycorp = app::get('business')->model('dlycorp');
		$corp_id_arr = $dlycorp->getList('corp_id',array("store_id"=>$aORet['store_id']));
		foreach($corp_id_arr as $items){
            $corp_ids[] = $items['corp_id'];
        }
		
        $dlycorp = $this->app->model('dlycorp');
        $this->pagedata['corplist'] = $dlycorp->getList('*',array("corp_id"=>$corp_ids));
        $this->pagedata['order'] = $aORet;
                ...
                if ($arrOdrObjects['obj_type'] == 'goods')
                {
                    ...
                    foreach ($arrOdrObjects['order_items'] as $arrOdrItems)
                    {
                        ...
                        if ($arrOdrItems['item_type'] != 'gift')
                        {
							...
							// 卡券商品的解析
                           else if ($arrOdrItems['item_type'] == 'virtual')
                            {                            
                                $good_id = $arrOdrItems['products']['goods_id'];
                                $product_id = $arrOdrItems['products']['product_id'];
                                $arrAddon = unserialize($arrOdrItems['addon']);
                                ...
                                if ($arr_service_goods_type_obj['cards'])
                                {
                                    ...
                                    $str_service_goods_type_obj->get_order_object(array('order_id'=>$arrOdrItems['order_id'],'goods_id' => $arrOdrItems['goods_id'],'product_id'=>$arrOdrItems['products']['product_id'],'type'=>'virtual'), $arrGoods);
								}
                                $arrOdrItems['products']['name']  = $arrOdrItems['name'];
                                /*if ($arrOdrItems['addon'])
                                {                                        
                                ...
                                $arrItems[] = array(
                                    ...
                                    'item_id' => $arrOdrItems['item_id'],
                                    'needsend' => $this->objMath->number_minus(array($arrOdrItems['quantity'], $arrOdrItems['sendnum'])),
                                    'card_pass' =>$arrGoods['card_pass'],
                                );
								if($arrGoods['card_pass']){
									$is_card_pass=true;
								}
                            }
							else if ($arrOdrItems['item_type'] == 'entity')
                            {                            
                                $good_id = $arrOdrItems['products']['goods_id'];
                                $product_id = $arrOdrItems['products']['product_id'];
                                $arrAddon = unserialize($arrOdrItems['addon']);
                                ...
                                {
									$str_service_goods_type_obj = $arr_service_goods_type_obj['cards'];
                                    $str_service_goods_type_obj->get_order_object(array('order_id'=>$arrOdrItems['order_id'],'goods_id' => $arrOdrItems['goods_id'],'product_id'=>$arrOdrItems['products']['product_id'],'type'=>'entity'), $arrGoods);
								}
                                $arrOdrItems['products']['name']  = $arrOdrItems['name'];
                                /*if ($arrOdrItems['addon'])
                                {                                        
        ...
        $this->pagedata['is_card_pass'] = $is_card_pass;
    ...
    public function goback_finish_order($order_id){
        $this->begin();
        ...
        $this->display('admin/order/gobackfinishorder.html');

    }


    public function goback_finish_order_edit(){
        ...
        $db = kernel::database();
        $db->exec("DELETE FROM sdb_ectools_refunds WHERE refund_id=(SELECT bill_id FROM sdb_ectools_order_bills WHERE rel_id='".$order_id."' AND bill_type='blances')");
        $db->exec("DELETE FROM sdb_ectools_order_bills WHERE rel_id='".$order_id."' AND bill_type='blances'");
        $db->exec("DELETE FROM sdb_b2c_order_log WHERE rel_id='".$order_id."' AND behavior='finish'");
        $db->exec("UPDATE sdb_b2c_orders SET status='active',confirm_time=unix_timestamp()+31536000 WHERE order_id='".$order_id."'");
    }
	

	//卡劵订单导出
	function  card_explode(){
		@ini_set('memory_limit','512M');
		set_time_limit(0); 
		$arr = $_POST;
		$order_object = app::get('b2c')->model('orders');
		$cols = $order_object->_columns();

        if($arr['isSelectedAll'] == '_ALL_' &&  empty($arr['order_id'])){
            $tmp = app::get('b2c')->model('orders')->getList('order_id',$arr);
            $tmp_order = array();
            if(!empty($tmp)){
                foreach($tmp as $tmp_k=>$tmp_v){
                    $tmp_order[] = $tmp_v['order_id'];
                }
            }
            if(!empty($tmp_order)){
                $arr['order_id'] = $tmp_order;
            }
        }

		$order_str = implode(',',$arr['order_id']);
		$sand_object = app::get('sand')->model('sandorder');
		$sand_data = $sand_object->getList('*',$arr);
		$title = array();
		array_push($title,'订单号(order_id)','运单号(logi_no)','发货状态(ship_status)','下单时间(createtime)','发货时间(t_begin)','配送方式(shipping)','会员名字(name)','人才号(login_name)','收货人(ship_name)','收货地址(ship_addr)','邮编(ship_zip)','收货人手机(ship_mobile)','订单备注(mark_text)','店铺名称(store_name)','卡劵号(card_no)','礼包名称(card_name)','商社名称(company_name)','商品名称(good_name)','购买数量(nums)');
		$csv_data[0] = $title;
		...
		$sql = "SELECT 
			...
			FROM 
			sdb_b2c_orders o 
			LEFT JOIN sdb_cardcoupons_cards_pass p  ON  SUBSTR(p.exchange_order_id, 16, 16) = o.order_id 
			LEFT JOIN sdb_pam_account acc ON acc.account_id = o.member_id
			LEFT JOIN sdb_b2c_members m ON m.member_id = o.member_id 
			LEFT JOIN sdb_business_storemanger shop on o.store_id = shop.store_id 
			LEFT JOIN sdb_b2c_order_items it on o.order_id = it.order_id
			LEFT JOIN sdb_b2c_delivery de on o.order_id = de.order_id
			LEFT JOIN sdb_b2c_goods gd on gd.goods_id = it.goods_id 
			where o.order_id in(".$order_str.") and order_kind = 'card'";

		$data_arr = kernel::database()->select($sql);
		...
		foreach($data_arr as $key=>$item){
			...
			$arr = array(
				...
				'last_modified'=>$item['t_begin'],
				'shipping'=>$item['shipping'],
				'name'=>$item['mname'],
				'login_name'=>$item['login_name'],	
				'ship_name'=>$item['ship_name'],	
				'ship_addr'=>$item['ship_addr'],	
				'ship_zip'=>$item['ship_zip'],
		...
		$csv_string = null;
		$csv_row = array();
		foreach( $csv_data as $k => $csv_item )
		{
			$current = array();
			foreach( $csv_item AS $key=> $item )
			{
			/****************************************************************************************************************************
			*很关键。 默认csv文件字符串需要 ‘ " ’ 环绕,否则导入导出操作时可能发生异常。
			****************************************************************************************************************************/
			
			if( in_array( $cols[$key]['type'],array('time','last_modify') ) && $item ){
				 $item = date('Y-m-d H:i',$item)."\t";
			}
			if ($cols[$key]['type'] == 'longtext'){
				if (strpos($item, "\n") !== false){
					$item = str_replace("\n", " ", $item);
				}
			}
			...
			if(is_array($cols[$key]['type'])){
				$item = $cols[$key]['type'][$item];
			}
