    ...
    */
    ...
    function index(){
        ...
        if (!$objCat->checkTreeSize())
        {
			...
			if($_GET['cat_id']){
				$editShowCatId = $_GET['cat_id'];
				$editShowCatPath = $objCat->getList('cat_path',array('cat_id'=>$editShowCatId));
				...
				foreach($tree as $key=>$val){
					if($val['cat_id']==$childcat_info[0]['pid']){
    ...
    function _info($id=0,$type='add'){
     ...
     function save(){
		 $objCat = &$this->app->model('goods_cat');
		 if($_POST['cat']['cat_id']){
			$this->begin('index.php?app=b2c&ctl=admin_goods_cat&act=index&cat_id='.$_POST['cat']['cat_id']);
		 }else{
			if($_POST['cat']['parent_id']){
				$nextCat = $objCat->getList('cat_id',array('parent_id'=>$_POST['cat']['parent_id']));
				if($nextCat){
					$this->begin('index.php?app=b2c&ctl=admin_goods_cat&act=index&cat_id='.$nextCat['0']['cat_id']);
				}else{
					$this->begin('index.php?app=b2c&ctl=admin_goods_cat&act=index&cat_id='.$_POST['cat']['parent_id']);
				}
			}else{
