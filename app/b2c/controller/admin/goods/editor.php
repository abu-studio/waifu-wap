...
class b2c_ctl_admin_goods_editor extends desktop_controller{
   ...
   //新增商品页面ctl
    ...
    function editprice(){
		...
		$goodsData = app::get('b2c')->model('goods')->getRow('price,goods_id',array('goods_id'=>$goods_id));	
		$this->pagedata['data'] = $goodsData;
        $this->display('admin/goods/detail/price.html');
    }
	//保存京东价格
	function saveprice(){
		 $objgoods = app::get('b2c')->model('goods');
		 $this->begin('index.php?app=jdsale&ctl=admin_goods&act=index');
		 if($objgoods->update(array('price'=>$_POST['goods']['price']),array('goods_id'=>$_POST['goods']['goods_id']))){
...
//                }
    ...
    function edit($goods_id){
		...
		if($goods['goods_application']=='4'){
			$this->pagedata['show_status']='1';
		}
        ksort($goods['images']);
