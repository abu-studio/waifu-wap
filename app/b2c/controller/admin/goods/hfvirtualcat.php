...
class b2c_ctl_admin_goods_hfvirtualcat extends desktop_controller{
    ...
    function index(){
        $objCat = &$this->app->model('goods_hfvirtual_cat');
        ...
        $this->page('admin/goods/hfvirtualcat/map.html');

    }
    function addNew($id=0){
        $this->begin('index.php?app=b2c&ctl=admin_goods_hfvirtualcat&act=index');
        $this->path[] = array('text'=>app::get('b2c')->_('商品虚拟分类新增'));
        $vobjCat = &$this->app->Model('goods_hfvirtual_cat');
        ...
        $this->pagedata['cat']['goods_type'] = $this->getGoodsType();
        $this->pagedata['cat']['current_goods_type'] = '';

        $this->pagedata['cat']['p_order'] = 0;
        $this->display('admin/goods/hfvirtualcat/info.html');
    }
    ...
    function doAdd(){
        ...
        if ($cat['goods_type'] == 'jdtype' ){
            $cat['filter'] = $_POST['adjunct']['items'][1];
        }else if($cat['goods_type'] == 'jdbook'){
            $cat['filter'] = $_POST['adjunct']['items'][2];
        }else{
    ...
    function doImport(){
        ...
        if(!is_array($_POST['cat']) || empty($_POST['cat'])){
            $this->splash('failed', 'index.php?ctl=goods/hfvirtualcat&act=import', app::get('b2c')->_('请选择商品分类源节点'));
        }else{
            $this->begin('index.php?app=b2c&ctl=admin_goods_hfvirtualcat&act=index');
            ...
            $objCat = &$this->system->loadModel('goods/hfvirtualcat');
    ...
    function edit($catid){
        ...
        $this->pagedata['cat']['current_goods_type'] = $aCat['goods_type'];
    ...
    function toRemove($id){
        $this->begin('index.php?app=b2c&ctl=admin_goods_hfvirtualcat&act=index');
        $objType = &$this->app->Model('goods_hfvirtual_cat');
    ...
    private function getGoodsType(){
        $result = array(0=>array('code' =>'normal','name' =>'普通商品'),
                        1=>array('code' =>'jdtype','name' =>'京东商品'),
                        2=>array('code' =>'jdbook','name' =>'京东图书')
        );
        return $result;
    }
