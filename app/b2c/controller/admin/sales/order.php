...
class b2c_ctl_admin_sales_order extends desktop_controller{
    ...
    public function index(){
		if($_POST['store_id']){
			...
			$actions=array();
			$actions[]=array('label'=>app::get('b2c')->_('添加规则'),'href'=>'index.php?app=b2c&ctl=admin_sales_order&act=add','target'=>'_blank');
			if($this->has_permission('salesorder')) $actions[]=array('label'=>app::get('b2c')->_('修改启用状态'),'submit'=>'index.php?app=b2c&ctl=admin_sales_order&act=update_status','target'=>'dialog::{title:\''.app::get('b2c')->_('启用规则').'\',width:960,height:480}');
			$this->finder('b2c_mdl_sales_rule_order',array(
				'title'=>app::get('b2c')->_('订单促销规则'),
				'actions'=>$actions,
				'use_buildin_filter'=>true,
				'base_filter'=>array('rule_type'=>'N','store_id|has'=>$store_id)
			));
		}else{
			$actions=array();
			$actions[]=array('label'=>app::get('b2c')->_('添加规则'),'href'=>'index.php?app=b2c&ctl=admin_sales_order&act=add','target'=>'_blank');
	...
	**/
	public function update_status(){
		if($_POST['update']=='status'){
			...
			$comments=$_POST['rule']['all']['comments'];
			unset($_POST['rule']['all']);
			$obj_db=kernel::database();
			foreach($_POST['rule'] as $key=>$value){
				$value['comments']=trim($value['comments']);
				if($value['status']=='false'&&!$value['comments']){
					$value['comments']=trim($comments);
				}
				$sql="update ".DB_PREFIX."b2c_sales_rule_order set `status`='".$value['status']."',`comments`='".$value['comments']."' where `rule_id`='".$key."'";
				
				$result=$obj_db->exec($sql);
			}
			$this->end(true,app::get('b2c')->_('操作成功'));
			//echo "<script>if(parent.$('ruleorderstatus_form').getParent('.dialog')) parent.$('ruleorderstatus_form').getParent('.dialog').retrieve('instance').close();window.location.href='index.php?app=b2c&ctl=admin_sales_order&act=index'</script>";
			exit;
		}
		$rules=kernel::single('b2c_mdl_sales_rule_order')->getList('*',array('rule_id'=>$_POST['rule_id']));
		foreach($rules as $key=>$arr){
			//会员等级
			if($arr['member_lv_ids']) {
				$member_lv_id = explode(',', $arr['member_lv_ids']);
				$member = $this->app->model('member_lv')->getList('name', array('member_lv_id'=>$member_lv_id) );
				if(count($member_lv_id)>count($member)) {
					$member[] = array('name'=>app::get('b2c')->_('非会员'));
				}
				$rules[$key]['member_lvs'] = $member;
			}
				//过滤条件
			if($arr['conditions']) {
				if($arr['c_template']) {
					$rule_condition=str_replace('X',$arr['conditions']['conditions'][0]['value'],kernel::single($arr['c_template'])->tpl_name);
					$rules[$key]['condition'] = $rule_condition;
				}
			}
			//优惠方案
			if($arr['action_solution']) {
				if($arr['s_template']) {
					...
					$rules[$key]['solution'] = $o->getString();
				}
			}
			
		}
		$this->pagedata['rules']=$rules;
		$this->page('admin/sales/promotion/status.html');
	}
    ...
    public function toAdd() {
		...
		if( $aData['store_id']){
            ...
            $comma_separated = ','.$aData['store_id'].',';
            $aData['store_id']=  $comma_separated;
        }
