    ...
    function save_editor(){
        ...
        if(isset($goods['goods_id']) && $goods['goods_id']) {
            ...
            foreach( $goods['product'] as $pk => $pv ){
                ...
                $xin_mktprice[] = $goods['product'][$pk]['price']['mktprice']['price'];
                $xin_cost[] = $goods['product'][$pk]['price']['cost']['price'];
            ...
            $aGoods['price'] = min($xin_price);//销售价取最低
            $aGoods['mktprice'] = max($xin_mktprice);//市场价取最高
            $aGoods['cost'] = min($xin_cost);//成本价取最低
