...
class package_ctl_admin_attendactivity extends desktop_controller{
    ...
    function delete(){
        ...
        foreach($ids as $k=>$v){
            ...
            /*bug repair by icebergdev*/
            $sdf_goods_id = array();
            $tmp_goods_id = $object->dump(array('id' => $v), 'gid');
            $goods_id = trim($tmp_goods_id['gid'], ',');
            $re = $object->delete(array('id' => $v));
            if ($re) 
            {
                $sql1 = 'SELECT goods_id,act_type FROM sdb_b2c_goods WHERE goods_id IN (' . $goods_id . ') ';
                $act_types = $oGoods->db->select($sql1);
                if (!empty($act_types) && is_array($act_types)) 
                {
                    foreach ($act_types as $key => $value) 
                    {
                        if ($value['act_type'] != 'package') unset($act_types[$key]);
                        else $sdf_goods_id[] = $value['goods_id'];
                    }
                }
                
                if (!empty($sdf_goods_id)) 
                {
                    $sql = 'UPDATE sdb_b2c_goods SET act_type="normal" WHERE goods_id IN ( '.implode( ',', $sdf_goods_id ).' )';
                    $rs = $oGoods->db->exec($sql);
                    if(!$rs){
                        $this->end(false, '删除失败');
                    }
