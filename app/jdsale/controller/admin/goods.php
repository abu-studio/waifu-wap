...
class jdsale_ctl_admin_goods extends desktop_controller{

     var $workground = 'jdsale.workground.order';

	 public function index(){
         app::get('jdsale')->setConf('goodsdata.isinitial','0');
         ...
         if ( isset($is_initial) && $is_initial === 1){
            //不显示按钮
             ...
             $custom_actions[] = array(
                 'label'  => app::get('b2c')->_('同步京东商品数据'),
                 'icon'   => 'download.gif',
                 'href' => 'index.php?app=jdsale&ctl=admin_goods&act=import',
                 'target' => ''
             );
         }
         ...
         $custom_actions[] = array(
                 ...
                 'icon'   => 'download.gif',
                 'href' => 'index.php?app=jdsale&ctl=admin_goods&act=importBook',
                 'target' => ''
             );
         ...
         if($this->has_permission('jdeditmarketable')){
             $group =array(
                 array(
                     ...
                     'icon'   => 'download.gif',
                     'submit' => 'index.php?app=jdsale&ctl=admin_goods&act=singleBatchEdit&p[0]=enable',
                     'target' => 'dialog'
                 ),
                 array('label'=>'_SPLIT_'),
                 array(
                     ...
                     'icon'   => 'download.gif',
                     'submit' => 'index.php?app=jdsale&ctl=admin_goods&act=singleBatchEdit&p[0]=disable',
                     'target' => 'dialog'
                 )
             );
         ...
         $this->finder('b2c_mdl_goods',array(
            ...
            'base_filter'=>array('disabled'=>'false','goods_kind'=>'jdgoods','cat_id'=>array('0')),
    ...
    public function import(){
        ...
        if (empty($shopId)){
            $this->splash('success','','未设置京东自营商铺！');
        }else{
            app::get('jdsale')->setConf('goodsdata.isinitial',0);
            $is_initial = app::get('jdsale')->getConf('goodsdata.isinitial');
            if ($is_initial != 1) {
                set_time_limit(0);
                $jdsale_goods_import = kernel::single('jdsale_goods_import');
                if ($jdsale_goods_import->initialData($shopId)){
                    app::get('jdsale')->setConf('goodsdata.isinitial',1);
                    $this->splash('success','','京东商品数据导入成功！');
                }else{
                    $this->splash('success','','京东商品数据导入失败！');
                }
            }else{
                $this->splash('success','','京东商品数据已经导入！');
            }
        }
    ...
    public function _views(){
        ...
        $sub_menu[0] = array('label'=>app::get('b2c')->_('全部'),'optional'=>true,'filter'=>'','addon'=>$mdl_goods->count(array('goods_kind'=>'jdgoods','cat_id'=>array('0'))));

        //已下架商品
        $filter = array('marketable'=>'false','goods_type'=>'normal','goods_kind'=>'jdgoods','cat_id'=>array('0'));
        $market_count = $mdl_goods->count($filter);
        if($market_count >0){
            $sub_menu[1] = array('label'=>app::get('b2c')->_('已下架商品'),'optional'=>true,'filter'=>$filter,'addon'=>$market_count,'href'=>'index.php?app=jdsale&ctl=admin_goods&act=index&view=1&view_from=dashboard');
        }

        //上架商品
        $markettrue_filter=array('marketable'=>'true','goods_type'=>'normal','goods_kind'=>'jdgoods','cat_id'=>array('0'));
        $markettrue_count = $mdl_goods->count($markettrue_filter);
        if($markettrue_count>0){
            $sub_menu[7] = array('label'=>app::get('b2c')->_('已上架'),'optional'=>true,'filter'=>$markettrue_filter,'addon'=>$markettrue_count,'href'=>'index.php?app=jdsale&ctl=admin_goods&act=index&view=7&view_from=dashboard');
        }
