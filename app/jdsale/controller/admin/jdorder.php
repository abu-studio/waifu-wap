...
class jdsale_ctl_admin_jdorder extends desktop_controller{
	...
	public function index(){
		...
		$finder_arr = array(
				...
				'base_filter'=>array('order_refer'=>'local','disabled'=>'false','order_kind'=>'jdorder'),
		...
		if($_GET['view'] == '6') {
			$finder_arr['actions']= array(
					array('label'=>app::get('b2c')->_('强制下单'),'submit'=>'index.php?app=jdsale&ctl=admin_jdorder&act=jd_submit','target'=>'_self'),
					array('label'=>app::get('b2c')->_('强制对账'),'submit'=>'index.php?app=jdsale&ctl=admin_jdorder&act=jd_checkorders','target'=>'_self'),

			);
		}

        $this->finder('jdsale_mdl_jdorders',$finder_arr);
	...
	public function _views(){
		...
		$order_filter=array('order_kind'=>'jdorder');
        ...
        foreach($sub_menu as $k=>$v){
            if($v['optional']==false){
                ...
                $show_menu[$k]['href'] = 'index.php?app=jdsale&ctl=admin_jdorder&act=index&view='.($k).'&view_from=dashboard';
				...
				if($data_tmp['pOrder']){
					...
					foreach($datalist['cOrder'] as $key=>$val){
						foreach($val['sku'] as $k=>$v){
						  ...
						  $datalist['cOrder'][$key]['sku'][$k]['link'] = $router->gen_url(array('app'=>'jdsale', 'ctl'=>'site_product','act'=>'index','arg0'=>$goodsDate['goods_id']));
						}
					}
	...
	function jd_submit(){
		...
		$this->begin('index.php?app=jdsale&ctl=admin_jdorder&act=index');
