...
class jdsale_autobookmessageget{
    function auto_getmessage(){
        $array = array(
            'type'=>'2,4,6,16,50',
        );
        do {
            $data = kernel::single('jdsale_api_area')->getMessage($array , 'book');
    ...
    $jddata_tmp = kernel::single('jdsale_api_goods')->getSellPrice($array , 'book');
	...
	if(!empty($jddata)){
		...
		$goods_array = $goods_obj->dump(array('bn|tequal'=>(string)$jddata[0]['skuId'],'goods_kind'=>'jdbook'),"goods_id,bn,price,agreed_price");
    ...
    private function  change_jdmarketable($data){
        ...
        $goods_obj = app::get("b2c")->model("goods");
        $products_obj = app::get("b2c")->model("products");
        $jddata_tmp = kernel::single('jdsale_api_goods')->queryProductState($array , 'book');
        ...
        if(!empty($jddata)){
            $goods_array = $goods_obj->dump(array('bn|tequal'=>(string)$jddata[0]['sku'],'goods_kind'=>'jdbook',),"goods_id,marketable,marketable_allow");
            if(!empty($goods_array)){
                ...
                if($goods_array['status'] != $jddata[0]['state']){
                    ...
                    $goods_obj->update($jdgoods,array('goods_id'=>$goods_array['goods_id'],'goods_kind'=>'jdbook'));
    ...
    private function change_jdgoods($data){
        ...
        $goods_obj = app::get("b2c")->model("goods");
        $jddata_tmp = kernel::single('jdsale_api_goods')->queryProductDetail($array , 'book');
		...
		if(!empty($jddata)){
            //构建商品品牌信息 brandName
            $brand_id = kernel::single('jdsale_goods_importbook')->buildBrand($jddata['brandName']);
            //构建商品分类信息 category
            $ret_cat = kernel::single('jdsale_goods_importbook')->buildCategory($jddata['category']);
            $jddata['category'] = $ret_cat;
            $jddata['brand_id'] = $brand_id;
            //保存商品和货品信息
            $goods_id = $goods_obj->dump(array('bn|tequal'=>(string)$jddata['sku'],'goods_kind'=>'jdbook'),"goods_id,bn");
            ...
            $goods_array = kernel::single('jdsale_goods_importbook')->updateGoodsData($jddata);
            kernel::single('jdsale_api_area')->delMessage(array('id'=>$data['id']) , 'book');
    ...
    private function get_jdgoods($data){
        ...
        if($data['result']['state'] == 1){
            ...
            $goods = app::get("b2c")->model("goods")->dump(array('bn|tequal'=>(string)$data['result']['skuId'],'goods_kind'=>'jdbook'),"goods_id,bn");
            if ( $goods ) {
                kernel::single('jdsale_api_area')->delMessage(array('id'=>$data['id']) , 'book');
                return false;
            }
            ...
            if(!empty($jddata)){
                ...
                $goods_array = kernel::single('jdsale_goods_importbook')->saveGoodsData($jddata);
		        if($goods_array){
		            kernel::single('jdsale_goods_importbook')->saveImageAttach($jddata['sku'],$goods_array);
                    kernel::single('jdsale_api_area')->delMessage(array('id'=>$data['id']) , 'book');
                }
            }
        }elseif($data['state'] == 2){
            ...
            $goods_obj = app::get("b2c")->model("goods");
            kernel::single('jdsale_api_area')->delMessage(array('id'=>$data['id']) , 'book');
            $goods_obj->update(array('marketable'=>'false'),array('bn|tequal'=>(string)$data['result']['skuId'],'goods_kind'=>'jdbook'));
        }else{
            //如果还有不知道的类型默认删除掉
            kernel::single('jdsale_api_area')->delMessage(array('id'=>$data['id']) , 'book');
        }
