...
/**
  ...
  */
class jdsale_regions_operation
{
    /**
    ...
    {
        if ($regionId)
            $aTemp = $this->model->getList('area_id as region_id,p_region_id,local_name,ordernum,region_path', array('p_region_id' => $regionId), 0, -1, 'ordernum ASC,region_id ASC');
        else
            $aTemp = $this->model->getList('area_id as region_id,p_region_id,local_name,ordernum,region_path', array('region_grade' => '1'), 0, -1, 'ordernum ASC,region_id ASC');
    ...
    {
        if ($prId)
            $sql="select area_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1).
            " where p_region_id=r.area_id) as child_count from ".$this->model->table_name(1).
            " as r where r.p_region_id=".intval($prId)." order by ordernum asc,region_id";
        else
            $sql="select area_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1).
                " where p_region_id=r.area_id) as child_count from ".$this->model->table_name(1).
                " as r where r.p_region_id is null order by ordernum asc,region_id";
        ...
        if (isset($row) && $row)
        {
            foreach ($row as $key => $val)
            {
                $this->regions[] = array(
                    ...
                    "region_id"=>$val['area_id'],
                    "region_grade"=>$val['region_grade'],
                    "ordernum"=>$val['ordernum']
                );
                
                if ($val['child_count'])
                    $this->getMap($val['area_id']);
            }
        }
    }
    ...
    public function insertDlArea($aData,&$msg)
    {
        ...
        {
            $msg = app::get('jdsale')->_('地区名称不能为空！');
            return false;
        }
        if (!trim($aData['area_id']))
        {
            $msg = app::get('jdsale')->_('京东地区编码不能为空！');
            return false;
        }

        if ($this->model->checkDlArea($aData['area_id'], $aData['p_region_id']))
        {
            $msg = app::get('jdsale')->_('该地区名称已经存在！');
            return false;
        }

        $tmp = $this->model->db->selectrow('select region_path from '.$this->model->table_name(1).' where area_id='.intval($aData['p_region_id']));

        if (!$tmp)
            $tmp['region_path'] = ",";
        
        $region_path = $tmp['region_path'];
        $aData['package'] = 'mainland';
        ...
        if ($this->model->save($aData))
        {
            ...
            $tmp['region_path'] = $region_path.$tmp['area_id'].',';
    ...
    public function updateDlArea($aData,&$msg)
    {
        ...
        if ($aData['area_id'] == $aData['p_region_id'])
        {
        ...
        {
            $idGroup = $this->model->getGroupRegionId($aData['area_id']);
            if ($idGroup)
            {
        ...
        {
            $cPath = $this->model->dump(intval($aData['area_id']), 'region_path');
        }
        ...
        if (intval($aData['p_region_id']))
        {
            ...
            $aData['region_path'] = $tmp['region_path'].$aData['area_id'].",";
        }
        else
        {
            $aData['region_path'] = ",".$aData['area_id'].",";
        }
    ...
    {
        $tmpRow = $this->model->dump(array('area_id'=>intval($regionId)), 'region_path');

        $this->model->delete(array('area_id' => intval($regionId)));
        // 删除相应的所有的下级地区
    ...
    private function getAllRegions($p_regionid='', $pkey='', &$region_count=array()) {
        ...
        if ($p_regionid)
            $sql="select area_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1)
                ." where p_region_id=r.area_id) as child_count from ".$this->model->table_name(1)
                ." as r where r.p_region_id=".intval($p_regionid)." order by ordernum asc,area_id";
        else
            $sql="select area_id,region_grade,local_name,ordernum,(select count(*) from ".$this->model->table_name(1)
                ." where p_region_id=r.area_id) as child_count from ".$this->model->table_name(1)
                ." as r where r.p_region_id is null order by ordernum asc,area_id";
        ...
        if (isset($row) && $row)
        {
            ...
            foreach ($row as $key => $val)
            {
                ...
                if ($val['child_count'] && $val['region_grade']<$show_depth) {
                    $this->getAllRegions($val['area_id'], $start_index-1, $region_count);
                }
            }
        }
    }
    ...
    /**
     * 根据最后一级地区id获取本地区的各级地址id
     * @param $region_id 地区名
     * @return $array
     */
    public function getRegionPathById($region_id){
        $row = $this->model->getRow('region_path',array('area_id' =>$region_id));

        if ($row['region_path']){
            $ret = explode(',',$row['region_path']);
