...
class jdsale_goods_import {
    ...
    public function initialData($shopID){
        $retval = true;
        $is_initial = app::get('jdsale')->getConf('goodsdata.isinitial');
        ...
        if($conn){
            ...
            // if(!empty($conn_array)){
            //     foreach($conn_array as $chi_k=>$chi_v){
            //         if($chi_v && $chi_v){
        ...
        $conn_init=@file_get_contents(ROOT_DIR.'/jdbook_pagenum.txt');
        if($conn_init){
        ...
        // 调用1.1.1 获取商品池编号
        // 过滤掉一部分 类别
        $pageNum = $this->api_goods->queryPageNum();
        ...
        foreach($pageNum['result'] as $k1 => $v1){
            ...
            error_log("=-=-=-=-商品池=-=-=-=-".var_export($v1,1).PHP_EOL,3,DATA_DIR."/jdimportgood-pagenum.log");
            ...
            {
                error_log("=-=-=-=-商品池=-=-=-=-".PHP_EOL.var_export($v1,1)."====一级分类====".$rootCat['name'].PHP_EOL,3,DATA_DIR."/jdimportgood.log");
                unset($rootCat);
                continue;
            }else{
                unset($rootCat);
            ...
            $sku_result_all =$this->api_goods->querySku($params1);
            ...
            foreach ($tmp_sku_arr as $tk1 => $tv1) {
                ...
                $sku_status = $this->api_goods->checkProduct($tmp_param);
            ...
            foreach($sku_result as $k2 => $v2){
                error_log("=-=-=-=-sku=-=-=-=-".PHP_EOL.var_export($v2,1).PHP_EOL,3,DATA_DIR."/jdimportgood.log");

//                $mdl_goods = app::get('b2c')->model('goods');
//                $c = $mdl_goods->count(array('bn'=> $v2));
//                if ($c>0){
//                    continue ;
//                }
                ...
                //调用1.1.3获取商品详细信息
                $sku_detail_result  =$this->api_goods->queryProductDetail($params2);
                $sku_detail = $sku_detail_result['result'];
                if (empty($sku_detail)){
                    continue;
                }

                //一般商品不处理book和video类型的商品 by zyp@2018-12-13
                if ($sku_detail['skuType'] == 'book' || $sku_detail['skuType'] == 'vedio') {
                    continue;
                }
                ...
                //获取商品评价信息
                $averagescore = $this->api_goods->queryProductComment(array('sku' =>$v2));
            ...
            //这里应该是记录已处理的商品池 by zyp@2018-12-20
    ...
    public function buildCategory($category){
        ...
        $catInfo_level_1_arr = $this->api_goods->getCategory(array('cid'=>$catArr[0]));
        ...
        $catInfo_level_2_arr = $this->api_goods->getCategorys(
            array('pageNo'=>1,'pageSize'=>5000,'parentId'=>$catInfo_level_1['catId'],'catClass'=>'1'));
        ...
        foreach($catInfo_level_2['categorys'] as $k1 => $v1){
            ...
            $catInfo_level_3_arr = $this->api_goods->getCategorys(
                array('pageNo'=>1,'pageSize'=>5000,'parentId'=>$v1['catId'],'catClass'=>'2'));
    ...
    public function saveGoodsData($sku_detail){
        ...
        //查询价格
        $sku_price = $this->api_goods->getSellPrice(array('sku' => $sku_id));
        error_log("=-=-=-=-sku_price=-=-=-=-".PHP_EOL.var_export($sku_price,1).PHP_EOL,3,DATA_DIR."/jdimportgood.log");
        ...
        if ($mdl_goods->save($goods_detail)){
            error_log("=-=-=-=-goods_detail['goods_id']=-=-=-=-".PHP_EOL.var_export($goods_detail['goods_id'],1).PHP_EOL,3,DATA_DIR."/jdimportgood.log");
    ...
    public function updateGoodsData($sku_detail){
        ...
        $sku_price = $this->api_goods->getSellPrice(array('sku' => $sku_detail['sku']));
        if($sku_price['result'][0]['jdPrice'] <= 0.01){
            return false;
        }
        $goods_detail =  array (
    ...
    public function saveImageAttach($skuId,$goods_id){
        ...
        $result = $this->api_goods->querySkuImage(array('sku'=>$skuId));
    ...
    public function getRootCatInfoByThirdCatId($catId){
        ...
        //三级分类信息
        $tmp_catInfo_level_1_arr = $this->api_goods->getCategory($catParams);
        $tmp_catInfo_level_1 = $tmp_catInfo_level_1_arr['result'];

        //二级分类信息
        if ($tmp_catInfo_level_1['catClass'] != '0' && $tmp_catInfo_level_1['parentId'] != '0'){
            $tmp_catInfo_level_2_arr = $this->api_goods->getCategory(array('cid'=>$tmp_catInfo_level_1['parentId']));
        ...
        if ($tmp_catInfo_level_2['catClass'] != '0' && $tmp_catInfo_level_2['parentId'] != '0'){
            $tmp_catInfo_level_3_arr = $this->api_goods->getCategory(array('cid'=>$tmp_catInfo_level_2['parentId']));
