...
class jdsale_checkjdbookorders{
    ...
    public function checkOrders($day = 30){
        ...
        //导入最新余额明细并保存到数据库
        $jdsale_account_import = kernel::single('jdsale_account_importbook');
    ...
    private function getJdOrderInfoByType($type,$day){
        ...
        for( $d =$day;$d>0;$d--){
            ...
            $result = array();
            if ($type =='newOrder'){
                $result = $jdsale_api_checkorder->checkNewOrder(array('date' => $current_date,'page' =>1) , 'book');
            }elseif($type =='checkDlok'){
                $result = $jdsale_api_checkorder->checkDlokOrder(array('date' => $current_date,'page' =>1) , 'book');
            }elseif($type =='checkRefuse'){
                $result = $jdsale_api_checkorder->checkRefuseOrder(array('date' => $current_date,'page' =>1) , 'book');
            }
                    ...
                    for($i=$curPage;$i<=$totalPage;$i++){
                        if ($type =='newOrder'){
                            $result_next = $jdsale_api_checkorder->checkNewOrder(
                                array('date' => $current_date,'page' =>$i) , 'book');
                        }elseif($type =='checkDlok'){
    ...
    private function checkAccount($check_jdorders=array()){
        ...
        foreach ( $check_jdorders as $item) {
            ...
            $jdorder_info = $jdsale_api_order->getOrderJdOrder(array('jdOrderId'=>$item['jdorders_id']) ,'book');
    ...
    private function checkAfsRefund($current_date){
        ...
        //获取退款信息
        $result = $mdl_balance->getList('*',array('order_kind'=>'jdbook' ,'trade_type' =>'416' ,'create_time|than' =>$current_date));
        if(empty($result)){
            return;
        }
        //$this->log($result);
