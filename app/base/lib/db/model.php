...
//                                $data[$k] = time();
        ...
        if(count($UpdateValues)>0){
            $tableName = $this->table_name(1);
            $sql = 'update `'.$tableName.'` set '.implode(',',$UpdateValues).' where '.(is_array( $filter )?base_db_tools::filter2sql($filter):$filter);
            /********************/
            if(in_array($tableName,array('sdb_b2c_goods'))){
                $debugPriceDir = ROOT_DIR . "/data/log/jd_image_bug/" . date('Ymd');
                if(!file_exists($debugPriceDir)){
                    @mkdir($debugPriceDir,0777,true);
                    @chgrp($debugPriceDir,'www');
                    @chown($debugPriceDir,'www');
                }
                $logFilename = $debugPriceDir . '/debugprice.txt';
                if(!file_exists($logFilename)){
                    @touch($logFilename);
                    @chgrp($logFilename,'www');
                    @chown($logFilename,'www');
                }
