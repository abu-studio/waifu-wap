...
class utils{
    ...
    static function _RemoveXSS($val)    {
        ...
        // /x6a/x61/x76/x61/x73/x63/x72/x69/x70/x74/x3a/x61/x6c/x65/x72/x74/x28827/x29
        $ra1 = array('location','window','alert','javascript', 'vbscript', 'expression', 'applet', 'meta', 'xml', 'blink', 'link', 'style', 'script', 'embed', 'object', 'iframe', 'frame', 'frameset', 'ilayer', 'layer', 'bgsound', 'title', 'base');
    ...
    static function filter_input_html($data){
        if(is_array($data)){
            foreach($data as $key=>$v){
                $data[$key] = self::filter_input_html($data[$key]);
            }
        }else{
            if(strlen($data)){
    ...
    static function filter_input_XSS($data){
        if(is_array($data)){
            foreach($data as $key=>$v){
                $data[$key] = self::filter_input_XSS($data[$key]);
            }
        }else{
            if(strlen($data)){
    ...
    static function filter_input_sql($data){
        if(is_array($data)){
            foreach($data as $key=>$v){
                $data[$key] = self::filter_input_sql($data[$key]);
            }
        }else{
            if(strlen($data)){
                $data = self::_RemoveSql($data);
            }else{
                $data = $data;
            }
        }
    ...
    static function filter_input($data){
        if(is_array($data)){
            foreach($data as $key=>$v){
                $data[$key] = self::filter_input($data[$key]);
            }
        }else{
            if(strlen($data)){
                $data=strip_tags($data);
    ...
    static function _RemoveSql($val){
        //return $val;
        $sql_pattern="/[^\\{\\s]{1}(\\s|\\b)+(?:select\\b|update\\b|insert(?:(\\/\\*.*?\\*\\/)|(\\s)|(\\+))+into\\b).+?(?:from\\b|set\\b)|[^\\{\\s]{1}(\\s|\\b)+(?:create|delete|drop|truncate|rename|desc)(?:(\\/\\*.*?\\*\\/)|(\\s)|(\\+))+(?:table\\b|from\\b|database\\b)|into(?:(\\/\\*.*?\\*\\/)|\\s|\\+)+(?:dump|out)file\\b|\\bsleep\\([\\s]*[\\d]+[\\s]*\\)|benchmark\\(([^\\,]*)\\,([^\\,]*)\\)|(?:declare|set|select)\\b.*@|union\\b.*(?:select|all)\\b|(?:select|update|insert|create|delete|drop|grant|truncate|rename|exec|desc|from|table|database|set|where)\\b.*(charset|ascii|bin|char|uncompress|concat|concat_ws|conv|export_set|hex|instr|left|load_file|locate|mid|sub|substring|oct|reverse|right|unhex)\\(|(?:master\\.\\.(sysdatabases|syslogins|sysloginroles)|msysaccessobjects|msysqueries|sysmodules|mysql\\.(db|user)|sys\\.(sql_logins|database_name|dbms_export_extension)|information_schema\\.(USER_PRIVILEGES|SCHEMATA|TABLES|COLUMNS)|sysobjects|sp_makewebtask|xp_cmdshell|sp_oamethod|sp_addextendedproc|sp_oacreate|xp_regread|sysdate\\(\\)|now\\(\\))|(?:\\b(and|or)\\b|\"|')/is";
        return preg_replace($sql_pattern, '', $val);
    }
