...
class base_session{
    ...
    public function set_cookie_expires($minute) 
    {
        ...
        if(isset($this->_sess_id)){
			...
			if(IS_DOMAIN){
				  header(sprintf('Set-Cookie: %s=%s; path=%s; expires=%s; httpOnly; domain=%s; Secure;', $this->_sess_key, $this->_sess_id, $cookie_path, gmdate('D, d M Y H:i:s T', time()+$minute*60),COOKIE_DOMAIN), true);
			}else{
        ...
        if($this->_session_started !== true){
            ...
            if(isset($_GET['sess_id'])){
                ...
                if($_COOKIE[$this->_sess_key] != $_GET['sess_id']){
                    if(IS_DOMAIN){
						header(sprintf('Set-Cookie: %s=%s; path=%s; %s; httpOnly; domain=%s; Secure;', $this->_sess_key, $this->_sess_id, $cookie_path, $cookie_expires,COOKIE_DOMAIN), true);
					}else{
    ...
    public function destory(){
        ...
        if($res){
            ...
            $cookie_path = $cookie_path ? $cookie_path : "/";
			if(IS_DOMAIN){
				  header(sprintf('Set-Cookie: %s=%s; path=%s; httpOnly; domain=%s; Secure;', $this->_sess_key, $this->_sess_id, $cookie_path,COOKIE_DOMAIN), true);
			}else{
