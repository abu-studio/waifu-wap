...
class base_kvstore{
    ...
    static private $__persistent_restore = true;
    ...
    static function config_persistent_tag()
    {
        self::$__persistent_restore = self::$__persistent;
    }

    static function config_persistent_restore()
    {
        self::$__persistent = self::$__persistent_restore ;
    }
    /*
     * 返回KV_PREFIX
     * @access public
    ...
    {
        if($this->get_controller() instanceof base_interface_kvstore_extension){
            return $this->get_controller()->expire($key, $time);
        }else{
            throw new Exception('this instance can\'t support expire');
        }
    }//End Function
    ...
    public function store($key, $value, $ttl=0)
    {
        ...
        self::$__store_count++;
        if(!(defined('WITHOUT_KVSTORE_PERSISTENT') && constant('WITHOUT_KVSTORE_PERSISTENT')) && self::$__persistent && get_class($this->get_controller())!='base_kvstore_mysql' && kernel::is_online() && $this->__prefix != 'sessions' && $this->__prefix != 'cache/expires'){//session,expires数据不要持久化
