...
class base_storage_ftpsystem implements base_interface_storager{
    function base_storage_ftpsystem($store_id=0){
        ...
        switch($_POST['_f_type']){
            case "public":
                ...
                break;
            case "private":
                $this->f_dir = $public_path.'/private'.$store_path;
    ...
    function save($file,&$url,$type,$addons,$ext_name=""){
        ...
        $ftp = kernel::single('b2c_ftp');
        $re = $ftp->up_file($file,$path);
        if($path && $re['status']){
            @chmod($path,0644);
            return $id;
        }else{
...
//        }
    ...
    function remove($id){
        ...
        if($id && file_exists($this->f_dir.$id)){
            return $re = $ftp->del_file($this->f_dir.$id);
        }else{
            return true;
        }
    }
