...
class site_router implements base_interface_router
{
        ...
        if(isset($this->urlmap[$params['app'].':'.$params['ctl']])){
            ...
            if(!isset($this->__gen_url_array[$gen_key])){
                ...
                if($this->__site_router_service[$app]){
				  ...
				  $this->__gen_url_array[$gen_key] = utils::_filter_crlf($this->__gen_url_array[$gen_key]);//防止crlf攻击 add by brucewang 20151013
            }
    ...
    public function http_status($code) 
    {
        ...
        if( $msg ) kernel::single('site_controller')->splash( 'failed',null,$msg,null,null,true);
    ...
    private function set_vary_cookie() 
    {
        ...
        if($cookie_vary !== $vary){
			if(IS_DOMAIN){
				setcookie('vary', $vary, time()+86400*30*12*10, '/',COOKIE_DOMAIN,1);
			}else{
    ...
    public function default_dispatch($query, $allow_name, $separator) 
    {
        ...
        if(is_array($query_args) && count($query_args)){
        ...
        if(method_exists($controller, $action)){
            try{
                ...
                app::get('site')->setConf('errorpage.405' , $e->getMessage());
