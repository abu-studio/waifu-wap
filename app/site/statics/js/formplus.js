...
(function() {
    ...
    var async = function(elem, event, _form) {
        ...
        //重置掉 options , 令form的action值改变后，能及时生效，而不是取old值!
        var frm = elem.getParent('form');
        var options = {url : frm.action};

        if (sync = elem.retrieve('ajax:cache', false)) return sync.send(options);
        sync = new Sync(elem).send(options);
    };
