...
class cardcoupons_frontpage extends site_controller{
    //todo
    ...
    function verify_member(){
        ...
        if(app::get('b2c')->member_id = $_SESSION['account'][pam_account::get_account_type(app::get('b2c')->app_id)]){
            ...
            if($_COOKIE['S']['MEMBER'] != app::get('b2c')->member_id){
               $_sess_key = 's';
    ...
    */
    ...
    function set_cookie($name,$value,$expire=false,$path=null){
        ...
        $expire = $expire === false ? time()+$this->cookie_life : $expire;
		 if(IS_DOMAIN){
				setcookie($name,$value,$expire,$this->cookie_path,COOKIE_DOMAIN,1);
			}else{
    ...
    function check_login(){
        kernel::single('base_session')->start();
        if($_SESSION['account'][pam_account::get_account_type(app::get('b2c')->app_id)]){
            return true;
        }
        else{
            return false;
    ...
    {
        kernel::single('base_session')->start();
        if($member_id = $_SESSION['account'][pam_account::get_account_type(app::get('b2c')->app_id)]){
