...
class cardcoupons_order_service_goods implements b2c_order_service_interface
{
    /**
     * 构造方法
    ...
    public function get_order_object($arr_object=array(), &$arrGoods, $tml='member_order_detail')
    {
        ...
        if ($arrGoods)
            $arrGoods['link_url'] = kernel::single('cardcoupons_ctl_site_product')->gen_url(array('app'=>'cardcoupons','ctl'=>'site_product','act'=>'index','arg0'=>$arr_object['goods_id']));
		...
		$card_pass_sql="SELECT cp.card_no,cp.type from sdb_cardcoupons_cards_pass as cp LEFT JOIN sdb_cardcoupons_cards as cc on cp.card_id=cc.card_id where cp.order_id='".$arr_object['order_id']."' and cc.goods_id='".$arr_object['goods_id']."' and cp.type='".$arr_object['type']."'; ";
		$card_pass_data=kernel::database()->select($card_pass_sql);
		if($card_pass_data){
			$card_pass_arr=array();
			foreach($card_pass_data as $key=>$value){
				$card_pass_arr[]=$value['card_no'];
			}
			$arrGoods['card_pass']=$card_pass_arr;
		}else{
			$arrGoods['card_pass']=false;
		}
        if ($arrGoods)
            return true;
        else
