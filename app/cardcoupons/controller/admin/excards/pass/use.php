...
class cardcoupons_ctl_admin_excards_pass_use extends desktop_controller {

    var $workground = 'cardcoupons.wrokground.card';


    function index(){
		$custom_actions[] = array('label'=>app::get('cardcoupons')->_('添加卡密解冻冻结申请'),'href'=>'index.php?app=cardcoupons&ctl=admin_excards_pass_use&act=add','target'=>'dialog::{title:\''.app::get('cardcoupons')->_('添加卡密解冻冻结申请').'\',width:500,height:400}');
        $actions_base['title'] = app::get('cardcoupons')->_('卡密解冻冻结');
		...
		$actions_base['base_filter'] =array('source'=>'external','type'=>'2');
	...
	function _views(){
		...
		$sub_menu = array(
            0 =>array('label'=>app::get('cardcoupons')->_('全部'),'optional'=>true,'filter'=>array('source'=>'external','type'=>'2')),
			1 =>array('label'=>app::get('cardcoupons')->_('待审核'),'optional'=>true,'filter'=>array('source'=>'external','type'=>'2','ex_status'=>'update')),
			2 =>array('label'=>app::get('cardcoupons')->_('审核成功'),'optional'=>true,'filter'=>array('source'=>'external','type'=>'2','ex_status'=>'true')),
			3 =>array('label'=>app::get('cardcoupons')->_('审核失败'),'optional'=>true,'filter'=>array('source'=>'external','type'=>'2','ex_status'=>'false')),
        );
	...
	function add(){
		$this->begin('index.php?app=cardcoupons&ctl=admin_excards_pass_use&act=index');
		if($_POST['add']=='add'){
			$db = kernel::database();
			...
			$this->display('admin/excards/pass/update/add/use.html');
		}
	}
	...
	function shenhe($id){
		...
		if($_POST['shenhe']=='shenhe'){
			...
			if($rs){
				//查询申请信息中 关联卡密ID 和 解冻冻结修改信息（状态）
				$pass_update_info = $pass_update_obj->dump(array('id'=>$_POST['id']),'card_pass_ids,status');
				if($pass_update_info && $pass_update_info['card_pass_ids']){
					...
					if($_POST['ex_status'] == "true"){
						$pass_date=array(
							//申请信息审核通过，关联卡密审核状态改为已审核并修改 状态
							'ex_status'=>"true",
							'status'=>$pass_update_info['status'],
						);
					}else{
			...
			$this->pagedata['id']=$id;
			$this->display('admin/excards/pass/update/shenhe/use.html');
		}
	}
}
