...
class cardcoupons_ctl_admin_excards_pass_disabled extends desktop_controller {

    var $workground = 'cardcoupons.wrokground.card';


    function index(){
		$custom_actions[] = array('label'=>app::get('cardcoupons')->_('添加卡密有效性申请'),'href'=>'index.php?app=cardcoupons&ctl=admin_excards_pass_disabled&act=add','target'=>'dialog::{title:\''.app::get('cardcoupons')->_('添加卡密有效性申请').'\',width:500,height:400}');
	...
	function add(){
		$this->begin('index.php?app=cardcoupons&ctl=admin_excards_pass_disabled&act=index');
		if($_POST['add']=='add'){
			$db = kernel::database();
			...
			$this->display('admin/excards/pass/update/add/disabled.html');
		}
	}
	...
	function shenhe($id){
		...
		if($_POST['shenhe']=='shenhe'){
			...
			$this->pagedata['id']=$id;
			$this->display('admin/excards/pass/update/shenhe/disabled.html');
		}
	}
}
