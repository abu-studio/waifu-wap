...
class cardcoupons_ctl_admin_cards_editor extends desktop_controller{
    var $simpleGoodsId = 1;

    function nospec($cat_id=0){
        $this->_editor($_POST['type_id']);
        $this->display('admin/cards/detail/spec/nospec.html');
    }

   //新增商品页面ctl
    function add(){
		//卡券类型
		$cards_type_obj = kernel::single('cardcoupons_mdl_cards_type');
        $cards_type_arr = $cards_type_obj->getList("*",array(),0,-1,"handle ASC");
		$this->pagedata['cards_type_arr'] = $cards_type_arr;

        $this->pagedata['title'] = app::get('b2c')->_('添加内部卡券');
        ...
        $this->singlepage('admin/cards/detail/frame.html');
    }
    ...
    function _editor($type_id){
		...
		if($card_style[0]['value']){
			$this->pagedata['default_card_style']=$card_style[0]['value'];
		}else{
			 header('Content-Type:text/html; charset=utf-8');
            echo app::get('b2c')->_('请先设置卡券默认风格');
            exit;
		}
...
/*{{{*/
        ...
        //获取体检名目
        $cards_service_object = kernel::single('cardcoupons_mdl_cards_service');
        $cards_service_data = $cards_service_object->getList('*');

        $this->pagedata['cards_service_data'] = $cards_service_data;
        ...
        $sections = array(
            'basic'=>array(
                ...
                'options'=>'',
                'file'=>'admin/cards/detail/basic.html',
            ),
			/*'card'=>array(
                    'label'=>app::get('b2c')->_('卡密设置'),
                    'options'=>'',
                    'file'=>'admin/cards/detail/card.html',
            ),*/
			'style'=>array(
                    'label'=>app::get('b2c')->_('卡风格设置'),
                    'options'=>'',
                    'file'=>'admin/cards/detail/card_style.html',
            ),
            /*'adj'=>array(
                'label'=>app::get('b2c')->_('配件'),
                'options'=>'',
                'file'=>'admin/cards/detail/adj.html',
            ),*/
            'content'=>array(
                'label'=>app::get('b2c')->_('详细介绍'),
                'options'=>'',
                'file'=>'admin/cards/detail/content.html',
            ),
            'msg_templet'=>array(
                'label'=>app::get('b2c')->_('短信模板'),
                'options'=>'',
                'file'=>'admin/cards/detail/edtmpl.html',
            ),
            /*'params'=>array(
                'label'=>app::get('b2c')->_('属性参数'),
                'options'=>'',
                'file'=>'admin/cards/detail/params.html',
            ),
            'rel'=>array(
                    'label'=>app::get('b2c')->_('相关商品'),
                    'options'=>'',
                    'file'=>'admin/cards/detail/rel.html',
            ),
            'seo'=>array(
                'label' => app::get('b2c')->_('HTML页面参数设置'),
                'options'=>'',
                'file'=>'admin/cards/detail/seo.html'
            ),*/
        );
...
/*}}}*/
    ...
    function toAdd(){
		...
		/*$_POST['goods']['oldstore']?$_POST['goods']['oldstore']:0;
		$pass_store=$_POST['goods']['oldstore']+$cards_result['store'];
		$oGoods->update(array('store'=>$pass_store),array('goods_id'=>$goods['goods_id']));*/
    ...
    function toAddCard($cards_id=''){
        if($cards_id)$cards['card_id']=$cards_id;
		$cards=$_POST['cards'];
		if($cards['pass'])unset($cards['pass']);
        $cards['name']			=$_POST['goods']['name'];
		$cards['source']		='internal';
        $cards['rules']			=$cards['rules']['num'];
        $cards['goods_id']		=$_POST['goods']['goods_id'];
        $cards['from_time']		=strtotime($cards['from_time'].' '.$_POST['_DTIME_']['H']['cards[from_time'].':'.$_POST['_DTIME_']['M']['cards[from_time']);
        $cards['to_time']		=strtotime($cards['to_time'].' '.$_POST['_DTIME_']['H']['cards[to_time'].':'.$_POST['_DTIME_']['M']['cards[to_time']);
		$cards['msg_templet']=$_POST['msg_templet'];
		$cards['default_type'] = $cards['default_type']?$cards['default_type']:0;
		$cards['default_score'] = $cards['default_score']?$cards['default_score']:0;
		$cards['default_goods_id'] = $cards['default_goods_id']?$cards['default_goods_id']:0;
		$cards['default_goods_num'] = $cards['default_goods_num']?$cards['default_goods_num']:0;
        $cards['message'] = htmlspecialchars($_POST['cards']['message']);
		if(!$cards_id)$cards['createtime']=time();
        $cards['lasttime']=time();
        /*
         * Array
                (
                    ...
                    [source] => internal
                    [passset] => manual
                    [createtime] => 1432285345
                    [lasttime] => 1432285345
                )
         * */
		/*$solutions=array();
		if($cards['solution']['goods']){
			$key_type='01';//实体卡
			if(!$cards['rules']){
				$return['error']='套餐上限数量为空';
				$return['result']=false;
				return $return;
			}
		}
		if($cards['solution']['package']){
			$key_type='02';//体检卡
		}
		if($cards['solution']['goods']&&$cards['solution']['package']){
			$key_type='03';//复合卡
			if(!$cards['rules']){
				$return['error']='套餐上限数量为空';
				$return['result']=false;
		...
		$i=0;
		foreach($cards['solution'] as $key=>$value){
			
			foreach($cards['solution'][$key] as $ckey=>$cvalue){
				$solutions[$i]['card_id']=$cards['card_id'];
				$solutions[$i]['key_type']=$key_type;
				if($key=='goods'){
					$solutions[$i]['goods_id']=$cvalue;
				}else{
					$solutions[$i]['services_id']=$cvalue;
				}
				$i++;
				//$service_re=$obj_service->insert($service);	
			}
		}*/
		$cards['card_solution']=$solutions;
        $card_return=kernel::single('cardcoupons_mdl_cards')->save($cards);
		if(!$cards['card_id']){
		...
		$obj_service=kernel::single('cardcoupons_mdl_cards_solution');
		$obj_service->delete(array('card_id'=>$cards['card_id']));
		$key_type=$cards['type_id'];
		if($cards['solution']['goods']){
			//$key_type='01';//实体卡
			if(!$cards['rules']){
				$return['error']='套餐上限数量为空';
		...
		foreach($cards['solution'] as $key=>$value){
			if(!is_array($cards['solution'][$key])){
				$package_value=$cards['solution'][$key];
				unset($cards['solution'][$key]);
				$cards['solution'][$key][0]=$package_value;
			}
			foreach($cards['solution'][$key] as $ckey=>$cvalue){
				$service=array();
				$service['card_id']=$cards['card_id'];
				$service['key_type']=$key_type;
				if($key=='goods'){
					$service['goods_id']=$cvalue;
				}else{
					$service['services_id']=$cvalue;
				}
			$service_re=$obj_service->insert($service);	
			}
		}
        
       /* $cards['pass']=$_POST['cards']['pass'];
		$cards['num']=$_POST['goods']['store'] - $_POST['goods']['oldstore'];

        $return=$this->dispose_pass($cards);*/
    ...
    function dispose_pass($data){
        ...
        switch ($data['passset']){
            case 'auto':
                $pass_array=$pass_obj->auto_pass($data,$pass_error);
                break;
            case 'ftp':
        ...
        if(!is_array($pass_array) || empty($pass_array)){
			...
			/*判断录入卡号是否已存在 start*/
			$old=$pass_obj->getList('*',array('card_no'=>$card_no_arr));
			if($old){
            ...
            foreach($pass_array as $key=>$value){
                ...
                $value['from_time']=$from_time?$from_time:$data['from_time'];
                $value['to_time']=$to_time?$to_time:$data['to_time'];
				if(!$value['from_time']||!$value['to_time']||!$value['card_id']){
					$error_pass[]=$value['card_no'];
					continue ;
				}
    ...
    function edit($goods_id){
		...
		//获取卡券信息
		$oCards=kernel::single('cardcoupons_mdl_cards');
		$cards=$oCards->dump(array('goods_id'=>$goods_id),'*','default');
        ...
        $this->pagedata['cards']['message'] = htmlspecialchars_decode($cards['message']);
        $this->singlepage('admin/cards/detail/frame.html');
    }
    ...
    function get_related_product(){
        ...
        header('Content-Type:text/html; charset=utf-8');
        echo $render->fetch('admin/cards/detail/ajax_rel_items.html','b2c');exit;
    }
    ...
    function set_spec(){
        ...
        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
        $this->display('admin/cards/detail/spec/set_spec.html');
    }
...
//        $this->pagedata['specs'] = $oSpec->getList('spec_id,spec_name,spec_memo',null,0,-1);
        ...
        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
        $this->display('admin/cards/detail/spec/set_spec_desc.html');
    }
...
//            'spec_image_id' => $_POST['spec']['specImageId'],
        ...
        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
        $this->display('admin/cards/detail/spec/spec_value.html');
    }
...
//        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
        ...
        $this->display('admin/cards/detail/spec/spec.html');
    }
    ...
    function update(){
        ...
        header("Cache-Control:no-store");
        header('Content-Type:text/html; charset=utf-8');
        $side_bar = $this->fetch('admin/cards/detail/sidebar.html');
        $goods_body = $this->fetch('admin/cards/detail/page.html');
    ...
    function addGrp(){
        ...
        $this->display('admin/cards/detail/adj/info.html');
    }
    ...
    function doAddGrp($goodsid){
        ...
        $this->display('admin/cards/detail/adj/row.html');
    }
    ...
    function specValue(){
        ...
        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
        $this->display('admin/cards/detail/spec/spec_value_tmpl.html');
    }
    ...
    function showfilter($type_id){
        ...
        $this->display('admin/cards/filter_addon.html');
    }

    function selAlbumsImg(){
        $this->pagedata['selImgs'] = explode(',',$_POST['selImgs']);
        $this->pagedata['img'] = $_POST['img'];
        $this->display('admin/cards/detail/spec/spec_selalbumsimg.html');
    }
    ...
    function set_mprice(){
        ...
        $this->display('admin/cards/detail/level_price.html');
    }
    ...
    function getSpecHtml($goods_id){
        ...
        $this->_editor($goods['type']['type_id']);
        $this->pagedata['goods'] = $goods;
        $html = $this->display("admin/cards/detail/spec/spec.html");
