...
class cardcoupons_ctl_admin_cardsbak extends desktop_controller{
...
function test(){
    ...
    // $re=kernel::single('cardcoupons_mdl_cards_pass')->update(array('card_code'=>'x2x1x'),array('card_id'=>'285','card_pass_id'=>array('439906','439907')));
    $cards=kernel::single('cardcoupons_mdl_cards_pass')->getList('card_no,card_pass',array('card_pass_id'=>array('439588')));
    // $cards1=kernel::single('cardcoupons_mdl_cards_pass')->getRow('*',array('card_pass_id'=>'439585'));
    // $cards2=kernel::single('cardcoupons_mdl_cards_pass')->dump(array('card_pass_id'=>'439585'),'*');
    $re = kernel::single('cardcoupons_mysqlkey')->getMysqlKeyList();
    // $re = base_kvstore::instance('cardcouponsCardPass')->delete('mysql_key_arr');
    // $keyInKv = array();
    // $re = kernel::single('cardcoupons_mysqlkey')->storeKey();
     $keyInKv1=kernel::single('cardcoupons_mysqlkey')->getMysqlKeyList();
     $keyInKv2=kernel::single('cardcoupons_mysqlkey')->getCurrentMysqlKey();
    // $obj_pass= kernel::single('cardcoupons_mdl_cards_pass');
    // $card_pass=$obj_pass->getList('*',array('order_id'=>'11','type'=>'virtual'));
    echo "<pre>";
    // var_export(array('1'=>$keyInKv1,'2'=>$cards2));
    var_export(array('re'=>$re,'cards'=>$cards));
}

function testsave(){
    
    $mdl = kernel::single('cardcoupons_mdl_cards_pass');
    $save_data =array (
  'from_time' => 1556640000,
  'to_time' => 1559232000,
  'card_no' => '010200017095',
  'card_pass' => '123asd',
  'passset' => 'auto',
  'source' => 'internal',
  'createtime' => 1557914463,
  'batch' => '1905151800551',
  'card_id' => '284',
  'card_name' => '20190108test',
  'is_send' => 'false',
  'type' => 'virtual',
  'lasttime' => 1557914463,
  'card_pass_id' => '439572',
