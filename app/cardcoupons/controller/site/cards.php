<?php
class cardcoupons_ctl_site_cards extends b2c_frontpage{
    ...
    function index($company=""){
		...
		$this->pagedata['company_description'] = $this->get_company_description($company);

        //Session未丢失 可直接进入领取页面
        ...
        if($_SESSION['account']['card']){
            ...
            $member_array = $cards_object->get_card_member($_SESSION['account']['card']);
            $cards_pass_data = $cards_pass_object->getList("*",array('card_no'=>$member_array['uname'],'disabled'=>'false','source'=>'internal'));
            if(empty($cards_pass_data)){
                $this->auto_logout();
                $this->set_tmpl("card_default");
                $this->page("site/login.html");
            }else{
                if($cards_pass_data[0]['status'] == 3 || $cards_pass_data[0]['status'] == 4){
                    $this->splash('success', $this->gen_url( array('app'=>'cardcoupons', 'ctl'=>'site_member', 'act'=>'orders')), app::get('physical')->_('该卡号已经兑换成功，正在跳转到订单页面!'));
                    exit();
                }
                
                if($cards_pass_data[0]['status'] == 1 || $cards_pass_data[0]['status'] == 2){
                    $this->splash('success', $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'showcard','arg'=>base64_encode($cards_pass_data[0]['card_no']))), app::get('physical')->_(' 您已经登陆正在跳转到兑换页面!'));
                    exit();
                }
    ...
    function checkcardnum(){
        ...
        $cards_pass_object = $this->app->model('cards_pass');
        $card_num = trim($obj_filter->check_input($_POST['card_num']));
        if(strlen($card_num) > 0){
            if(!preg_match("/^\d{12}$/",$card_num)){
                echo "<span class='font-red'>卡号格式有误，请重新输入！</span>";
                exit();
            }
            $cards_pass_data =  $cards_pass_object->getList('card_pass_id', array('card_no' => $card_num,'status'=>array(1,2,3,4),'disabled'=>"false",'source'=>'internal'));
            if(!empty($cards_pass_data)){
                echo "<a href='".$this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'showcard','arg'=>base64_encode($card_num)))."' target='blank'>查看卡内产品</a>";
                exit;
            }
        }
    }
...
*/
    function login(){
        ...
        $verifycode = trim($obj_filter->check_input($_POST['verifycode']));
        ...
        if(!empty($cards_pass_data)){
            ...
            if($cards_pass_object->checkuname($card_num)){
                //判断用户密码，走登陆接口
                $_POST=array(
                    'uname' => $card_num,
                    'password' => $password,
                ...
                $params['redirect']= base64_encode($this->gen_url(array('app'=>'b2c','ctl'=>'site_passport','act'=>'post_login','arg'=>base64_encode($url))));
                if($cards_pass_object->login($params)){
                    if(in_array($cards_pass_data[0]['status'],array(3,4))){
                        $url = $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_member','act'=>'orders'));
                        echo json_encode(array('status'=>'success','url'=>$url,'msg'=>''));
                        exit();
                    }
                    $last_url = $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'showcard','arg'=>base64_encode($card_num)));
                    echo json_encode(array('status'=>'success','url'=>$last_url,'msg'=>''));
                    exit();
                }
            }else{
                //走注册,登陆接口
                $email = $card_num."@youfu.com";
                if($cards_pass_object->register($card_num,$password,$email)){
                    //如果是第一次登陆，激活卡号 status为 2
                    if(in_array($cards_pass_data[0]['status'],array('1'))){
                        $card_pass_object = kernel::single("cardcoupons_mdl_cards_pass");
                        $card_pass_object->update(array('status'=>'2'),array('card_no'=>$card_num));
                    }
                        $_POST=array(
                            'uname' => $card_num,
                            'password' => $password,
                        ...
                        $params['redirect']= base64_encode($this->gen_url(array('app'=>'b2c','ctl'=>'site_passport','act'=>'post_login','arg'=>base64_encode($url))));
                        if($cards_pass_object->login($params)){
                            if($cards_pass_data[0]['status'] == '0'){
                                $cards_pass_object->update(array('status'=>'1'),array('card_pass_id'=>$cards_pass_data[0]['card_pass_id']));
                            }
                            $last_url = $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'showcard','arg'=>base64_encode($card_num)));
                            echo json_encode(array('status'=>'success','url'=>$last_url,'msg'=>''));
                            exit();
    ...
    function showcard($card_no){
        ...
        if(in_array($cards_pass_data[0]['status'],array(3,4))){
            $this->redirect(array('app'=>'cardcoupons','ctl'=>'site_member','act'=>'orders','full'=>1));
            die();
        }

        //卡号校验
        $cards_object = $this->app->model('cards');
        ...
        $this->set_tmpl("card_main");
        $sessObj = kernel::single('base_session');
        $sessObj->start();
        $kvstoreKeyname = 'showcard0' . $sessObj->sess_id();
        $time = time();
        base_kvstore::instance('cardcouponsSiteCards')->store($kvstoreKeyname , $time , 3600);
        //$_SESSION['showcard'] = $time;
        if($_SESSION['account']['card']){
            //检测是否是同一卡券登陆
            $card_member_data = $cards_object->get_card_member($_SESSION['account']['card']);
            if($card_member_data['uname'] != $card_no){
                $this->auto_logout();
                $this->splash('failed', $this->gen_url( array('app'=>'cardcoupons', 'ctl'=>'site_member', 'act'=>'index')), app::get('physical')->_('该卡号已经兑换成功，正在跳转到订单页面!'));
                exit();
            }
            if($cards_info['card_type'] == "02"){
                $url = $this->gen_url(array('app'=>'physical','ctl'=>'site_exchange','act'=>'index','args'=>array('2','1','0',$cards_info['goods_id'],$card_no,base64_encode($cards_pass_data[0]['card_pass_ori']))));
            ...
            $this->page('site/shownomembercard.html');
        }
    }
    ...
    function card_data(){
        ...
        if($showcard > 0){
            ...
            $url = $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'showcard','arg'=>base64_encode($card_pass_no)));
           ...
           $this->pagedata['member_id'] = $arrMember['member_id'];
            ...
            if($card_data[0]['type_id'] == "01"){
                ...
                if($goods_card_num != 0 || $goods_num != 0){
                    $cardhtml = "distribution1.html";
                }else{
                    $cardhtml = "receiver.html";
                }
            }elseif($card_data[0]['type_id'] == "02"){
                $url = $this->gen_url(array('app'=>'physical','ctl'=>'site_exchange','act'=>'index','args'=>array('2','1','0',$obj_filter->check_input($_POST['cardgoods']),$obj_filter->check_input($_POST['card_pass_no']),base64_encode($card_pass_data[0]['card_pass_ori']))));
                header('Content-Type: text/html; charset=utf-8');
                header('Location:'.$url);
                die();
            ...
            $sessObj = kernel::single('base_session');
            $sessObj->start();
            $kvstoreKeyname = 'card_time0' . $sessObj->sess_id();
            base_kvstore::instance('cardcouponsSiteCards')->store($kvstoreKeyname , $time , 3600);
    ...
    $this->_cardFreezList =array();
        //session 数据开始
        kernel::single('base_session')->start();
        //url
        $this->begin(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'card_data'));

        //如果用户没有登陆，需要他们跳转到 卡券登陆页面
        kernel::single('base_session')->start();
        if(!$_SESSION['account']['card']){
            $this->auto_logout();
            $this->end(true, $this->app->_("卡券用户登陆超时！"), $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'index')),'',true);
            die();
        }
        ...
        //获取会员信息
        $arrMember = $cards_object->get_card_member($_SESSION['account']['card']);

        $card_status = $cards_top_info[0]['status'];
        //获取可兑金额
        $card_change_num = $obj_card['rules'] - $cards_top_info[0]['exchange_num'];

        if(!$card_status == '2' || !$card_status == '9'){
            if($card_status == '3')
            $this->end(true, $this->app->_("该卡券已经兑换！"), $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'index')),'',true);
            die();
        }
        ...
        if(empty($_POST['goodsinfo'])){
            $this->end(true, $this->app->_("无商品信息！"), $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'index')),'',true);
            die();
        }
        $allCart = array();
        foreach($obj_filter->check_input($_POST['goodsinfo']) as $k=>$v){
            ...
            if($v['goods']['goods_card'] != '0' || $v['goods']['virtual_card'] != '0'){
                if(!$this->check_card_num($v['goods'])){
                    $this->cancelFreezKv();//不满足可用卡密库存，直接返回前去除冻结数
                    $this->end(true, $this->app->_("无库存，请重新选择商品！"), "",'',true);
                    die();
                }
            }
...
// sleep(30);
        ...
        if(count($_POST['goodsinfo']) != count($allCart['object']['goods'])){
            $this->cancelFreezKv();//购物车信息与提交信息不等时，直接返回前去除冻结数
            $this->end(true, $this->app->_("所选产品无库存或者没有上架，请重新选择产品！"), "",'',true);
            die();
        }
        // echo "<pre>";var_dump($_POST['goodsinfo']);var_dump($allCart['object']['goods']);exit;
        ...
        foreach($temp_split_order as $store_id=>$sgoods){
             foreach($sgoods['slips'] as $order_sp=>$sorder){
                //判断是否支持的配送方式
                if(!$sorder['shipping']){
                    $this->cancelFreezKv();//不支持配送返回前去除冻结数
        ...
        foreach($temp_split_order as $store_id=>$sgoods){
            foreach($sgoods['slips'] as $order_sp=>$sorder){
                ...
                $sum_total_amount +=$order_data['total_amount'];
                if($obj_card['change_way'] == 'num'){
                    if($sum_itemnum > $card_change_num){
                        $this->cancelFreezKv();//超量兑换返回前去除冻结
                ...
                if ($result){
                    ...
                    $this->cancelFreezKv();//订单创建失败时去除冻结数
                    $db->rollback();
                }
                ...
                if ($result){
                    ...
                    //匿名卡券将手机号更新至匿名会员
                    $obj_mem->update(array('order_num'=>$order_num,'mobile'=>$_POST['delivery']['ship_mobile']), array('member_id'=>$order_data['member_id']));
                    /** 兑换卡券订单处理  start**/
                    if ($order_data){
                        // 模拟支付流程
                        ...
                        if (!$is_payed){
                            ...
                            $this->cancelFreezKv();//订单自动支付失败时去除冻结数
                            $this->end(false, $msg, $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'index')));
                        }
                        ...
                        $this->cancelFreezKv();   //订单后续动作之后去除冻结数
          }
        ...
        if($flag){
            ...
            if(count($aOrders)>1){
                $this->end(true, $this->app->_("卡券兑换成功！"), $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_member','act'=>'orders')),'',true);
            }else{
                $this->end(true, $this->app->_("卡券兑换成功！"), $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_member','act'=>'orders')),'',true);
            }
        }else{
            $this->end(false, $msg, $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'index')),true,true);
        }
    ...
    function logout(){
        ...
        $this->redirect(array('app'=>'cardcoupons','ctl'=>'site_cards','act'=>'index','full'=>1));
    }
    ...
    function check_card_num($card_info = array()){
        ...
        //获取原有冻结，没有则设置
        if ($card_info['goods_card'] != 0){
          $tmpType = 'entity';
          $tmpNum = $card_info['goods_card'];
        }else{
          $tmpType = 'virtual';
          $tmpNum = $card_info['virtual_card'];
        }
        $kvName = $cards_data['card_id'].$tmpType;
        $this->_cardFreezList[$kvName] = $tmpNum;
        base_kvstore::instance('cardcouponsSiteCards')->fetch($kvName , $oriFreezNum);
        $freezNum =  $oriFreezNum + $tmpNum;
        base_kvstore::instance('cardcouponsSiteCards')->store($kvName, $freezNum);
        error_log("=-=-=-=card+++-=-=-=-=-".PHP_EOL.var_export(array($kekvNamey,$tmpNum,$freezNum),1).PHP_EOL,3,DATA_DIR."/card_freeze_log.log");

        //
        // echo "<pre>";var_dump(array($cards_data,$card_info));exit;
        if($cards_data['source'] == "internal" && $card_info['goods_card'] != 0){
            ...
            //计算可用库存，减去总冻结
            $useableStore = $cards_pass_object->count(array("card_id"=>$cards_data['card_id'],"status"=>'0','ex_status'=>'true','source'=>'internal','disable'=>'false','type'=>'entity'));
            if ($useableStore - $freezNum < 0){
              return false;
            }
        }
        if($cards_data['source'] == "external" && $card_info['goods_card'] != 0){
            ...
            //计算可用库存，减去总冻结
            $useableStore = $cards_pass_object->count(array("card_id"=>$cards_data['card_id'],"status"=>'0','ex_status'=>'true','source'=>'external','disable'=>'false','type'=>'entity'));
            if ($useableStore - $freezNum < 0){
              return false;
            }            
        }
        if($cards_data['source'] == "external" && $card_info['virtual_card'] != 0){
            ...
            //计算可用库存，减去总冻结
            $useableStore = $cards_pass_object->count(array("card_id"=>$cards_data['card_id'],"status"=>'0','ex_status'=>'true','source'=>'external','disable'=>'false','type'=>'virtual'));
            if ($useableStore - $freezNum < 0){
              return false;
            }            
        }
     ...
     * @return bool
     */
    function get_company_description($company){
        if($company){
            $theme_url =THEME_DIR. "/".kernel::single('site_theme_base')->get_default();
            $file = $theme_url."/images/company/".$company."/description.php";
            if(file_exists($file)){
                require($file);
                return $description;
            }else{
    ...
    //完成时去除冻结数字
    function cancelFreezKv()
    {
      foreach ($this->_cardFreezList as $key => $value) {
        base_kvstore::instance('cardcouponsSiteCards')->fetch($key,$savedValue);
        error_log("=-=-=-=card-=-=-=-=-".PHP_EOL.var_export(array($key,$savedValue),1).PHP_EOL,3,DATA_DIR."/card_freeze_log.log");
        $newValue = $savedValue - $value;
        base_kvstore::instance('cardcouponsSiteCards')->store($key,$newValue);
      }
    } 
