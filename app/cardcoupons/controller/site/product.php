...
class cardcoupons_ctl_site_product extends cardcoupons_frontpage{
    ...
    function call(){
        ...
        if(method_exists($this,$action)){
            ...
            $objSchema = app::get('b2c')->model('goods/schema');
    ...
    function getVirtualCatById($cat_id=0){
        $vobjCat = app::get('b2c')->model('goods_virtual_cat');
        ...
        foreach($result as $k=>$v){
			...
			$result[$k]['url']=$this->gen_url(array('app'=>'b2c','ctl'=>'site_gallery','act'=>app::get('b2c')->getConf('gallery.default_view'),'args'=>array(implode(',',$cat_id),$filter)));
        }
    ...
    public function index() {
        ...
        if(!$this->id){
            ...
            $rs = $objGoods->dump(array('goods_id'=>$this->id),'goods_id,goods_kind');
            if(!$rs || empty($rs)){
                $this->splash('failed', 'back', app::get('b2c')->_('无效商品！<br>可能是商品未上架'));
            }
			//判断是否卡券商品
			if($rs['goods_kind'] != "card"){
				$this->splash('failed', 'back', app::get('b2c')->_('非卡券商品！'));
			}
			//判断是否关联卡券
			$obj_cards = $this->app->model('cards');
			$card_info = $obj_cards->dump(array('goods_id'=>$this->id),'card_id,type_id,source');
            if(!$card_info || empty($card_info)){
                $this->splash('failed', 'back', app::get('b2c')->_('无效卡券商品！'));
            }
			$this->pagedata['type_id'] = $card_info['type_id'];
			$this->pagedata['source'] = $card_info['source'];
        }
       ...
       $this->pagedata['specShowItems'] =$specValue;
        ...
        if($aGoods_list[0]['act_type'] == 'timedbuy'){
            ...
            if(isset($act_id[0]['aid'])){
                ...
                $this->pagedata['goodshtml']['store'] = kernel::single('timedbuy_goods_detail_store')->cardshow($gid,$aGoods);
                $this->pagedata['timedbuy'] = 'true';
                $this->pagedata['NOWTIME'] = time();
            }else{
                $this->pagedata['goodshtml']['store'] = kernel::single('b2c_goods_detail_store')->cardshow($gid,$aGoods);
            }
        }else{
            $this->pagedata['goodshtml']['store'] = kernel::single('b2c_goods_detail_store')->cardshow($gid,$aGoods);
        }
    ...
    */
    ...
    function viewpic($goodsid, $selected='def'){
        ...
        $this->pagedata['company_name'] = str_replace("'","&apos;",htmlspecialchars(app::get('b2c')->getConf('system.shopname')));
        ...
        if(!$dImg){
            ...
            $dImg[$id]=array(
                'gimage_id'=>$id,
                'goods_id'=>$goodsid,
                'small'=>(app::get('b2c')->getConf('site.default_small_pic')),
                'big'=>(app::get('b2c')->getConf('site.default_big_pic')),
                'thumbnail'=>(app::get('b2c')->getConf('site.default_thumbnail_pic'))
            );*/
        }
        if(is_array($dImg)){
    ...
    function selllog($gid,$nPage=0){
        $nPage = $nPage?$nPage:1;
        $oPro = app::get('b2c')->model('products');
    ...
    function goodspics($goodsId,$images=array(),$imgGstr=''){
        $Goods=app::get('b2c')->model('goods/gimage');
        $objGoods = app::get('b2c')->model('trading/goods');
        $gimg=$_POST['gimages'];
    ...
    private function _get_servicelist_by($servicelist,$sign=false)
    {
        ...
        foreach( kernel::servicelist($servicelist) as $object ) {
			...
			//卡券商品去除购物车按钮  2015/7/11
			if(get_class($object) == 'b2c_site_product_btn_buy') continue;
    ...
    function goodsBodyContent($gid=0, $aGoods=null) {
        ...
        $aGoods['description'] = preg_replace("/(\<img[\s\S]+)src=/Us","\\1src='".app::get('b2c')->res_url."/images/transparent.gif' img-lazyload=",$aGoods['description']);
        /**** begin 商品评论 ****/
        $aComment['switch']['ask'] = app::get('b2c')->getConf('comment.switch.ask');
        $aComment['switch']['discuss'] = app::get('b2c')->getConf('comment.switch.discuss');
        if($aComment['switch']['ask'] == "on") {
            $aComment['askCount'] = app::get('b2c')->model("member_comments")->count(array('object_type'=>'ask', 'display'=>'true', 'type_id'=>$gid));
        }
        if($aComment['switch']['discuss'] == "on") {
            $aComment['discussCount'] = app::get('b2c')->model("member_comments")->count(array('object_type'=>'discuss', 'display'=>'true', 'type_id'=>$gid, 'for_comment_id'=>0, 'comments_type'=>'1'));
        }
        ...
        /**** start 销售记录 ****/
        $this->pagedata['sellLogList'] = app::get('b2c')->model('products')->getGoodsSellLogList($gid,0,1);
        $this->pagedata['sellLog']['display'] = array(
            'switch' => app::get('b2c')->getConf('selllog.display.switch'),
            'limit' => app::get('b2c')->getConf('selllog.display.limit'),
            'listnum'=>app::get('b2c')->getConf('selllog.display.listnum')
        );
        /**** end 销售记录 ****/
    ...
    public function ajax_selllog(){
        ...
        $objProduct = app::get('b2c')->model('products');
        $sellLogList = $objProduct->getGoodsSellLogList($gid,$page,app::get('b2c')->getConf('selllog.display.listnum'));
        ...
        $oSellLog = app::get('b2c')->model('sell_logs');
    ...
    public function ajax_see(){
        ...
        $this->pagedata['image_set'] = $imageDefault;
        $this->pagedata['defaultImage'] = $imageDefault['S']['default_image'];
        $this->pagedata['setting']['buytarget'] = app::get('b2c')->getConf('site.buy.target');
