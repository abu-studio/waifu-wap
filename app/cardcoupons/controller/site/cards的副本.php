    ...
    function showcard($card_no){
        ...
        if($_SESSION['account']['card']){
            ...
            if($cards_info['card_type'] == "02"){
                $url = $this->gen_url(array('app'=>'physical','ctl'=>'site_exchange','act'=>'index','args'=>array('2','1','0',$cards_info['goods_id'],$card_no,base64_encode($cards_pass_data[0]['card_pass']))));
       ...
       if(!empty($card_id) && !empty($card_pass_no)){
           ...
           $this->pagedata['member_id'] = $arrMember['member_id'];
            ...
            if($card_data[0]['type_id'] == "01"){
                ...
                $url = $this->gen_url(array('app'=>'physical','ctl'=>'site_exchange','act'=>'index','args'=>array('2','1','0',$obj_filter->check_input($_POST['cardgoods']),$obj_filter->check_input($_POST['card_pass_no']),base64_encode($card_pass_data[0]['card_pass']))));
                header('Content-Type: text/html; charset=utf-8');
                header('Location:'.$url);
                die();
...
sleep(30);
    ...
    function cancelFreezKv()
    {
      foreach ($this->_cardFreezList as $key => $value) {
        ...
        error_log("=-=-=-=-=-=-=-=-".PHP_EOL.var_export(array($key,$savedValue),1).PHP_EOL,3,DATA_DIR."/xxxxxxxxx.log");
