<?php
class cardcoupons_ctl_site_cards2 extends b2c_frontpage{
	function __construct($app){
        ...
        if($_SESSION['sfsc']['model'] != ""){
            $this->pagedata['sfsc_model']  = $_SESSION['sfsc']['model'];
            $this->sfsc_model = $this->pagedata['sfsc_model']."/";
        }else{
            $this->pagedata['sfsc_model'] = "";
        }
    ...
    function login(){
        ...
        if(!empty($cards_pass_data)){
			...
			$last_url = $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards2','act'=>'showcard','arg'=>base64_encode($card_num)));
    ...
    function showcard($card_no){
        ...
        $sessObj->start();
        $kvstoreKeyname = 'showcard2' . $sessObj->sess_id();
        $time = time();
        base_kvstore::instance('cardcouponsSiteCards')->store($kvstoreKeyname , $time , 3600);
		if($cards_info['card_type'] == "02"){
			$url = $this->gen_url(array('app'=>'physical','ctl'=>'site_exchange','act'=>'index','args'=>array('1','1','0',$cards_info['goods_id'],$card_no,base64_encode($cards_pass_data[0]['card_pass_ori']))));
		...
		if($cards_info['change_way'] != "num"){
				$change_page = "site/{$this->sfsc_model}showmembercard_price.html";
			}else{
				$change_page = "site/{$this->sfsc_model}showmembercard.html";
			}
    ...
    function card_data(){
        ...
        if($showcard > 0){
            ...
            $url = $this->gen_url(array('app'=>'cardcoupons','ctl'=>'site_cards2','act'=>'showcard','arg'=>base64_encode($card_pass_no)));
       ...
       if(!empty($card_id) && !empty($card_pass_no)){
           ...
           $this->pagedata['member_id'] = $arrMember['member_id'];
            ...
            if($card_data[0]['type_id'] == "01"){
                ...
                $url = $this->gen_url(array('app'=>'physical','ctl'=>'site_exchange','act'=>'index','args'=>array('1','1','0',$obj_filter->check_input($_POST['cardgoods']),$obj_filter->check_input($_POST['card_pass_no']),base64_encode($card_pass_data[0]['card_pass_ori']))));
                header('Content-Type: text/html; charset=utf-8');
                header('Location:'.$url);
                die();
            ...
            $sessObj = kernel::single('base_session');
            $sessObj->start();
            $kvstoreKeyname = 'card_time2' . $sessObj->sess_id();
            base_kvstore::instance('cardcouponsSiteCards')->store($kvstoreKeyname , $time , 3600);
			$this->pagedata['card_time'] = $time;
            $this->page("site/{$this->sfsc_model}".$cardhtml);
    ...
    function creat_order(){
        ...
        kernel::single('base_session')->start();
        //url
        $this->begin(array('app'=>'cardcoupons','ctl'=>'site_cards2','act'=>'card_data'));
    ...
    function logout(){
        ...
        $this->redirect(array('app'=>'cardcoupons','ctl'=>'site_cards2','act'=>'index','full'=>1));
    }
    ...
    function check_card_num($card_info = array()){
        ...
        error_log("=-=-=-=card2+++-=-=-=-=-".PHP_EOL.var_export(array($kekvNamey,$tmpNum,$freezNum),1).PHP_EOL,3,DATA_DIR."/card_freeze_log.log");
    ...
    function cancelFreezKv()
    {
      foreach ($this->_cardFreezList as $key => $value) {
        ...
        error_log("=-=-=-=card2-=-=-=-=-".PHP_EOL.var_export(array($key,$savedValue),1).PHP_EOL,3,DATA_DIR."/card_freeze_log.log");
