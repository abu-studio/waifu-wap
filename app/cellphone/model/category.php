...
class cellphone_mdl_category extends dbeav_model{
    ...
    function getCatParentById($id){
        ...
        foreach((array)$result as $cat_key=>$cat_value){
            ...
            $result[$cat_key]['link']=app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_gallery','act'=>$default_view,'args'=>array(null,null,'0','','',$cat_value['cat_id'])));
        }
        return $result;
    }
    ...
    function getTreeList($pid=0, $listMark='all'){
        if($listMark == 'all'){
            ...
            foreach($aCat as $k => $row){
                ...
                $aCat[$k]['url'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_gallery','view'=>$default_view,'args'=>array(implode(",",$filters['cat_id']),$oSearch->encode($filters))));
            }
        }
        return $aCat;
    }
