...
class cellphone_ctl_admin_activity extends desktop_controller{
    ...
    function index(){
        $this->finder('cellphone_mdl_activity',array(
            ...
            'actions'=>array(
                array('label'=>app::get('b2c')->_('添加活动'),
                       'icon'=>'add.gif',
                       'href'=>'index.php?app=cellphone&ctl=admin_activity&act=create',
                     //'target'=>'_blank'
               ),
            ),
        ));
    ...
    function create(){
        $this->pagedata['reUrl'] = $this->router->gen_url(array('app'=>'cellphone','ctl'=>'admin_activity','act'=>'index'));
    ...
    function save(){
        $this->begin($this->router->gen_url(array('app'=>'cellphone','ctl'=>'admin_activity','act'=>'index'))); 
        list($act_id, $app) = explode('_', $_POST['unit_id']);
        ...
        $aData['position'] = $_POST['position'];
        if(!$_POST['position']){
         $this->end(false,'请选择所属板块');
        }
        $aData['layout'] = $_POST['layout'];
        if('index' == $aData['position']){
            if(!$_POST['layout']){
             $this->end(false,'请选择布局');
            }
        }
