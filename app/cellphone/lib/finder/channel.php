...
class cellphone_finder_channel{
    ...
    function column_edit($row){
        ...
        $row = $this->channel->getList('*',array('id'=>$row['id']));
        $row = $row[0];
		$html = '';
		if($row['disabled']=='false'){
			
	    $html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_channel','act'=>'edit','id'=>$row['id']) ) .'" >'.app::get('cellphone')->_('编辑').'</a>&nbsp;&nbsp;';
			
		if($row['is_active']=='false'){
				$html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_channel','act'=>'openActivity','id'=>$row['id']) ) .'" >'.app::get('cellphone')->_('开启').'</a>&nbsp;&nbsp;';
		}else{
				$html .= '<a href="'. $this->router->gen_url( array('app'=>'cellphone','ctl'=>'admin_channel','act'=>'closeActivity','id'=>$row['id']) ) .'" >'.app::get('cellphone')->_('关闭').'</a>&nbsp;&nbsp;';
			}
	  }
        return $html;
