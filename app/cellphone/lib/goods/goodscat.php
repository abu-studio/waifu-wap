<?php
class cellphone_goods_goodscat extends cellphone_cellphone
{
    ...
    public function get(){
        ...
        $sql="select cat_id,cat_name,parent_id,cat_logo,cat_path from sdb_b2c_goods_cat where    disabled='false' ".$filter." ORDER BY p_order,cat_id";
    ...
    public function child(){
        ...
        $cat_list=array();
        $is_cached=cachemgr::get('cellphone_goods_cat_child_'.$parent_id, $cat_list);
        if(!$is_cached){
            $mdl_category=$this->app->model('category');
            ...
            cachemgr::co_start();
            cachemgr::set('cellphone_goods_cat_child_'.$parent_id, $cat_list, cachemgr::co_end());
        }
    ...
    private function get_tree_cat($parent_id,$cache_key=''){
        $result=array();
        $is_cached=cachemgr::get('cellphone_goods_cat_tree_'.$parent_id, $result);
    ...
    private function get_cache_result($parent_id){
        ...
        cachemgr::set('cellphone_goods_cat_tree_'.$parent_id, $result, cachemgr::co_end());
        return $result;
    }
    ...
    private function format_cat($cat){
        if($cat['cat_logo']){
            $cat['image']=$this->get_img_url($cat['cat_logo'],'cs');
        }
        if($cat['cat_path']){
            $cat['cat_path']=$cat['cat_path'];
        }
        return $cat;
    }
}