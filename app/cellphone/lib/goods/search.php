...
class cellphone_goods_search extends cellphone_cellphone
{
    ...
    private function search(){
        ...
        if(is_object($searchApp)&&empty($filter['bn|in'])){
    ...
    protected function get_filter(){
        if($this->params['cat_id']){
            ...
            if(!cachemgr::get('cellphone_goods_virtual_cat_'.intval($vcatid), $vcat)){
                cachemgr::co_start();
                $vcat = $virCatObj->getList('cat_id,cat_path,filter,addon,cat_name',array('cat_id'=>intval($vcatid)));
        ...
        //如果在特定店铺内搜索 则传入store_id(数组或字符串) 进入过滤条件数组
    ...
    protected function get_bn(){
        $key='';
        if(!empty($this->params['bn'])){
            $key=json_decode($this->params['bn'],true);
    ...
    protected function _check($params){
        if(empty($params['cat_id'])&& !isset($params['key'])&&!isset($params['bn'])){
            $this->send(false,null,'分类ID和关键字、编号不能同时为空');
        }
    }
}
