   ...
   //获取banner轮播区域启用的内容
    function getbannerlist(){
	 $params = $this->params;
	  ...
	  $curtime = time();
      $sql= 'SELECT b.associate_id,b.associate_type,b.image_id,b.d_order FROM sdb_cellphone_banner AS b WHERE b.is_active="true" AND b.start_time <='.$curtime.'  AND b.end_time >'.$curtime.' ORDER BY b.d_order ASC  LIMIT '.($params['npage']-1)*$params['pageLimit'].','.$params['pageLimit'];
      $data = $banner->db->select($sql);
	 
	  if($data){
		 foreach($data as $key=>&$val){
		 $val['image_id']=$this->get_img_url($val['image_id'],$params['picSize']);
		 }
	...
	//获取后台配置文章
	function getartid(){
		$art_id = app::get('cellphone')->getConf('cellphone.art_id');
		$mdl_article_indexs = app::get('content')->model('article_indexs');
		$sql = ' select a.title,a.article_id from sdb_content_article_indexs as  a  
        where a.article_id ='.$art_id;
		$data = $mdl_article_indexs->db->selectrow($sql);
		if($data){
			$this->send(true,$data,app::get('cellphone')->_('文章标题'));
		}else{
			$this->send(true,null,app::get('cellphone')->_('没有数据'));
		}
	}
