 ...
 */
class cellphone_base_payment_mobilepay extends cellphone_cellphone
{
    ...
    */
    ...
    public function dopay($pay_object = 'order')
    {
        ...
        foreach ($aryorder_id as $item)
        {
            ...
            if ($arrOrders['payinfo']['pay_app_id'] != $sdf['pay_app_id'])
            {
                ...
                foreach ($obj_app_plugins as $obj_app)
                {
                    ...
                    if (isset($arr_class_name[count($arr_class_name) - 1]) && $arr_class_name[count
                        ($arr_class_name) - 1])
                    {
                ...
                $cost_payment = $objMath->number_multiple(array($objMath->number_minus(array($arrOrders['total_amount'],
                            $arrOrders['payinfo']['cost_payment'])), $arrPayment['setting']['pay_fee']));
                $total_amount = $objMath->number_plus(array($objMath->number_minus(array($arrOrders['total_amount'],
                            $arrOrders['payinfo']['cost_payment'])), $cost_payment));
        ...
        $sdf['merge_payment_id'] = $merge_payment_id;
        $rel_order = array(
            'rel_id' => $merge_payment_id,
            'bill_type' => 'payments',
            'pay_object' => 'order',
            'bill_id' => $sdf['merge_payment_id'],
            'money' => $money);
    ...
    #获取手机端支付方式（包含账户支付和预存款支付方式）
    public function getpayments()
    {

        $opay = app::get('ectools')->model('payment_cfgs');
        $aOld = $opay->getList('*', array('staus' => 'true'));

        $aData = array();
        foreach ($aOld as $val)
        {
            if ($val['platform'] == 'isapp')
            {
                $aData[] = $val;
            }
        }

        if (empty($aData) || !isset($aData))
        {
            $this->send('false', null, '没有可用的支付方式');
        }

        $this->send('true', $aData, 'succ');
    }
