...
,avg_point,freight_bear,store,store_freeze,notify_num,score,score_setting,weight,unit,min_buy,package_scale,package_unit,package_use,nostore_sell,disabled,spec_desc,brief,adjunct";
        ...
        //商品所在的店铺信息 added by cuiqw
        $storeinfo = app::get('business')->model('storemanger')->getRow('store_id,store_name',array('store_id'=>intval($aGoods_list[0]['store_id'])));
        $aGoods['store_name'] = $storeinfo['store_name'];
        // end
        ...
        //判断会员是否登录 若登录判断商品是否收藏 加入会员的浏览历史
...
WHERE p.goods_id ={$gid}";
        ...
        if($aGoods['freight_bear'] == 'member'){
            $aGoods['dlytype_info'] = $this->freight_detail($gid,$aGoods['store_id'],$aGoods['weight'],$area_id);
    ...
    /*  if(!$siteMember['member_id']){
        ...
        if($aGoods['act_type'] && $aGoods['act_type'] != 'normal' && $aGoods['act_type'] != 'package'){
            ...
            $aGoods['act_detail']['act_price'] = isset($aApply['last_price'])?$aApply['last_price']:$aApply['price'];
            $aGoods['act_detail']['personlimit'] = $aApply['personlimit'];
            $aGoods['act_detail']['start_time'] = $aAct[$aApply['aid']]['start_time'];
            $aGoods['act_detail']['end_time'] = $aAct[$aApply['aid']]['end_time'];
            $aGoods['act_detail']['act_name'] = $aAct[$aApply['aid']]['name'];// added by cuiqw
            ...
            $aGoods['current_price'] = $aGoods['act_detail']['act_price'];
        ...
        $this->send(true, $aGoods, app::get('b2c')->_('单个商品详细信息'));
    }
...
WHERE p.goods_id in (".implode(',',$goods_id).")";
        ...
        $aGoods['act_detail']['act_price'] = isset($aApply['last_price'])?$aApply['last_price']:$aApply['price'];
        $aGoods['act_detail']['personlimit'] = $aApply['personlimit'];
        $aGoods['act_detail']['start_time'] = $aAct['start_time'];
        $aGoods['act_detail']['end_time'] = $aAct['end_time'];
        $aGoods['act_detail']['act_name'] = $aAct['name'];// added by cuiqw
        $aGoods['current_price'] = $aGoods['act_detail']['act_price'];
        ...
        if($app_id == 'scorebuy'){
            if($siteMember['member_lv']>0 && $aApply['isMemLv'] == '1'){
                $memLvScoreObj = app::get('scorebuy')->model('memberLvScore');
                $memlvs = $memLvScoreObj->getMemLvScoreByIds($aApply['aid'],$gid);
                $aGoods['act_detail']['act_price'] = $memlvs[$siteMember['member_lv']]['last_price'];
                $aGoods['act_detail']['score'] = $memlvs[$siteMember['member_lv']]['score'];
            }else{
                $aGoods['act_detail']['act_price'] = $aApply['last_price'];
                $aGoods['act_detail']['score'] = $aApply['score'];
            }
        }
        $aGoods['dlytype_info'] = empty($aGoods['dlytype_info']['area'])?null:$aGoods['dlytype_info'];
    ...
    public function get_regions(){
        ...
        foreach((array)$objRegions->getList('region_id,local_name', array('region_grade'=>1,'disabled'=>'false'), 0, -1) as $key => $row){
            ...
            $aData[$row['region_id']]['region_id'] = $row['region_id'];// added by cuiqw
        }
        $region_info = array();
    ...
    private function region_structure($source, &$data){
        foreach((array)$source as $row){
            if(empty($data) || array_key_exists($row['p_region_id'],$data)){
                $data[$row['p_region_id']]['items'][$row['region_id']] = array('region_id'=>$row['region_id'],'local_name'=>$row['local_name']);
                array_shift($source);
    ...
    // 获取运费模板update by cuiqw 2014-5-8
    ...
    private function freight_detail($goods_id, $store_id, $weight, $area_id=0){
        ...
        $aReturn = array();
        $aReturn['area_id'] = $aDt['area'];
        $aReturn['local_name'] = $aDt['localname'];
        $aReturn['top_area_id'] = $aDt['parent'];
...
from sdb_b2c_member_comments as c  left join sdb_b2c_members as m on c.author_id=m.member_id 
...
group by  comment_id,i.goods_id order by time desc limit {$offset},{$limit}";
        ...
        foreach((array)$objComment->db->select($sql) as $val){
            ...
            // added by cuiqw
            $gpoint = app::get('b2c')->model('comment_goods_point')->getRow('goods_point',array('comment_id'=>$val['comment_id']));
            $val['goods_point'] = intval($gpoint['goods_point']);
            // end
            $aId[] = $val['comment_id'];
    ...
    // 到货通知功能 update by cuiqw 2014-5-8
    public function toNotify(){
        $params = $this->params;
        $must_params = array(
            'session'=>'会员登录session_id',
            'goods_id'=>'商品标识',
            'product_id'=>'货品标识',
        );
        $this->check_params($must_params);
        ...
        if(!empty($params['cellphone']) && !preg_match('/^(1[3458])-?\d{9}$/',$params['cellphone'])){
        ...
        if(!$ret){
            $this->send(false,null,app::get('b2c')->_('该货品不存在'));exit;
        }
        ...
        $data['cellphone'] = $phone ? $phone : null;
        if($member_goods->check_gnotify($data)){
            $this->send(false,null,app::get('b2c')->_('您已登记过'));exit;
        }else{
            if($member_goods->add_gnotify($member_id,$params['goods_id'],$params['product_id'],$email,$phone)){
