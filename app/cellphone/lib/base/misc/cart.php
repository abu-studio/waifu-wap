...
class cellphone_base_misc_cart extends cellphone_cellphone
{
    ...
    public function set_cart(){
        ...
        switch($type_temp){
            ...
            $aid = $applyObj->getOnActIdByGoodsId($aData['goods']['goods_id']);
            if($aid){
                $obj_business_activity = kernel::single('auction_business_activity');
                if (!$obj_business_activity->checkPersonLimit($aData['goods']['num'],$arrMember['member_id'],$aid,&$msg)){
                    $this->send(false, null, $msg);exit;
                }
            }else{
                $this->send(false, null, '以下原因导致无法购买:1.该活动已下单 <br /> 2:您没有拍下物品不能下单 <br /> 3:没有达到保留价');exit;
            }
            break;
        }
        ...
        if(isset($aData[1]) && $aData[1] == 'fastbuy'){
            switch($type_temp){
                ...
                break;
                case 'auction':
                $oCartGoods=kernel::single('auction_cart_object_goods');
                $oCartCoupon=kernel::single('auction_cart_object_coupon');
                $isfast = 5;
                break;
                default:
    ...
    public function update_item(){
        ...
        foreach($aCart as $row){
            if($row['obj_ident'] == $params['obj_ident']){
                $update_row = $row;
                break;
            }
        }
        ...
        $_flag = $mCartObject->update_object($params['type'],$params['obj_ident'],$arr_object);
        if(is_array($_flag) && isset($_flag['status']) && isset($_flag['msg'])){
            if($_flag['status']){
                $this->ajax_update = true;
                $this->update_obj_ident['ident'] = $params['obj_ident']; //值不同。修改 失败直接推出循环
                $msg = $_flag['msg'];
            }else{
    ...
    public function delete_item(){
        ...
        if($params['type'] == 'coupon'){
        ...
        }elseif($params['type'] == 'all' || empty($params['obj_ident'])){
            ...
            if(!$mCartObject->remove_object_part($params['type'], $params['obj_ident'], $arr_quantity, $msg)){
                $this->send(false, null, $msg);exit;
            }
        }else{
            if(!$mCartObject->remove_object($params['type'], $params['obj_ident'], $msg)){
                $this->send(false, null, $msg);exit;
            }
        }
        $picSize = in_array(strtolower($params['picSize']), array('cl', 'cs'))?strtolower($params['picSize']):'cl';
    ...
    public function _common($flag=0,$picSize,$isfast=0){
        if(!$isfast){
            ...
            kernel::single('auction_cart_fastbuy_goods')->get_fastbuy_arr(
                $_SESSION['S[Cart_Fastbuy]']['goods'],
                $_SESSION['S[Cart_Fastbuy]']['coupon'],
                $aCart
            );
        ...
        $aData['subtotal_consume_score'] = $aCart['subtotal_consume_score']; // 消费的总积分（如：用积分换优惠券) int
        ...
        $aData['store_count'] = $aCart['store_count']; // 优惠券优惠金额
        $aData['coupon'] = $aCart['coupon']; // 优惠券列表
        ...
        $aData['md5_cart_info'] = $aCart['md5_cart_info']; //购物车的md5_cart_info 信息
        //未享受的订单规则
    ...
    */
    ...
    function checkout($isfast=0){
      ...
      //会员重新选择收货地址时传入收货地址ID addr_id 
        $this->checkout_result($aCart, $params['addr_id']);
        $aData = array();
        $aData['md5_cart_info'] = $aCart['md5_cart_info'];
        $aData['delivery'] = !empty($aCart['delivery'])?$aCart['delivery']:null;

        /**加入平台的在线支付方式**/
        $obj_payment = kernel::single('ectools_api_payment');
        if(is_object($obj_payment)){
           $aData['pay_info'] = $obj_payment->get_all($sdf);
           $aData['pay_info'] = array_values($aData['pay_info']);
        }      
        //end
        ...
        foreach($aCart['order_split'] as $store_id => $items){
            ...
            $aData['order_items'][$store_id]['store_count'] = $aCart['store_count'][$store_id]; //优惠券
            ...
            $aData['order_items'][$store_id]['store_slip_subtotal'] = $totle; //店铺总金额合计（含运费）
            $aData['order_items'][$store_id]['goods_count'] = 0; //该店铺下要购买的商品类型数
        ...
        $aData['order_items'] = array_values($aData['order_items']);//数组转为数字0-n的数字索引
        //end
    ...
    function get_coupon(){
        ...
        if(empty($aData[$params['store_id']])){
          $this->send(true,null,'没有可用优惠券');
        }
        $this->send(true, $aData[$params['store_id']], '获取优惠券列表成功');exit;
    }
    ...
    function update_total($store_id=0,$no_coupon){
        ...
        if($store_id){
            ...
            $aData['store_count'] = array(
                ...
                'coupon_name' =>$aCart['coupon'][$store_id]['name'],//优惠券名称 add bu cuiqiwen
...
/*
    public function update_cart(){
        ...
        $must_params = array(
            'session'=>'登录标识',
            'cartinfo'=>'array(array(obj_ident,quantity))',
        );
        $this->check_params($must_params);
        ...
        $params['cartinfo'] = json_decode($params['cartinfo'],1);
        $cart_info = array();
        if(is_array($params['cartinfo']) && !empty($params['cartinfo'])){
            foreach($params['cartinfo'] as $item){
                $cart_info[$item['obj_ident']] = array('quantity'=>$item['quantity']);
            }
        }
        if(empty($cart_info)){
            ...
            $arr_args = array(
                'get' => array(),
                'post' => array('modify_quantity'=>$cart_info),
            );
            $_POST['extends_args'] = json_encode($arr_args);
            $aData = $this->_common(1);
            //$aData['subtotal_prefilter'] = $this->objMath->number_minus(array($aData['subtotal'], $aData['discount_amount_prefilter']));
            $promotion_subtotal = $this->objMath->number_minus(array($aData['subtotal'], $aData['subtotal_discount']));
        }
        $this->send(true, array(
                'promotion_subtotal'=>$promotion_subtotal,
                'extends_args'=>$_POST['extends_args'],
            ), 'success');exit;
    }
    */
}