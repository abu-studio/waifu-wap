...
class cellphone_base_member_passport extends cellphone_cellphone{
    ...
    public function login(){
		...
		if($uc = kernel::service("uc_user_login")){
        	...
        	$password_string = pam_encrypt::get_encrypted_password(trim($params['password']),'member',
                                                            array('login_name'=> trim($params['uname'])));
        ...
        if(empty($rows)){
            ...
            if(empty($mem_email_rows)){
                ...
                if(empty($mem_mobile_rows)){
                   $this->send(false,null,app::get('b2c')->_('用户名不存在'));
                }else{
    ...
    public function signup(){
        ...
        $must_params = array(
            'uname'=>app::get('b2c')->_('用户登录名'),
            'password'=>app::get('b2c')->_('登录密码'),
            'cellphonevcode'=>app::get('b2c')->_('短信验证码')
        );
        $this->check_params($must_params);
        ...
        //同步到ucenter
        $source_arr = array('pc','mobile','phone');//pc:电脑端，mobile：手机触屏，phone：手机app
        if(isset($params['source'])){
            if(in_array($params['source'],$source_arr)){
                $reg_data['source'] = $params['source'];
            }else{
                $this->send(false,null,'source参数错误');
            }
        }else{
            $reg_data['source'] = 'phone';
        }
        
        if(!$member_mdl->save($reg_data)){ // 存储失败
            $db->rollBack();
            $this->send(false,null,app::get('b2c')->_('注册失败'));
        }
       ...
       if($data[0]['mobile'] < 1){
		...
		if( $member_object = kernel::service("uc_user_edit")) {
			...
			if(!$member_object->uc_user_edit_pwd($aData)){
				$this->send(false,null, app::get('b2c')->_('第三方修改密码失败,请重试'));
			}
		}
		//同步到ucenter
        ...
        if(app::get('pam')->model('account')->save($sdf)){
            ...
            $this->send(false,null,app::get('b2c')->_('发送失败，请与商家联系'));
        }
