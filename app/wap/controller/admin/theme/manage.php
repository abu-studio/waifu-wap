...
class wap_ctl_admin_theme_manage extends desktop_controller{
    ...
    {
        $rows = app::get('base')->model('apps')->getList('app_id,status',array('installed'=>1));
        foreach($rows as $r)
        {
            //安装后未启动@Author:panbiaophp@163.com @DateTime:2019-06-21 14:13
            if ($r['app_id'] == 'wap' && $r['status'] == 'installed')
            {
                app::get('base')->model('apps')->update(array('status'=>'active'));
            }
        ...
        /** 获取所有已安装的模版 **/
        $all_themes = app::get('wap')->model('themes')->getList('*');
